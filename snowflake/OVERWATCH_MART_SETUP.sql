...
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS ROUTE_EMAIL VARCHAR(500);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_PRIMARY VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_SECONDARY VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_GROUP VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_TARGET VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS ROUTE_SOURCE VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS ROUTE_EVIDENCE VARCHAR(2000);
...
WITH base AS (
  SELECT
    ...
    NULL AS ROUTE_EMAIL,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Review') AS REVIEW_PRIMARY,
    NULL AS REVIEW_SECONDARY,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Lead') AS REVIEW_GROUP,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Lead') AS REVIEW_TARGET,
    'ALERT_RULE' AS ROUTE_SOURCE,
    'Matched alert rule or alert row route' AS ROUTE_EVIDENCE,
    COALESCE(
      r.SLA_HOURS,
...
SELECT
  ...
  CASE
    ...
    ELSE COALESCE(REVIEW_TARGET, ROUTED_ROUTE, WORKFLOW_ROUTE, 'DBA')
...
BEGIN
  ...
  INSERT INTO FACT_CHARGEBACK_DAILY (
    ...
    CHARGEBACK_READY, SCOPE_REVIEW, COST_ATTRIBUTION, ROUTE_SOURCE, ROUTE_EVIDENCE
  )
  (WITH query_hour AS (
    SELECT
      HOUR_START,
  ...
  INSERT INTO FACT_CHANGE_CONTROL_OPERABILITY_DAILY (
    ...
    FINDING_TYPE, ENTITY, WORKFLOW_ROUTE, REVIEW_TARGET, SEVERITY,
  ...
  (WITH evidence_rollup AS (
    SELECT
      ...
      MAX_BY(REVIEW_TARGET, SNAPSHOT_TS) AS REVIEW_TARGET,
      MAX_BY(SEVERITY, SNAPSHOT_TS) AS SEVERITY,
  ...
  action_rollup AS (
    SELECT
      ...
      MAX_BY(REVIEW_TARGET, COALESCE(UPDATED_AT, CREATED_AT)) AS REVIEW_TARGET,
      MAX_BY(SEVERITY, COALESCE(UPDATED_AT, CREATED_AT)) AS SEVERITY,
      0 AS EVIDENCE_ROWS,
      COUNT_IF(UPPER(SEVERITY) IN ('CRITICAL', 'HIGH')) AS HIGH_RISK_CHANGES,
      0 AS ROUTE_BLOCKED,
...
BEGIN
  ...
  (WITH value_items AS (
    ...
    SELECT
      ...
      COALESCE(ENVIRONMENT, 'ALL') AS ENVIRONMENT,
      STATUS,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEW_PRIMARY, ''), NULLIF(REVIEW_TARGET, ''), 'DBA / Cost attribution') AS WORKFLOW_ROUTE,
...
BEGIN
  ...
  action_agg AS (
    SELECT
      ...
      MAX(COALESCE(WORKFLOW_ROUTE, REVIEW_PRIMARY, REVIEW_TARGET, 'DBA Review')) AS WORKFLOW_ROUTE,
...
BEGIN
  ...
  (WITH action_queue AS (
    SELECT
      ...
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEW_GROUP, ''), NULLIF(REVIEW_TARGET, ''), 'Route gap') AS WORKFLOW_ROUTE,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEW_GROUP, ''), NULLIF(REVIEW_TARGET, '')) IS NULL AS WORKFLOW_GAP,
...
BEGIN
  ...
  action_rollup AS (
    SELECT
      ...
      MAX_BY(
        COALESCE(NULLIF(q.WORKFLOW_ROUTE, ''), NULLIF(q.REVIEW_PRIMARY, ''), NULLIF(q.REVIEW_TARGET, ''), NULLIF(q.REVIEW_GROUP, '')),
