...
USING (
  SELECT * FROM VALUES
    ('WAREHOUSE', '*', 'ALL', 'ALL', 'Cost & Contract', 'default-workflow', TRUE),
    ('TASK', '*', 'ALL', 'ALL', 'Workload Operations', 'default-workflow', TRUE),
    ('PROCEDURE', '*', 'ALL', 'ALL', 'Workload Operations', 'default-workflow', TRUE),
    ('USER', '*', 'ALL', 'ALL', 'Security Monitoring', 'default-workflow', TRUE),
    ('ROLE', '*', 'ALL', 'ALL', 'Security Monitoring', 'default-workflow', TRUE),
    ('DATABASE', '*', 'ALL', 'ALL', 'Data Trust', 'default-workflow', TRUE),
    ('SCHEMA', '*', 'ALL', 'ALL', 'Data Trust', 'default-workflow', TRUE),
    ('ALERT', '*', 'ALL', 'ALL', 'Alert Center', 'default-workflow', TRUE),
    ('ACTION', '*', 'ALL', 'ALL', 'Action Queue', 'default-workflow', TRUE),
    ('CORTEX', '*', 'ALL', 'ALL', 'Cost & Contract', 'default-workflow', TRUE)
  AS t(ENTITY_TYPE, ENTITY_PATTERN, COMPANY, ENVIRONMENT, WORKFLOW_ROUTE, SOURCE, ACTIVE)
) src
...
WHEN MATCHED THEN UPDATE SET
  ...
  UPDATED_AT = CURRENT_TIMESTAMP()
WHEN NOT MATCHED THEN INSERT (
  ENTITY_TYPE, ENTITY_PATTERN, COMPANY, ENVIRONMENT, WORKFLOW_ROUTE, SOURCE, ACTIVE
)
VALUES (
  src.ENTITY_TYPE, src.ENTITY_PATTERN, src.COMPANY, src.ENVIRONMENT, src.WORKFLOW_ROUTE,
  src.SOURCE, src.ACTIVE
);
...
USING (
  SELECT * FROM VALUES
    ...
    ('WH_ALFA_OVERWATCH_USAGE', 'WH_ALFA_OVERWATCH', 'WAREHOUSE', 'USAGE', 'Scheduled mart refresh and Streamlit runtime queries', TRUE, 'SHOW GRANTS ON WAREHOUSE WH_ALFA_OVERWATCH;', 'Security / DBA', 'Grant USAGE on WH_ALFA_OVERWATCH to the runtime roles and keep AUTO_SUSPEND controlled.', TRUE),
...
-- -----------------------------------------------------------------------------
-- COCO section summary marts folded from retired 09_summary_marts.sql.
-- These are active compact marts and secure app views, so they live in the
-- approved mart table deployment file rather than a ninth setup step.
-- -----------------------------------------------------------------------------
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_EXECUTIVE_SUMMARY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  WAREHOUSE_NAME                 VARCHAR(200),
  EXECUTIVE_NARRATIVE            VARCHAR(4000),
  ...
  YEAR_ELAPSED_PCT               FLOAT,
  PROJECTED_PERIOD_END_CREDITS   FLOAT,
  PROJECTED_OVER_UNDER_USD       FLOAT,
  FORECAST_BUDGET_VARIANCE_USD   FLOAT,
  CRITICAL_HIGH_ISSUES           NUMBER,
  OPEN_WORK_ITEMS                NUMBER,
  TOP_COST_DRIVER                VARCHAR(500),
  OVERWATCH_SELF_COST_USD        FLOAT,
  BUDGET_SOURCE                  VARCHAR(500)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_DBA_MORNING_COCKPIT (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  QUEUED_WAREHOUSES              NUMBER,
  FAILED_TASKS_LAST_HOUR         NUMBER,
  ACTIVE_CRITICAL_HIGH_ALERTS    NUMBER,
  FOUR_HOUR_BURN_CREDITS         FLOAT,
  BASELINE_FOUR_HOUR_BURN        FLOAT,
  LIVE_STATE_LABEL               VARCHAR(200),
  ACCOUNT_USAGE_LATENCY_NOTE     VARCHAR(1000)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_SOURCE_FRESHNESS (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  WAREHOUSE_NAME                 VARCHAR(200),
  SOURCE_NAME                    VARCHAR(300),
  LATEST_SOURCE_TS               TIMESTAMP_TZ,
  FRESHNESS_MINUTES              FLOAT,
  SOURCE_LATENCY_NOTE            VARCHAR(1000),
  STATUS                         VARCHAR(40)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_ALERT_INTELLIGENCE (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  SOURCE_FRESHNESS               VARCHAR(1000),
  SUGGESTED_ACTION               VARCHAR(4000),
  TICKET_ID                      VARCHAR(200),
  CLOSURE_STATUS                 VARCHAR(200),
  RELATED_WORK_ITEM              VARCHAR(500),
  DELIVERY_STATUS                VARCHAR(100),
  ACK_AGE_MINUTES                NUMBER,
  RESOLVE_AGE_MINUTES            NUMBER,
  IS_OVERDUE                     BOOLEAN,
  SOURCE_OBJECT                  VARCHAR(300)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_TASK_STATUS_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  ERROR_CODE_FREQUENCY           NUMBER,
  TOP_ERROR_CODE                 VARCHAR(100),
  FAILED_QUERY_TREND             FLOAT,
  P95_QUERY_DURATION             FLOAT,
  REMOTE_SPILL_GB                FLOAT,
  QUERIES_WAITING                NUMBER,
  BLOCKED_TIME                   FLOAT,
  TASK_SUCCESS_RATE              FLOAT,
  SLA_ATTAINMENT                 FLOAT
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_WAREHOUSE_DAILY_CREDITS (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  AVG_BLOCKED                    FLOAT,
  UTILIZATION                    FLOAT,
  MULTI_CLUSTER_PRESSURE         FLOAT
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_COST_FORECAST (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  FORECAST_CREDITS               FLOAT,
  BUDGET_CREDITS                 FLOAT,
  LOWER_BOUND_CREDITS            FLOAT,
  UPPER_BOUND_CREDITS            FLOAT,
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_CONTRACT_BURN_DOWN (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  COMMITTED_CREDITS              FLOAT,
  CONSUMED_CREDITS               FLOAT,
  ANNUAL_COMMIT_BURN_PCT         FLOAT,
  YEAR_ELAPSED_PCT               FLOAT,
  PROJECTED_PERIOD_END_CREDITS   FLOAT,
  PROJECTED_OVER_UNDER_CREDITS   FLOAT,
  PROJECTED_OVER_UNDER_USD       FLOAT,
  BUDGET_SOURCE                  VARCHAR(500)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_LOGIN_SECURITY_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  THRESHOLD_TEXT                 VARCHAR(1000),
  EVIDENCE_COUNT                 NUMBER,
  CONFIDENCE                     VARCHAR(100),
  SOURCE_LATENCY                 VARCHAR(1000)
);

CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_QUERY_ERROR_SUMMARY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_STORAGE_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_CORTEX_CODE_USAGE_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_COST_ALLOCATION_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
...
CREATE TRANSIENT TABLE IF NOT EXISTS MART_V2_OVERWATCH_APP_SELF_COST_DAILY (
  SNAPSHOT_TS                    TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  COMPANY                        VARCHAR(100),
  ENVIRONMENT                    VARCHAR(100),
  WINDOW_DAYS                    NUMBER,
  ...
  USAGE_DATE                     DATE,
  TASK_CREDITS                   FLOAT,
  APP_QUERY_CREDITS              FLOAT,
  REFRESH_WAREHOUSE_CREDITS      FLOAT,
  EST_MONTHLY_COST_USD           FLOAT,
  PCT_OF_MONITORED_SPEND         FLOAT
);

CREATE OR REPLACE SECURE VIEW V_EXECUTIVE_SUMMARY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  EXECUTIVE_NARRATIVE, PLATFORM_SCORE, RELIABILITY_SCORE, COST_SCORE,
  SECURITY_SCORE, FRESHNESS_SCORE, ACTION_SCORE, COMMITTED_CREDITS,
  CONSUMED_CREDITS, YEAR_ELAPSED_PCT, PROJECTED_PERIOD_END_CREDITS,
  PROJECTED_OVER_UNDER_USD, FORECAST_BUDGET_VARIANCE_USD,
  CRITICAL_HIGH_ISSUES, OPEN_WORK_ITEMS, TOP_COST_DRIVER,
  OVERWATCH_SELF_COST_USD, BUDGET_SOURCE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_EXECUTIVE_SUMMARY;

CREATE OR REPLACE SECURE VIEW V_DBA_MORNING_COCKPIT AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  RUNNING_QUERIES, BLOCKED_SESSIONS, QUEUED_WAREHOUSES,
  FAILED_TASKS_LAST_HOUR, ACTIVE_CRITICAL_HIGH_ALERTS,
  FOUR_HOUR_BURN_CREDITS, BASELINE_FOUR_HOUR_BURN,
  LIVE_STATE_LABEL, ACCOUNT_USAGE_LATENCY_NOTE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_DBA_MORNING_COCKPIT;

CREATE OR REPLACE SECURE VIEW V_SOURCE_FRESHNESS AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  SOURCE_NAME, LATEST_SOURCE_TS, FRESHNESS_MINUTES,
  SOURCE_LATENCY_NOTE, STATUS,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_SOURCE_FRESHNESS;

CREATE OR REPLACE SECURE VIEW V_ALERT_INTELLIGENCE AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  ALERT_ID, SEVERITY, STATUS, ENTITY, MESSAGE, SOURCE_FRESHNESS,
  SUGGESTED_ACTION, TICKET_ID, CLOSURE_STATUS, RELATED_WORK_ITEM,
  DELIVERY_STATUS, ACK_AGE_MINUTES, RESOLVE_AGE_MINUTES, IS_OVERDUE,
  SOURCE_OBJECT,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_ALERT_INTELLIGENCE;

CREATE OR REPLACE SECURE VIEW V_TASK_STATUS_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  QUERY_FAILURE_RATE, ERROR_CODE_FREQUENCY, TOP_ERROR_CODE,
  FAILED_QUERY_TREND, P95_QUERY_DURATION, REMOTE_SPILL_GB,
  QUERIES_WAITING, BLOCKED_TIME, TASK_SUCCESS_RATE, SLA_ATTAINMENT,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_TASK_STATUS_DAILY;

CREATE OR REPLACE SECURE VIEW V_WAREHOUSE_DAILY_CREDITS AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  USAGE_DATE, CREDITS_USED, COST_USD, AVG_RUNNING, AVG_QUEUED,
  AVG_BLOCKED, UTILIZATION, MULTI_CLUSTER_PRESSURE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_WAREHOUSE_DAILY_CREDITS;

CREATE OR REPLACE SECURE VIEW V_COST_FORECAST AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  DAY, ACTUAL_CREDITS, FORECAST_CREDITS, BUDGET_CREDITS,
  LOWER_BOUND_CREDITS, UPPER_BOUND_CREDITS, METHOD_LABEL,
  CONFIDENCE_LABEL, SOURCE_FRESHNESS, BUDGET_SOURCE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_COST_FORECAST;

CREATE OR REPLACE SECURE VIEW V_CONTRACT_BURN_DOWN AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  COMMITTED_CREDITS, CONSUMED_CREDITS, ANNUAL_COMMIT_BURN_PCT,
  YEAR_ELAPSED_PCT, PROJECTED_PERIOD_END_CREDITS,
  PROJECTED_OVER_UNDER_CREDITS, PROJECTED_OVER_UNDER_USD, BUDGET_SOURCE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_CONTRACT_BURN_DOWN;

CREATE OR REPLACE SECURE VIEW V_LOGIN_SECURITY_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  RULE_NAME, THRESHOLD_TEXT, EVIDENCE_COUNT, CONFIDENCE, SOURCE_LATENCY,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_LOGIN_SECURITY_DAILY;

CREATE OR REPLACE SECURE VIEW V_QUERY_ERROR_SUMMARY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  ERROR_CODE, ERROR_COUNT, QUERY_FAILURE_RATE, TOP_ERROR_CODE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_QUERY_ERROR_SUMMARY;

CREATE OR REPLACE SECURE VIEW V_STORAGE_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  USAGE_DATE, STORAGE_BYTES, FAILSAFE_BYTES, COST_USD,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_STORAGE_DAILY;

CREATE OR REPLACE SECURE VIEW V_CORTEX_CODE_USAGE_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  USAGE_DATE, REQUEST_COUNT, TOKEN_COUNT, CREDITS_USED, COST_USD,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_CORTEX_CODE_USAGE_DAILY;

CREATE OR REPLACE SECURE VIEW V_COST_ALLOCATION_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  USAGE_DATE, BUSINESS_UNIT, MATCH_TYPE, MATCH_PATTERN,
  ALLOCATED_CREDITS, COST_USD, UNALLOCATED_CREDITS,
  ALLOCATION_CONFIDENCE,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_COST_ALLOCATION_DAILY;

CREATE OR REPLACE SECURE VIEW V_OVERWATCH_APP_SELF_COST_DAILY AS
SELECT
  SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
  USAGE_DATE, TASK_CREDITS, APP_QUERY_CREDITS, REFRESH_WAREHOUSE_CREDITS,
  EST_MONTHLY_COST_USD, PCT_OF_MONITORED_SPEND,
  DATEDIFF('MINUTE', SNAPSHOT_TS, CURRENT_TIMESTAMP()) AS AGE_MINUTES
FROM MART_V2_OVERWATCH_APP_SELF_COST_DAILY;
...
GRANT SELECT ON VIEW V_EXECUTIVE_SUMMARY TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_EXECUTIVE_SUMMARY TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_DBA_MORNING_COCKPIT TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_DBA_MORNING_COCKPIT TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_SOURCE_FRESHNESS TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_SOURCE_FRESHNESS TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_ALERT_INTELLIGENCE TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_ALERT_INTELLIGENCE TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_COST_FORECAST TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_COST_FORECAST TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_CONTRACT_BURN_DOWN TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_CONTRACT_BURN_DOWN TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_COST_ALLOCATION_DAILY TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_COST_ALLOCATION_DAILY TO ROLE SNOW_SYSADMINS;
GRANT SELECT ON VIEW V_OVERWATCH_APP_SELF_COST_DAILY TO ROLE SNOW_ACCOUNTADMINS;
GRANT SELECT ON VIEW V_OVERWATCH_APP_SELF_COST_DAILY TO ROLE SNOW_SYSADMINS;
