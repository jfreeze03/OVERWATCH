...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      ...
      FIRST_NAME::VARCHAR,
      LAST_NAME::VARCHAR,
      COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME)::VARCHAR AS USER_DISPLAY_NAME,
      COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NAME)::VARCHAR AS USER_CHART_LABEL,
      IFF(
        NAME IS NOT NULL
        AND NAME <> COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME),
        COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME) || ' (' || NAME || ')',
        COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME)
...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      ...
      COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, sr.USER_NAME) AS USER_DISPLAY_NAME,
      COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), u.NAME, sr.USER_NAME) AS USER_CHART_LABEL,
      IFF(
        u.NAME IS NOT NULL
        AND u.NAME <> COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, sr.USER_NAME),
        COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, sr.USER_NAME) || ' (' || u.NAME || ')',
        COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, sr.USER_NAME)
      ) AS USER_ADMIN_LABEL,
      u.EMAIL AS USER_EMAIL,
      sr.TYPE,
      sr.DOMAIN,
...
BEGIN
  ...
  WITH candidates AS (
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      IFF(SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0, 'High', 'Watch'),
  ...
  FROM (
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'credential_expirations', 'Credential expirations',
           CASE
             WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRED_COUNT) || ' expired · ' || TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRING_7D_COUNT) || ' due within 7d'
           ...
           'integer',
           'credentials',
           IFF(SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER IS NULL, 'No credential expiration exposure in the selected window', 'Next: ' || SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER || ' · ' || COALESCE(SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE, 'credential')),
