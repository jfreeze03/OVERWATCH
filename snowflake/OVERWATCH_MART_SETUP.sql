...
BEGIN
  ...
  SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, METRIC_KEY, METRIC_LABEL,
         ...
         CASE
           WHEN METRIC_KEY LIKE 'cortex%' THEN 'cortex_daily'
           WHEN METRIC_KEY LIKE 'cost%' OR METRIC_KEY IN ('total_spend','spend_movement_pct','forecast_run_rate','top_cost_driver') THEN 'cost_daily'
           WHEN METRIC_KEY LIKE 'alert%' OR METRIC_KEY IN ('critical_high','active_alerts','overdue_alerts') THEN 'alert_events'
           WHEN METRIC_KEY LIKE 'failed%' OR METRIC_KEY LIKE 'queue%' OR METRIC_KEY LIKE 'pipeline%' THEN 'query_hourly'
           WHEN METRIC_KEY LIKE 'security%' OR METRIC_KEY LIKE 'risky%' OR METRIC_KEY LIKE 'mfa%' THEN 'security_operability'
           ELSE LOWER(REGEXP_REPLACE(SECTION_NAME, '[^A-Za-z0-9]+', '_'))
         END AS SOURCE_KEY,
         CONFIDENCE,
...
FROM (
  ...
  FROM VALUES
    ...
    ('APP_VERSION'), ('PERSISTENCE_STATUS'), ('LOAD_TS')
) required_columns
