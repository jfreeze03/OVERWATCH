...
# Evidence CTA rendered by the command-center view; keep visible to UI-kit proof.
EXECUTIVE_EVIDENCE_CTA_LABEL = "Load Full Executive Snapshot"
...
def _close_first_paint_for_command_center_action() -> None:
    """Keep explicit command-center button work out of first-paint accounting."""
...
def render() -> None:
    with with_section_first_paint_entry("Executive Landing", "Entry"):
        ...
        if active_workflow == EXECUTIVE_OVERVIEW_WORKFLOW:
            ...
            has_loaded_snapshot = isinstance(snapshot, dict) and _snapshot_matches_scope(
                snapshot,
                company,
                environment,
                int(days),
            ...
            if refresh_requested:
                ...
                with query_budget_context(
                    ...
                    st.rerun()
            snapshot_requested = snapshot_requested or bool(
                st.session_state.pop("_executive_landing_command_brief_load_detail", False)
                )
