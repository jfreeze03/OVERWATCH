...
def _render_workflow_launchpad() -> None:
    st.markdown("**Architecture Investigation Workflows**")
    ...
    for col, row in zip(cols, visible):
        with col:
            ...
            st.caption(row["MOVE"])
            if st.button(row["BUTTON_LABEL"], key=f"architecture_shell_{row['PANE']}", width="stretch"):
                _open_workspace(str(row["PANE"]))

    show_all = bool(st.session_state.get("architecture_shell_show_all"))
    if not show_all and st.button("More Architecture Workflows", key="architecture_shell_more"):
        st.session_state["architecture_shell_show_all"] = True
        st.rerun()
    ...
    if show_all:
        ...
        for col, row in zip(extra_cols, _WORKFLOWS[3:]):
            with col:
                st.markdown(f"**{row['PANE']}**")
                st.caption(row["MOVE"])
                if st.button(row["BUTTON_LABEL"], key=f"architecture_shell_extra_{row['PANE']}", width="stretch"):
                    _open_workspace(str(row["PANE"]))
        if st.button("Hide Architecture Workflows", key="architecture_shell_hide"):
            st.session_state["architecture_shell_show_all"] = False
            st.rerun()
