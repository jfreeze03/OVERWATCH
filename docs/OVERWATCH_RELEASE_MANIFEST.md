...
- 12 Power User Performance: FAIL, `PERF_12_POWER_USERS_RELEASE_RERUN3`, p95 `15388.74 ms`, p99 `37835.50 ms`, errors `0`, skipped buttons `0`, readiness `64/100`; expert review `perf_tests/results/PERF_12_POWER_USERS_RELEASE_RERUN3_expert_review.md`
...
- Deferred items: Reason: new live Snowflake regression was not rerun for `24cd05e`; rerun only when a fresh credentialed release gate is required. Reason: the 12-heavy-power-user benchmark still fails for `24cd05e` because RERUN3 p95 `15388.74 ms` exceeds the `10000 ms` release threshold and readiness is `64/100`; skipped buttons remained `0`. Diagnostics point to App Shell/browser first response and paint under concurrency: `responseStart` p95 `9169.90 ms`, `domContentLoadedEventEnd` p95 `11692.50 ms`, and `first-contentful-paint` p95 `37408.00 ms`, while server `shell:total_render_app` p95 was `212.41 ms`.
