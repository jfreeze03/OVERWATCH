...
USING (
  SELECT * FROM VALUES
    ...
    ('ALFA_WAREHOUSES', '["BLCOMPUTE_WH","COMPUTE_WH","CROWDSTRIKE_WH","DOC_ALWH","POSIT_WORKBENCH","SNOWFLAKE_LEARNING_WH","SYSTEM$STREAMLIT_NOTEBOOK_WH","WH_ALFA_ANALYTICS","WH_ALFA_LOAD","WH_ALFA_QA","WH_ALFA_QUERY","WH_ALFA_TRANSFORM","WH_ALFA_UNLOAD"]', 'JSON', 'Governed ALFA warehouse list used by scope controls and summary loaders.'),
    ...
    ('ALL_WAREHOUSES', '["BLCOMPUTE_WH","COMPUTE_WH","CROWDSTRIKE_WH","DOC_ALWH","POSIT_WORKBENCH","SNOWFLAKE_LEARNING_WH","SYSTEM$STREAMLIT_NOTEBOOK_WH","WH_ALFA_ANALYTICS","WH_ALFA_LOAD","WH_ALFA_QA","WH_ALFA_QUERY","WH_ALFA_TRANSFORM","WH_ALFA_UNLOAD","WH_TRXS_LOAD","WH_TRXS_QUERY","WH_TRXS_TRANSFORM","WH_TRXS_UNLOAD"]', 'JSON', 'Governed combined warehouse list. Keep aligned with company-specific warehouse settings.'),
    ...
    ('REFRESH_WAREHOUSE', 'COMPUTE_WH', 'STRING', 'Warehouse used by summary mart refresh tasks until a dedicated OVERWATCH refresh warehouse is approved.'),
...
BEGIN
  ...
  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_ALERTS (
    ...
    OWNER, SLA, EVENT_TS, ROUTE_KEY, EVIDENCE_ID, UPDATED_AT
  )
  SELECT
    COMPANY, ENVIRONMENT, SEVERITY, 'Open', SIGNAL, DETAIL, ENTITY_NAME,
    COALESCE(OWNER_NAME, OWNER_ROUTE), SLA_STATE, COALESCE(FIRST_SEEN_TS, SNAPSHOT_TS),
...
BEGIN
  ...
  facts AS (
    SELECT
      ...
      GREATEST_IGNORE_NULLS(
        ...
        login.LOAD_TS, grants.LOAD_TS, cred.LOAD_TS, owner.LOAD_TS, val.LOAD_TS, prod.LOAD_TS, app.LOAD_TS, fc.LOAD_TS,
  ...
  security_decision AS (
    SELECT f.*,
      ...
      'Security view of failed logins, credential expirations, risky grants, privilege changes, sharing exposure, security alerts, and owner coverage.' AS DECISION_SUMMARY,
      ...
      CASE
        ...
        ELSE 'Due ' || TO_VARCHAR(ACTION_DUE_DATE)
      END AS DECISION_SLA_STATE,
      'FACT_SECURITY_OPERABILITY_DAILY; MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT; ALERT_EVENTS; MART_OPERATIONAL_OWNER_COVERAGE' AS DECISION_EVIDENCE_SOURCE
  ...
  SELECT metric_candidates.BRIEF_ID, metric_candidates.SECTION_NAME, metric_candidates.COMPANY,
         ...
         CASE
           WHEN metric_candidates.METRIC_KEY IN ('platform_health','production_readiness','data_trust','verified_value','owner_coverage') THEN 'lower_is_worse'
         ...
         CASE
           ...
           WHEN metric_candidates.METRIC_KEY IN ('owner_coverage','unassigned_findings') THEN 'owner_coverage'
...
BEGIN
  ...
  SELECT
    ...
    LEFT('Credential expiration for ' || COALESCE(c.USER_DISPLAY_NAME, c.USER_NAME, 'credential owner'), 2000),
    OBJECT_CONSTRUCT_KEEP_NULL(
      'credential', c.CREDENTIAL_NAME,
      'type', c.TYPE,
      'domain', c.DOMAIN,
...
UNION ALL
SELECT 'MART_USER_DIM_CURRENT', COUNT(*) FROM MART_USER_DIM_CURRENT
UNION ALL
SELECT 'MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT', COUNT(*) FROM MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT
UNION ALL
...
UNION ALL
SELECT 'MART_EXECUTIVE_COMMAND_CENTER_KPI', COUNT(*) FROM MART_EXECUTIVE_COMMAND_CENTER_KPI
UNION ALL
SELECT 'MART_EXECUTIVE_COMMAND_CENTER_TIMESERIES', COUNT(*) FROM MART_EXECUTIVE_COMMAND_CENTER_TIMESERIES
UNION ALL
SELECT 'MART_EXECUTIVE_COMMAND_CENTER_WAREHOUSE', COUNT(*) FROM MART_EXECUTIVE_COMMAND_CENTER_WAREHOUSE
UNION ALL
SELECT 'MART_EXECUTIVE_COMMAND_CENTER_ALERTS', COUNT(*) FROM MART_EXECUTIVE_COMMAND_CENTER_ALERTS
UNION ALL
SELECT 'MART_EXECUTIVE_COMMAND_CENTER_CONTEXT', COUNT(*) FROM MART_EXECUTIVE_COMMAND_CENTER_CONTEXT;


SELECT
  'USER_DISPLAY_DIMENSION_COLUMNS' AS CHECK_NAME,
...
LEFT JOIN INFORMATION_SCHEMA.COLUMNS C
 ...
 AND UPPER(C.TABLE_NAME) = 'MART_USER_DIM_CURRENT'
