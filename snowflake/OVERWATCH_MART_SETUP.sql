...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      ...
      ROUND(SUM(raw.CREDITS_USED) * :credit_price, 2) AS EST_COST_USD,
...
BEGIN
  ...
  cost_rollup AS (
    SELECT
      s.COMPANY,
      s.WINDOW_DAYS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_BILLED, COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0), 0) * cfg.CREDIT_PRICE_USD, 0)) AS COST_USD,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -2 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()), COALESCE(c.CREDITS_BILLED, COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0), 0) * cfg.CREDIT_PRICE_USD, 0)) AS PRIOR_COST_USD,
      ...
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0), 0)) AS ACCOUNT_USED_CREDITS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_USED_COMPUTE, 0), 0)) AS COMPUTE_CREDITS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0), 0)) AS CLOUD_SERVICES_CREDITS,
      ...
      SUM(IFF(
        c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE())
        AND c.USAGE_DATE < CURRENT_DATE()
        AND REGEXP_LIKE(UPPER(COALESCE(c.SERVICE_CATEGORY, '') || ' ' || COALESCE(c.SERVICE_TYPE, '') || ' ' || COALESCE(c.SOURCE_VIEW, '')), 'WAREHOUSE|COMPUTE')
        AND COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0) > 0,
        COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0),
        0
      ...
      SUM(IFF(
        ...
        AND REGEXP_LIKE(UPPER(COALESCE(c.SERVICE_CATEGORY, '') || ' ' || COALESCE(c.SERVICE_TYPE, '') || ' ' || COALESCE(c.SOURCE_VIEW, '')), 'WAREHOUSE|COMPUTE')
        AND COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0) > 0,
        (COALESCE(c.CREDITS_USED_COMPUTE, 0) + COALESCE(c.CREDITS_USED_CLOUD_SERVICES, 0)) * cfg.CREDIT_PRICE_USD,
        0
      )) AS WAREHOUSE_COST_USD,
    ...
    FROM (
      SELECT
        ...
        ROW_NUMBER() OVER (
          ...
          ORDER BY SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED_COMPUTE, 0) * cfg.CREDIT_PRICE_USD) DESC
        ) AS RN
      FROM scopes s
      CROSS JOIN settings cfg
  ...
  cortex_rollup AS (
    SELECT
      ...
      SUM(COALESCE(cx.CREDITS_USED, 0) * cfg.CREDIT_PRICE_USD) AS CORTEX_COST_USD,
  ...
  facts AS (
    SELECT
      ...
      IFF(c.LOAD_TS IS NULL, NULL, c.COMPUTE_CREDITS) AS COMPUTE_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, c.CLOUD_SERVICES_CREDITS) AS CLOUD_SERVICES_CREDITS,
      ...
      IFF(c.LOAD_TS IS NULL, NULL, c.WAREHOUSE_CREDITS) AS WAREHOUSE_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, c.WAREHOUSE_COST_USD) AS WAREHOUSE_COST_ESTIMATE_USD,
      IFF(c.LOAD_TS IS NULL, NULL, c.WAREHOUSE_COST_USD) AS WAREHOUSE_COST_USD,
      IFF(c.LOAD_TS IS NULL, NULL, GREATEST(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(c.WAREHOUSE_CREDITS, 0), 0)) AS SERVICE_OTHER_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, GREATEST(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(c.WAREHOUSE_CREDITS, 0), 0) * COALESCE(cfg.CREDIT_PRICE_USD, 0)) AS SERVICE_OTHER_COST_USD,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(c.WAREHOUSE_CREDITS, 0)) AS BILLING_BRIDGE_DELTA_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, (COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(c.WAREHOUSE_CREDITS, 0)) * COALESCE(cfg.CREDIT_PRICE_USD, 0)) AS BILLING_BRIDGE_DELTA_USD,
      CASE
        WHEN c.LOAD_TS IS NULL THEN 'pending'
        WHEN ABS(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(c.WAREHOUSE_CREDITS, 0)) < 0.000001 THEN 'matched'
        WHEN COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) > COALESCE(c.WAREHOUSE_CREDITS, 0) THEN 'warehouse_lower_than_billed'
  ...
  trend_aggregates AS (
    SELECT
      ...
      SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED_COMPUTE, 0) * s.CREDIT_PRICE_USD) AS TREND_VALUE
    FROM trend_scopes s
    JOIN FACT_COST_DAILY c
      ON (s.COMPANY = 'ALL' OR c.COMPANY = s.COMPANY)
    ...
    SELECT
      ...
      'cortex_spend',
      cx.USAGE_DATE,
      SUM(COALESCE(cx.CREDITS_USED, 0) * s.CREDIT_PRICE_USD)
    FROM trend_scopes s
...
BEGIN
  ...
  cost_driver_ranked AS (
    SELECT
      ...
      SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED_COMPUTE, 0) * :credit_price) AS COST_USD,
      ROW_NUMBER() OVER (
        PARTITION BY s.COMPANY, s.WINDOW_DAYS
        ORDER BY
          SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED_COMPUTE, 0) * :credit_price) DESC,
  ...
  cortex_rollup AS (
    SELECT
      ...
      SUM(COALESCE(cx.CREDITS_USED, 0) * :credit_price) AS COST_USD,
      MAX(cx.LOAD_TS) AS LOAD_TS
    FROM scopes s
    LEFT JOIN FACT_CORTEX_DAILY cx
      ON (s.COMPANY = 'ALL' OR cx.COMPANY = s.COMPANY)
