...
BEGIN
  ...
  WITH evidence_rollup AS (
    SELECT
      ...
      COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Route Blocked') AS ROUTE_BLOCKED,
      COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Closure Blocked') AS CLOSURE_BLOCKED,
      COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Review Ready') AS REVIEW_READY,
      ...
      CASE
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Route Blocked') > 0 THEN 'Route Blocked'
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Closure Blocked') > 0 THEN 'Closure Blocked'
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Review Ready') > 0 THEN 'Review Ready'
        ELSE 'Review Required'
      END AS CONTROL_STATE,
      CASE
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Route Blocked') > 0 THEN 0
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Closure Blocked') > 0 THEN 1
        WHEN COUNT_IF(CHANGE_EVIDENCE_READINESS = 'Review Ready') > 0 THEN 8
        ELSE 5
      END AS CONTROL_RANK,
      MAX_BY(NEXT_CONTROL_ACTION, SNAPSHOT_TS) AS NEXT_CONTROL_ACTION,
      MAX(SNAPSHOT_TS) AS LAST_ACTIVITY_TS
  ...
  action_rollup AS (
    SELECT
      ...
      COALESCE(ENVIRONMENT, 'No Database Context') AS ENVIRONMENT,
      'Action Queue' AS CONTROL_SOURCE,
      COALESCE(ENTITY_NAME, CATEGORY, 'Change Control') AS CONTROL_KEY,
