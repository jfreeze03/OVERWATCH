...
These seven numbered files are the active deployment path for the OVERWATCH
mart. [`../OVERWATCH_MART_SETUP.sql`](../OVERWATCH_MART_SETUP.sql) is generated
from these files and remains **byte-for-byte** equivalent to their ordered
concatenation (enforced by `tests/test_mart_setup_split.py`).

Post-deploy smoke checks live in
[`../validation/validate_overwatch_mart_setup.sql`](../validation/validate_overwatch_mart_setup.sql)
so validation SQL does not look like runtime setup DDL.
...
| Order | File | Contents |
|-------|------|----------|
| 1 | `01_runtime_objects.sql` | Database, schema, current `WH_ALFA_OVERWATCH` warehouse, and resource monitor. Establishes the session `USE DATABASE/SCHEMA` context. |
...
> populate them. (Roles intentionally come *after* the
...
snowsql -c <connection> -f ../validation/validate_overwatch_mart_setup.sql
```
...
Options A and C deploy the same active setup. Run the validation file afterward
when you want operator smoke checks.
