...
SELECT
  ...
  COUNT_IF(REQUIRED_SOURCE_COUNT IS NULL OR SOURCE_COVERAGE_PCT IS NULL OR DATA_AVAILABILITY_STATE IS NULL) AS ISSUE_COUNT,
  IFF(COUNT_IF(REQUIRED_SOURCE_COUNT IS NULL OR SOURCE_COVERAGE_PCT IS NULL OR DATA_AVAILABILITY_STATE IS NULL) = 0, 'PASS', 'FAIL') AS VALIDATION_STATUS
FROM MART_SECTION_COMMAND_BRIEF
WHERE SNAPSHOT_TS = (SELECT MAX(SNAPSHOT_TS) FROM MART_SECTION_COMMAND_BRIEF);
