...
- 12 Power User Performance: FAIL, `PERF_12_POWER_USERS_RELEASE_RERUN`, p95 `10712.96 ms`, p99 `19204.93 ms`, errors `0`, skipped buttons `0`, readiness `80/100`; expert review `perf_tests/results/PERF_12_POWER_USERS_RELEASE_RERUN_expert_review.md`
...
- Deferred items: Reason: new live Snowflake regression was not rerun for `24cd05e`; rerun only when a fresh credentialed release gate is required. Reason: the 12-heavy-power-user benchmark still fails for `24cd05e` because rerun p95 `10712.96 ms` exceeds the `10000 ms` release threshold and readiness is `80/100`; tune App Shell initial-load tail latency and DBA Control Room first-iteration navigation before high-traffic rollout.
