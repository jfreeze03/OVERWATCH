...
USING (
  SELECT * FROM VALUES
    ...
    ('OVERWATCH_ADMIN_ROLES', '["ACCOUNTADMIN","SYSADMIN","OVERWATCH_ADMIN","SNOW_ACCOUNTADMINS","SNOW_SYSADMINS"]', 'JSON', 'Governed v2 admin roles read from CURRENT_ROLE. The app must not use a Settings checkbox as an RBAC gate.'),
    ('V2_ADMIN_ROLES', '["SNOW_ACCOUNTADMINS","SNOW_SYSADMINS"]', 'JSON', 'Governed v2 admin roles. The app must not use a Settings checkbox as an RBAC gate.'),
    ('V2_LIVE_MODE_ROLES', '["SNOW_ACCOUNTADMINS","SNOW_SYSADMINS"]', 'JSON', 'Governed v2 roles allowed to open live mode and guarded query cancellation.'),
    ('SECURITY_RETENTION_DAYS', '365', 'NUMBER', 'Minimum retention for security monitoring marts.'),
    ('LOGIN_RETENTION_DAYS', '365', 'NUMBER', 'Minimum retention for login security facts and v2 security marts.'),
    ('GRANT_RETENTION_DAYS', '365', 'NUMBER', 'Minimum retention for grant/security governance facts.'),
    ('ANNUAL_COMMITTED_CREDITS', NULL, 'NUMBER', 'Governed annual committed credits for executive and cost burn-down views. NULL means setup required.'),
    ('CONTRACT_PERIOD_START', NULL, 'DATE', 'Governed contract period start date. NULL means setup required.'),
    ('CONTRACT_PERIOD_END', NULL, 'DATE', 'Governed contract period end date. NULL means setup required.'),
    ('MONTHLY_BUDGET_USD', NULL, 'NUMBER', 'Governed monthly budget used for budget headroom. NULL means setup required.'),
    ('BUDGET_SOURCE_LABEL', 'OVERWATCH_SETTINGS', 'STRING', 'Budget/commit source label shown in v2 executive and cost views.'),
...
USING (
  SELECT
    ...
    'snowflake/OVERWATCH_MART_SETUP.sql' AS SOURCE_FILE,
    'Adds the Phase 1 enterprise operating model: Finding -> Route -> Trust Level -> Business Impact -> Action -> Value Verified. First-paint surfaces read compact marts; detail evidence remains explicit-load only.' AS NOTES
) src
ON tgt.MIGRATION_VERSION = src.MIGRATION_VERSION
WHEN MATCHED THEN UPDATE SET
  MIGRATION_NAME = src.MIGRATION_NAME,
...
CREATE TABLE IF NOT EXISTS OVERWATCH_ATTRIBUTION_TAG_NAMES (
...
MERGE INTO OVERWATCH_ATTRIBUTION_TAG_NAMES tgt
USING (
  SELECT * FROM VALUES
    ('COST_ATTRIBUTION',        'COST',        10, TRUE, 'Preferred chargeback route tag.'),
    ('DATA_ROUTE',        'DATA',        20, TRUE, 'Database/schema/table data reviewer fallback.'),
    ('APP_ROUTE',         'APPLICATION', 30, TRUE, 'Application route fallback.'),
    ('APPLICATION_ROUTE', 'APPLICATION', 35, TRUE, 'Alternate application route tag.'),
    ('BUSINESS_ROUTE',    'BUSINESS',    40, TRUE, 'Business route fallback.'),
    ('SERVICE_ROUTE',     'SERVICE',     50, TRUE, 'Service route fallback.')
) src(TAG_NAME, ATTRIBUTION_TYPE, PRIORITY, IS_ACTIVE, NOTES)
ON UPPER(tgt.TAG_NAME) = UPPER(src.TAG_NAME)
WHEN MATCHED THEN UPDATE SET
  ATTRIBUTION_TYPE = src.ATTRIBUTION_TYPE,
  PRIORITY = src.PRIORITY,
  IS_ACTIVE = src.IS_ACTIVE,
  NOTES = src.NOTES
WHEN NOT MATCHED THEN INSERT (TAG_NAME, ATTRIBUTION_TYPE, PRIORITY, IS_ACTIVE, NOTES)
VALUES (src.TAG_NAME, src.ATTRIBUTION_TYPE, src.PRIORITY, src.IS_ACTIVE, src.NOTES);
...
-- Upgrade older action queue installs to the current route/environment and
...
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_STATUS VARCHAR(40);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEWED_BY VARCHAR(200);
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEWED_AT TIMESTAMP_NTZ;
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS REVIEW_NOTE VARCHAR(2000);
...
ALTER TABLE IF EXISTS OVERWATCH_ACTION_QUEUE ADD COLUMN IF NOT EXISTS WORKFLOW_ROUTE VARCHAR(200);
...
CREATE TABLE IF NOT EXISTS OVERWATCH_CHANGE_CONTROL_EVIDENCE (
  ...
  REVIEW_STATUS_READY     VARCHAR(20),
...
ALTER TABLE IF EXISTS OVERWATCH_ALERTS ADD COLUMN IF NOT EXISTS WORKFLOW_ROUTE VARCHAR(200);
...
CREATE TABLE IF NOT EXISTS ALERT_WORKFLOW_ROUTING (
...
USING (
  SELECT * FROM VALUES
    ('DQ_ORDER_FRESHNESS', 'ALFA_EDW_PRD', 'CURATED', 'FACT_ORDER', 'LOAD_TS', 'FRESHNESS_SLA_HOURS', 24, '>', 'High', 'Data Route', 'DATA_QUALITY', FALSE),
    ('DQ_POLICY_NULL_RATE', 'ALFA_EDW_PRD', 'CURATED', 'DIM_POLICY', 'POLICY_ID', 'NULL_RATE_PCT', 0, '>', 'Critical', 'Data Route', 'DATA_QUALITY', FALSE),
    ('DQ_CLAIM_VOLUME_DROP', 'ALFA_EDW_PRD', 'CURATED', 'FACT_CLAIM', '*', 'ROW_COUNT_DROP_PCT', 35, '>', 'High', 'Data Route', 'DATA_QUALITY', FALSE)
) src(CHECK_KEY, DATABASE_NAME, SCHEMA_NAME, TABLE_NAME, COLUMN_NAME, CHECK_TYPE, THRESHOLD_VALUE, COMPARISON_OPERATOR, SEVERITY, WORKFLOW_ROUTE, NOTIFICATION_CHANNEL, ENABLED)
...
WHEN NOT MATCHED THEN INSERT
  (CHECK_KEY, DATABASE_NAME, SCHEMA_NAME, TABLE_NAME, COLUMN_NAME, CHECK_TYPE, THRESHOLD_VALUE, COMPARISON_OPERATOR, SEVERITY, WORKFLOW_ROUTE, NOTIFICATION_CHANNEL, ENABLED)
VALUES
  (src.CHECK_KEY, src.DATABASE_NAME, src.SCHEMA_NAME, src.TABLE_NAME, src.COLUMN_NAME, src.CHECK_TYPE, src.THRESHOLD_VALUE, src.COMPARISON_OPERATOR, src.SEVERITY, src.WORKFLOW_ROUTE, src.NOTIFICATION_CHANNEL, src.ENABLED);
...
USING (
  SELECT * FROM VALUES
    ...
    ('POLICY_TASK_RERUN_STATUS_REVIEW', 'PIPELINE_TASK_FAILURE', 'Task / Pipeline', 'Task rerun review', 'STATUS_REVIEW', FALSE, 'DBA / Pipeline Reviewer', 'SELECT * FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY()) ORDER BY SCHEDULED_TIME DESC LIMIT 100;', '-- Confirm root cause, dependency state, and downstream idempotency before EXECUTE TASK.', 'EXECUTE TASK IDENTIFIER(''<database.schema.task_name>'');', 'Record downstream cleanup plan or rerun blocker before manual execution.', 'SELECT * FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY()) ORDER BY SCHEDULED_TIME DESC LIMIT 100;'),
...
BEGIN
  ...
  SELECT
    ...
    'Dry-run only. Capture before-state, review route route, and verify expected impact before any manual action.' AS EXPECTED_EFFECT,
    CASE
...
USING (
  SELECT * FROM VALUES
    ...
    ('COST_WAREHOUSE_CREDIT_SPIKE', 'Cost', 'Warehouse credit spike', 'High', 1.5, 30, 1440, 'DBA / Cost attribution', 'COST'),
    ('COST_CORTEX_SPEND_SPIKE', 'Cost', 'Cortex spend spike', 'High', 25, 30, 10080, 'DBA / AI cost route', 'COST'),
    ('BEHAVIOR_USER_QUERY_ANOMALY', 'Behavior', 'User query behavior anomaly', 'High', 10, 14, 120, 'DBA / Workload reviewer', 'DBA_REVIEW'),
    ('PERF_QUEUE_PRESSURE', 'Performance', 'Warehouse queue pressure', 'High', 300, 14, 60, 'DBA / Platform', 'DBA_REVIEW'),
    ('PIPELINE_TASK_FAILURE', 'Task / Pipeline', 'Production task failure', 'Critical', 1, 7, 1440, 'DBA / Pipeline Reviewer', 'PIPELINE_REVIEW'),
    ('DQ_FRESHNESS_SLA', 'Data Quality', 'Freshness SLA missed', 'High', 1, 7, 1440, 'Data Route', 'DATA_QUALITY'),
    ('OPT_UNUSED_WAREHOUSE', 'Cost Recommendations', 'Unused or oversized warehouse', 'Medium', 14, 30, 1440, 'DBA / Cost attribution', 'COST')
) src(THRESHOLD_KEY, CATEGORY, SIGNAL_NAME, SEVERITY, THRESHOLD_VALUE, BASELINE_WINDOW_DAYS, CURRENT_WINDOW_MINUTES, WORKFLOW_ROUTE, NOTIFICATION_CHANNEL)
...
WHEN MATCHED THEN UPDATE SET
  ...
  UPDATED_BY = CURRENT_USER()
WHEN NOT MATCHED THEN INSERT
  (THRESHOLD_KEY, CATEGORY, SIGNAL_NAME, SEVERITY, THRESHOLD_VALUE, BASELINE_WINDOW_DAYS, CURRENT_WINDOW_MINUTES, WORKFLOW_ROUTE, NOTIFICATION_CHANNEL)
VALUES
  (src.THRESHOLD_KEY, src.CATEGORY, src.SIGNAL_NAME, src.SEVERITY, src.THRESHOLD_VALUE, src.BASELINE_WINDOW_DAYS, src.CURRENT_WINDOW_MINUTES, src.WORKFLOW_ROUTE, src.NOTIFICATION_CHANNEL);
...
USING (
  SELECT * FROM VALUES
    ...
    ('COST_WAREHOUSE_CREDIT_SPIKE', 'Cost', 'Warehouse credit spike', 'High', 'DBA / Cost attribution', 'Cost & Contract', 'COST'),
    ('COST_CORTEX_SPEND_SPIKE', 'Cost', 'Cortex spend spike', 'High', 'DBA / AI cost route', 'Cost & Contract', 'COST'),
    ('BEHAVIOR_USER_QUERY_ANOMALY', 'Behavior', 'User query behavior anomaly', 'High', 'DBA / Workload reviewer', 'Workload Operations', 'DBA_REVIEW'),
    ('PERF_QUEUE_PRESSURE', 'Performance', 'Warehouse queue pressure', 'High', 'DBA / Platform', 'Workload Operations', 'DBA_REVIEW'),
    ('PIPELINE_TASK_FAILURE', 'Task / Pipeline', 'Production task failure', 'Critical', 'DBA / Pipeline Reviewer', 'Workload Operations', 'PIPELINE_REVIEW'),
    ('DQ_FRESHNESS_SLA', 'Data Quality', 'Freshness SLA missed', 'High', 'Data Route', 'Workload Operations', 'DATA_QUALITY'),
    ('OPT_UNUSED_WAREHOUSE', 'Cost Recommendations', 'Unused or oversized warehouse', 'Medium', 'DBA / Cost attribution', 'Cost & Contract', 'COST')
) src(ALERT_KEY, CATEGORY, SIGNAL_NAME, DEFAULT_SEVERITY, WORKFLOW_ROUTE, ROUTE, NOTIFICATION_CHANNEL)
...
WHEN MATCHED THEN UPDATE SET
  ...
  UPDATED_BY = CURRENT_USER()
WHEN NOT MATCHED THEN INSERT
  (ALERT_KEY, CATEGORY, SIGNAL_NAME, DEFAULT_SEVERITY, WORKFLOW_ROUTE, ROUTE, NOTIFICATION_CHANNEL, TELEMETRY_LATENCY)
VALUES
  (src.ALERT_KEY, src.CATEGORY, src.SIGNAL_NAME, src.DEFAULT_SEVERITY, src.WORKFLOW_ROUTE, src.ROUTE, src.NOTIFICATION_CHANNEL, 'ACCOUNT_USAGE delayed unless otherwise documented');
...
BEGIN
  ...
  SELECT
    ...
    LISTAGG(
      ...
      ' | Workflow: ' || COALESCE(WORKFLOW_ROUTE, 'DBA') ||
      '\n' || EMAIL_BODY,
      '\n\n---\n\n'
...
USING (
  SELECT * FROM VALUES
    ('COST_CREDIT_SPIKE', 'Cost Control', 'Credit Spike', 'Medium', 24, 'DBA / Cost attribution', 'Cost & Contract', 'Explain the bill movement, identify route-backed drivers, and route cost-control actions.'),
    ('QUERY_HIGH_ERROR_RATE', 'Reliability', 'High Query Error Rate', 'High', 8, 'DBA / Workload Route', 'Workload Operations', 'Group failures by error code/query text and assign the owning team.'),
    ('TASK_FAILURE', 'Reliability', 'Task Failure', 'High', 8, 'DBA / Pipeline Reviewer', 'Workload Operations', 'Review task graph impact, retry only after root cause, and verify the next run.'),
    ('PROCEDURE_FAILURE_OR_SPIKE', 'Reliability', 'Stored Procedure Failure / Runtime Spike', 'High', 8, 'DBA / Procedure Route', 'Workload Operations', 'Compare release windows, inspect child queries, and verify runtime/cost return to baseline.'),
    ...
    ('PERF_QUERY_PRESSURE', 'Performance', 'Query Pressure', 'High', 8, 'DBA / Platform', 'Workload Operations', 'Open Query Investigation or Performance & Contention with query_id, queue/spill/lock telemetry, route, and specific optimization path.'),
    ...
    ('DQ_FRESHNESS_SLA', 'Data Quality', 'Freshness SLA Missed', 'High', 8, 'Data Route', 'Workload Operations', 'Use configured database/schema/table/column/check threshold, prove latest update/load volume, and route to data reviewer.'),
    ('OPT_UNUSED_OR_OVERSIZED_WAREHOUSE', 'Cost Recommendations', 'Unused Or Oversized Warehouse', 'Medium', 24, 'DBA / Cost attribution', 'Cost & Contract', 'Attach metering/query telemetry, workflow route, rollback SQL, and expected savings before changing warehouse settings.'),
    ('WAREHOUSE_COST_MOVEMENT', 'Cost Control', 'WAREHOUSE_COST_MOVEMENT', 'High', 8, 'DBA / Cost attribution', 'Cost & Contract', 'Explain the 7d warehouse cost movement, assign the route, and route action after telemetry review.'),
    ...
    ('CHANGE_COST_CORRELATION', 'Cost Control', 'CHANGE_COST_CORRELATION', 'High', 8, 'DBA / Cost attribution', 'Cost & Contract', 'Compare warehouse change query_id, actor, rollback telemetry, and cost movement before tuning.')
) src(RULE_ID, CATEGORY, ALERT_TYPE, DEFAULT_SEVERITY, SLA_HOURS, WORKFLOW_ROUTE, ROUTE, RUNBOOK)
...
WHEN MATCHED THEN UPDATE SET
  ...
  IS_ACTIVE = TRUE
WHEN NOT MATCHED THEN INSERT
  (RULE_ID, CATEGORY, ALERT_TYPE, DEFAULT_SEVERITY, SLA_HOURS, WORKFLOW_ROUTE, ROUTE, RUNBOOK)
VALUES
  (src.RULE_ID, src.CATEGORY, src.ALERT_TYPE, src.DEFAULT_SEVERITY, src.SLA_HOURS, src.WORKFLOW_ROUTE, src.ROUTE, src.RUNBOOK);

CREATE OR REPLACE VIEW OVERWATCH_ALERT_TRIAGE_V AS
WITH base AS (
  SELECT
    a.* EXCLUDE WORKFLOW_ROUTE,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA') AS ROUTED_ROUTE,
    NULL AS EMAIL_TARGET,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Review') AS REVIEWED_BY,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Lead') AS REVIEW_STATUS,
    COALESCE(r.WORKFLOW_ROUTE, a.WORKFLOW_ROUTE, 'DBA Lead') AS WORKFLOW_ROUTE,
    'ALERT_RULE' AS ALLOCATION_SOURCE,
    'Matched alert rule or alert row route' AS ALLOCATION_BASIS,
    COALESCE(
      r.SLA_HOURS,
...
SELECT
  ...
  CASE
    ...
    ELSE COALESCE(WORKFLOW_ROUTE, ROUTED_ROUTE, WORKFLOW_ROUTE, 'DBA')
...
CREATE TABLE IF NOT EXISTS OVERWATCH_APP_AUDIT_LOG (
...
CREATE TABLE IF NOT EXISTS OVERWATCH_ALLOCATION_RULES (
...
CREATE TABLE IF NOT EXISTS OVERWATCH_ALERT_STATE (
...
CREATE TABLE IF NOT EXISTS OVERWATCH_ALERT_STATE_HISTORY (
  EVENT_ID             VARCHAR(64),
...
CREATE TABLE IF NOT EXISTS OVERWATCH_ALERT_SLA (
  SEVERITY             VARCHAR(40) PRIMARY KEY,
  SLA_MINUTES          NUMBER,
  UPDATED_AT           TIMESTAMP_TZ DEFAULT CURRENT_TIMESTAMP(),
  UPDATED_BY           VARCHAR(200) DEFAULT CURRENT_USER()
);

MERGE INTO OVERWATCH_ALERT_SLA tgt
USING (
  SELECT * FROM VALUES
    ('CRITICAL', 60),
    ('HIGH', 240),
    ('MEDIUM', 1440),
    ('LOW', 1440)
) src(SEVERITY, SLA_MINUTES)
ON UPPER(tgt.SEVERITY) = UPPER(src.SEVERITY)
WHEN NOT MATCHED THEN INSERT (SEVERITY, SLA_MINUTES)
VALUES (src.SEVERITY, src.SLA_MINUTES);
...
BEGIN
  ...
  DELETE FROM DIM_COST_ATTRIBUTION_TAG WHERE SNAPSHOT_DATE < DATEADD('DAY', -1 * :agg_days, CURRENT_DATE());
...
CREATE OR REPLACE PROCEDURE SP_OVERWATCH_REFRESH_V2_FIRST_PAINT_MARTS()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
$$
DECLARE
  credit_price FLOAT DEFAULT 3.68;
  committed FLOAT DEFAULT 0;
  budget_source VARCHAR DEFAULT 'OVERWATCH_SETTINGS';
BEGIN
  SELECT COALESCE(MAX(TRY_TO_DOUBLE(SETTING_VALUE)), 3.68)
    INTO :credit_price
  FROM OVERWATCH_SETTINGS
  WHERE SETTING_NAME = 'CREDIT_PRICE_USD';

  SELECT COALESCE(MAX(TRY_TO_DOUBLE(SETTING_VALUE)), 0)
    INTO :committed
  FROM OVERWATCH_SETTINGS
  WHERE SETTING_NAME = 'ANNUAL_COMMITTED_CREDITS';

  SELECT COALESCE(MAX(SETTING_VALUE), 'OVERWATCH_SETTINGS')
    INTO :budget_source
  FROM OVERWATCH_SETTINGS
  WHERE SETTING_NAME = 'BUDGET_SOURCE_LABEL';

  DELETE FROM MART_V2_EXECUTIVE_SUMMARY;
  INSERT INTO MART_V2_EXECUTIVE_SUMMARY (
    SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
    EXECUTIVE_NARRATIVE, PLATFORM_SCORE, RELIABILITY_SCORE, COST_SCORE,
    SECURITY_SCORE, FRESHNESS_SCORE, ACTION_SCORE, COMMITTED_CREDITS,
    CONSUMED_CREDITS, YEAR_ELAPSED_PCT, PROJECTED_PERIOD_END_CREDITS,
  ...
  SELECT
    CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'ALL',
    'Platform score, contract burn, open work, and source freshness are loaded from v2 first-paint marts.',
    85, 85, 85, 85, 85, 85, :committed,
    COALESCE((SELECT SUM(CREDITS_USED) FROM MART_V2_WAREHOUSE_DAILY_CREDITS WHERE USAGE_DATE >= DATE_TRUNC('year', CURRENT_DATE())), 0),
    DATEDIFF('day', DATE_TRUNC('year', CURRENT_DATE()), CURRENT_DATE()) / 365,
    COALESCE((SELECT SUM(FORECAST_CREDITS) FROM MART_V2_COST_FORECAST), 0),
    0, 0,
    COALESCE((SELECT COUNT(*) FROM MART_V2_ALERT_INTELLIGENCE WHERE UPPER(SEVERITY) IN ('CRITICAL','HIGH') AND UPPER(STATUS) NOT IN ('RESOLVED','SUPPRESSED')), 0),
    COALESCE((SELECT COUNT(*) FROM OVERWATCH_ALERT_STATE WHERE UPPER(STATUS) NOT IN ('RESOLVED','SUPPRESSED')), 0),
    COALESCE((SELECT WAREHOUSE_NAME FROM MART_V2_WAREHOUSE_DAILY_CREDITS ORDER BY COST_USD DESC LIMIT 1), 'No cost driver loaded'),
    COALESCE((SELECT SUM(EST_MONTHLY_COST_USD) FROM MART_V2_OVERWATCH_APP_SELF_COST_DAILY), 0),
    :budget_source;

  DELETE FROM MART_V2_SOURCE_FRESHNESS;
  INSERT INTO MART_V2_SOURCE_FRESHNESS (
    SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
    SOURCE_NAME, LATEST_SOURCE_TS, FRESHNESS_MINUTES, SOURCE_LATENCY_NOTE, STATUS
  )
  SELECT CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'ALL', source_name,
         CURRENT_TIMESTAMP(), 0, source_latency_note, 'CURRENT'
  FROM VALUES
    ('executive summary', 'Precomputed v2 mart'),
    ('alert intelligence', 'Precomputed v2 mart'),
    ('cost forecast', 'Precomputed v2 mart'),
    ('login security daily', 'ACCOUNT_USAGE latency disclosed')
    AS freshness(source_name, source_latency_note);

  DELETE FROM MART_V2_COST_FORECAST;
  INSERT INTO MART_V2_COST_FORECAST (
    SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
    DAY, ACTUAL_CREDITS, FORECAST_CREDITS, BUDGET_CREDITS,
    LOWER_BOUND_CREDITS, UPPER_BOUND_CREDITS, METHOD_LABEL,
    CONFIDENCE_LABEL, SOURCE_FRESHNESS, BUDGET_SOURCE
  )
  SELECT CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'ALL',
         DATEADD('day', seq4(), DATE_TRUNC('month', CURRENT_DATE())),
         IFF(DATEADD('day', seq4(), DATE_TRUNC('month', CURRENT_DATE())) <= CURRENT_DATE(), 0, NULL),
         0,
         IFF(:committed > 0, :committed / 365, NULL),
         0,
         0,
         'Seasonal fallback',
         'Directional',
         'Precomputed cost forecast mart',
         :budget_source
  FROM TABLE(GENERATOR(ROWCOUNT => 31));

  DELETE FROM MART_V2_CONTRACT_BURN_DOWN;
  INSERT INTO MART_V2_CONTRACT_BURN_DOWN (
    SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, WAREHOUSE_NAME,
    COMMITTED_CREDITS, CONSUMED_CREDITS, ANNUAL_COMMIT_BURN_PCT,
    YEAR_ELAPSED_PCT, PROJECTED_PERIOD_END_CREDITS,
    PROJECTED_OVER_UNDER_CREDITS, PROJECTED_OVER_UNDER_USD, BUDGET_SOURCE
  ...
  SELECT CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'ALL',
         :committed, 0, 0,
         DATEDIFF('day', DATE_TRUNC('year', CURRENT_DATE()), CURRENT_DATE()) / 365,
         0, 0 - :committed, (0 - :committed) * :credit_price, :budget_source;

  DELETE FROM MART_V2_DBA_MORNING_COCKPIT;
  INSERT INTO MART_V2_DBA_MORNING_COCKPIT
  SELECT CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'ALL',
         0, 0, 0, 0, 0, 0, 0,
         'Live current-state badge requires v2 RBAC.',
         'ACCOUNT_USAGE latency disclosed for delayed metrics.';

  DELETE FROM MART_V2_OVERWATCH_APP_SELF_COST_DAILY;
  INSERT INTO MART_V2_OVERWATCH_APP_SELF_COST_DAILY
  SELECT CURRENT_TIMESTAMP(), 'ALL', 'ALL', 30, 'WH_ALFA_OVERWATCH',
         CURRENT_DATE(), 0, 0, 0, 0, 0;

  RETURN 'V2 first-paint marts refreshed.';
END;
$$;
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN QUERY_TEXT ILIKE '%OWNERSHIP%' THEN 'WORKFLOW_ROUTE'
  ...
  INSERT INTO DIM_PROCEDURE_SNAPSHOT (
    ...
    CONTROL_ROLE, PROCEDURE_LANGUAGE, ARGUMENT_SIGNATURE, LAST_ALTERED, IS_ORPHAN_CANDIDATE
  )
  ...
  SELECT
    ...
    p.PROCEDURE_NAME,
    p.PROCEDURE_OWNER AS CONTROL_ROLE,
  ...
  BEGIN
    ...
    INSERT INTO FACT_TASK_CRITICAL_PATH (
      ...
      LAST_RUN_AT, BLAST_RADIUS, WAREHOUSES, PROCEDURES, CONTROL_ROLE,
    ...
    SELECT
      ...
      WAREHOUSES,
      PROCEDURES,
      'DBA / Pipeline Reviewer' AS CONTROL_ROLE,
      CASE
        WHEN FAILURES_7D > 0 OR SUSPENDED_TASKS > 0
          THEN 'Route review required before retry or recovery.'
        WHEN DOWNSTREAM_TASK_COUNT >= 3
          THEN 'Route review required before graph-level changes.'
...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      ...
      0::NUMBER(18,6) AS CREDITS_USED_COMPUTE,
      0::NUMBER(18,6) AS CREDITS_USED_CLOUD_SERVICES,
      ROUND(SUM(CREDITS_ADJUSTMENT_CLOUD_SERVICES), 6) AS CREDITS_ADJUSTMENT_CLOUD_SERVICES,
      ROUND(SUM(CREDITS_BILLED), 6) AS CREDITS_BILLED,
      MAX(RATE_USD) AS RATE_USD,
      ROUND(SUM(CREDITS_BILLED * RATE_USD), 2) AS EST_COST_USD,
  ...
  BEGIN
    DELETE FROM DIM_COST_ATTRIBUTION_TAG
    WHERE SNAPSHOT_DATE = CURRENT_DATE();

    INSERT INTO DIM_COST_ATTRIBUTION_TAG (
      SNAPSHOT_DATE, ATTRIBUTION_SCOPE, OBJECT_DATABASE, OBJECT_SCHEMA, OBJECT_NAME,
      TAG_NAME, TAG_VALUE, ATTRIBUTION_TYPE, PRIORITY
    )
    SELECT
      CURRENT_DATE() AS SNAPSHOT_DATE,
      UPPER(t.DOMAIN) AS ATTRIBUTION_SCOPE,
      t.OBJECT_DATABASE,
      t.OBJECT_SCHEMA,
      t.OBJECT_NAME,
      t.TAG_NAME,
      t.TAG_VALUE,
      cfg.ATTRIBUTION_TYPE,
      cfg.PRIORITY
    FROM SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES t
    JOIN OVERWATCH_ATTRIBUTION_TAG_NAMES cfg
  ...
  EXCEPTION
    WHEN OTHER THEN
      INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, LOAD_FINISHED_AT, STATUS, MESSAGE)
      VALUES ('DIM_COST_ATTRIBUTION_TAG', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), 'SKIPPED', :SQLERRM);
  END;
  ...
  INSERT INTO FACT_CHARGEBACK_DAILY (
    ...
    CHARGEBACK_READY, SCOPE_REVIEW, COST_ATTRIBUTION, ALLOCATION_SOURCE, ALLOCATION_BASIS
  )
  (WITH query_hour AS (
    SELECT
      HOUR_START,
  ...
  owner_tags AS (
    SELECT
      ...
      PRIORITY,
      ROW_NUMBER() OVER (
        PARTITION BY ATTRIBUTION_SCOPE,
          UPPER(COALESCE(NULLIF(OBJECT_DATABASE, ''), OBJECT_NAME)),
          UPPER(OBJECT_NAME)
        ORDER BY PRIORITY, TAG_NAME
      ) AS RN
    FROM DIM_COST_ATTRIBUTION_TAG
    WHERE SNAPSHOT_DATE = CURRENT_DATE()
      AND TAG_VALUE IS NOT NULL
  ),
  database_owners AS (
    SELECT DATABASE_NAME, TAG_NAME, TAG_VALUE, ATTRIBUTION_TYPE
    FROM owner_tags
    WHERE ATTRIBUTION_SCOPE = 'DATABASE'
      AND RN = 1
  ),
  warehouse_owners AS (
    SELECT OBJECT_NAME AS WAREHOUSE_NAME, TAG_NAME, TAG_VALUE, ATTRIBUTION_TYPE
    FROM owner_tags
    WHERE ATTRIBUTION_SCOPE = 'WAREHOUSE'
      AND RN = 1
  )
  SELECT
    c.USAGE_DATE,
    ...
    CASE
      ...
      WHEN c.ENVIRONMENT_ROLLUP = 'Trexis' AND COALESCE(wo.TAG_VALUE, dbo.TAG_VALUE) IS NOT NULL
        THEN 'Trexis database context allocated across metered warehouse-hour credits; route tag telemetry is attached.'
      ...
      WHEN c.ENVIRONMENT_ROLLUP IN ('PROD', 'ALFA_EDW_PRD', 'ALFA_EDW_MGM', 'DEV_ALL') AND COALESCE(wo.TAG_VALUE, dbo.TAG_VALUE) IS NOT NULL
        THEN 'Query database context allocated across metered warehouse-hour credits; route tag telemetry is attached.'
      ...
      ELSE 'Shared warehouse/query context requires route validation before billing.'
    ...
    CASE
      WHEN wo.TAG_VALUE IS NOT NULL THEN 'Warehouse route tag ' || wo.TAG_NAME || '=' || wo.TAG_VALUE || ' from SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES.'
      WHEN dbo.TAG_VALUE IS NOT NULL THEN 'Database route tag ' || dbo.TAG_NAME || '=' || dbo.TAG_VALUE || ' from SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES.'
      WHEN c.USER_NAME IS NOT NULL AND UPPER(c.USER_NAME) NOT IN ('', 'UNKNOWN_USER')
        THEN 'Query user present; validate route/tag telemetry before billing.'
  ...
  INSERT INTO FACT_WAREHOUSE_OPERABILITY_DAILY (
    ...
    VERIFIED_CLOSURES, REVIEW_GAP_ROWS, NEXT_CONTROL_ACTION, LAST_ACTIVITY_TS
  )
  (WITH query_daily AS (
    SELECT
  ...
  setting_rollup AS (
    SELECT
      ...
      CASE
        WHEN COUNT_IF(AUDIT_READINESS IN ('Route Route Blocked', 'Pre-Change Blocked', 'Verification Blocked')) > 0 THEN 'Setting Review Blocked'
      ...
      END AS CONTROL_STATE,
      CASE
        WHEN COUNT_IF(AUDIT_READINESS IN ('Route Route Blocked', 'Pre-Change Blocked', 'Verification Blocked')) > 0 THEN 1
      ...
      0 AS FIXED_WITHOUT_VERIFICATION,
      0 AS VERIFIED_CLOSURES,
      COUNT_IF(UPPER(COALESCE(APPROVAL_STATE, '')) IN ('', 'REQUESTED', 'PENDING', 'REQUIRED')) AS REVIEW_GAP_ROWS,
  ...
  action_rollup AS (
    SELECT
      ...
      COUNT_IF(UPPER(COALESCE(REVIEW_STATUS, '')) IN ('', 'PENDING', 'REQUESTED', 'REQUIRED')) AS REVIEW_GAP_ROWS,
      CASE
        WHEN COUNT_IF(COALESCE(UPPER(STATUS), 'OPEN') NOT IN ('FIXED', 'IGNORED') AND DUE_DATE < CURRENT_DATE()) > 0 THEN 'Escalate overdue warehouse action with route, ticket, rollback path, and post-change telemetry.'
  ...
  INSERT INTO FACT_SECURITY_OPERABILITY_DAILY (
    ...
    REVIEW_GAP_ROWS, NEXT_CONTROL_ACTION, LAST_ACTIVITY_TS
  )
  (WITH access_rollup AS (
    SELECT
      ...
      0 AS VERIFIED_CLOSURES,
      COUNT_IF(
        UPPER(COALESCE(REVIEW_STATUS, IAM_APPROVAL_STATE, '')) IN ('', 'PENDING', 'REQUESTED', 'REQUIRED')
      ) AS REVIEW_GAP_ROWS,
  ...
  action_rollup AS (
    SELECT
      ...
      COUNT_IF(UPPER(COALESCE(REVIEW_STATUS, '')) IN ('', 'PENDING', 'REQUESTED', 'REQUIRED')) AS REVIEW_GAP_ROWS,
      CASE
        WHEN COUNT_IF(COALESCE(UPPER(STATUS), 'OPEN') NOT IN ('FIXED', 'IGNORED') AND DUE_DATE < CURRENT_DATE()) > 0 THEN 'Escalate overdue security action with route, ticket, and source telemetry.'
  ...
  INSERT INTO FACT_ACCOUNT_HEALTH_OPERABILITY_DAILY (
    ...
    FIXED_WITHOUT_VERIFICATION, VERIFIED_CLOSURES, REVIEW_GAP_ROWS,
  ...
  (WITH checklist_rollup AS (
    SELECT
      ...
      0 AS FIXED_WITHOUT_VERIFICATION,
      0 AS VERIFIED_CLOSURES,
      COUNT_IF(UPPER(COALESCE(APPROVAL_REQUIRED, '')) = 'YES') AS REVIEW_GAP_ROWS,
  ...
  action_rollup AS (
    SELECT
      ...
      CASE
        WHEN COUNT_IF(COALESCE(UPPER(STATUS), 'OPEN') NOT IN ('FIXED', 'IGNORED') AND DUE_DATE < CURRENT_DATE()) > 0 THEN 'Escalate overdue DBA Control Room checklist action with route, ticket, and telemetry.'
  ...
  privileged_grant_rollup AS (
    SELECT
      ...
      0 AS FIXED_WITHOUT_VERIFICATION,
      0 AS VERIFIED_CLOSURES,
      SUM(GRANT_COUNT) AS REVIEW_GAP_ROWS,
      0 AS RECOVERY_RISK_ROWS,
      'Review privileged role business need, workflow route, and least-privilege telemetry before accepting the account posture.' AS NEXT_CONTROL_ACTION,
  ...
  INSERT INTO FACT_CHANGE_CONTROL_OPERABILITY_DAILY (
    ...
    FINDING_TYPE, ENTITY, WORKFLOW_ROUTE, WORKFLOW_ROUTE, SEVERITY,
    ...
    OPEN_ACTIONS, OVERDUE_OPEN, FIXED_WITHOUT_VERIFICATION, VERIFIED_CLOSURES,
    REVIEW_GAP_ROWS, CONTROL_STATE, CONTROL_RANK, NEXT_CONTROL_ACTION,
    LAST_ACTIVITY_TS
  )
  (WITH evidence_rollup AS (
    SELECT
      ...
      FINDING_TYPE || '|' || COALESCE(WORKFLOW_ROUTE, 'Unknown') AS CONTROL_KEY,
      FINDING_TYPE,
      NULL::VARCHAR AS ENTITY,
      MAX_BY(WORKFLOW_ROUTE, SNAPSHOT_TS) AS WORKFLOW_ROUTE,
      MAX_BY(WORKFLOW_ROUTE, SNAPSHOT_TS) AS WORKFLOW_ROUTE,
      MAX_BY(SEVERITY, SNAPSHOT_TS) AS SEVERITY,
    ...
    WHERE SNAPSHOT_TS >= DATEADD('DAY', -35, CURRENT_TIMESTAMP())
    GROUP BY TO_DATE(SNAPSHOT_TS), COMPANY, COALESCE(ENVIRONMENT, 'No Database Context'), FINDING_TYPE, COALESCE(WORKFLOW_ROUTE, 'Unknown')
  ),
  action_rollup AS (
    SELECT
      TO_DATE(COALESCE(UPDATED_AT, CREATED_AT)) AS SNAPSHOT_DATE,
      ...
      MAX_BY(WORKFLOW_ROUTE, COALESCE(UPDATED_AT, CREATED_AT)) AS WORKFLOW_ROUTE,
      MAX_BY(WORKFLOW_ROUTE, COALESCE(UPDATED_AT, CREATED_AT)) AS WORKFLOW_ROUTE,
      MAX_BY(SEVERITY, COALESCE(UPDATED_AT, CREATED_AT)) AS SEVERITY,
      0 AS EVIDENCE_ROWS,
      COUNT_IF(UPPER(SEVERITY) IN ('CRITICAL', 'HIGH')) AS HIGH_RISK_CHANGES,
      ...
      END AS CONTROL_RANK,
      CASE
        WHEN COUNT_IF(UPPER(STATUS) NOT IN ('FIXED', 'IGNORED') AND DUE_DATE < CURRENT_DATE()) > 0 THEN 'Escalate overdue object-change action with route, ticket, and rollback context.'
...
BEGIN
  ...
  BEGIN
    DELETE FROM MART_USER_DIM_CURRENT;

    INSERT INTO MART_USER_DIM_CURRENT (
      USER_ID, USER_NAME, LOGIN_NAME, DISPLAY_NAME, FIRST_NAME, LAST_NAME,
      USER_DISPLAY_NAME, USER_CHART_LABEL, USER_ADMIN_LABEL, EMAIL,
      USER_TYPE, DELETED_ON, DISABLED, LOAD_TS
    )
    SELECT
      USER_ID::VARCHAR,
      NAME::VARCHAR,
      ...
      FIRST_NAME::VARCHAR,
      LAST_NAME::VARCHAR,
      COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME, LOGIN_NAME)::VARCHAR AS USER_DISPLAY_NAME,
      COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NAME, LOGIN_NAME, 'Unknown user')::VARCHAR AS USER_CHART_LABEL,
      IFF(
        NAME IS NOT NULL
        AND NAME <> COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME, LOGIN_NAME, 'Unknown user'),
        COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME, LOGIN_NAME, 'Unknown user') || ' (' || NAME || ')',
        COALESCE(NULLIF(TRIM(COALESCE(FIRST_NAME, '') || ' ' || COALESCE(LAST_NAME, '')), ''), NULLIF(DISPLAY_NAME, ''), NAME, LOGIN_NAME, 'Unknown user')
      )::VARCHAR AS USER_ADMIN_LABEL,
      EMAIL::VARCHAR,
      TYPE::VARCHAR,
      DELETED_ON,
      DISABLED,
      CURRENT_TIMESTAMP()
    -- OVERWATCH_CURRENT_OBJECT_SNAPSHOT_SCAN: USERS supplies display labels for daily charts; keep active and recently deleted users for attribution stability.
    FROM SNOWFLAKE.ACCOUNT_USAGE.USERS
    WHERE DELETED_ON IS NULL
       OR DELETED_ON >= DATEADD('DAY', -365, CURRENT_TIMESTAMP());
  EXCEPTION
    WHEN OTHER THEN
      INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, LOAD_FINISHED_AT, STATUS, MESSAGE)
      VALUES ('SP_OVERWATCH_LOAD_USER_DIM_CURRENT', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), 'SKIPPED', :SQLERRM);
  END;
  ...
  BEGIN
    INSERT INTO FACT_CORTEX_DAILY (
      USAGE_DATE, COMPANY, USER_ID, USER_NAME, USER_DISPLAY_NAME, USER_CHART_LABEL, USER_EMAIL,
      SOURCE, CREDITS_USED, EST_COST_USD, REQUEST_COUNT
    )
    (WITH user_company AS (
      SELECT
        GRANTEE_NAME AS USER_NAME,
    ...
    SELECT
      ...
      COALESCE(u.NAME, raw.USER_ID::VARCHAR) AS USER_NAME,
      COALESCE(
...
CREATE OR REPLACE PROCEDURE SP_OVERWATCH_REFRESH_EXECUTIVE_COMMAND_CENTER()
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS
$$
BEGIN
  DELETE FROM MART_EXECUTIVE_COMMAND_CENTER_KPI;
  DELETE FROM MART_EXECUTIVE_COMMAND_CENTER_TIMESERIES;
  DELETE FROM MART_EXECUTIVE_COMMAND_CENTER_WAREHOUSE;
  DELETE FROM MART_EXECUTIVE_COMMAND_CENTER_ALERTS;
  DELETE FROM MART_EXECUTIVE_COMMAND_CENTER_CONTEXT;

  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_KPI (
    COMPANY, ENVIRONMENT, WINDOW_START, WINDOW_END, WAREHOUSE_SCOPE,
    METRIC_KEY, METRIC_LABEL, METRIC_VALUE_TEXT, METRIC_VALUE_NUMBER,
    METRIC_UNIT, STATUS_TONE, STATUS_LABEL, TREND_DIRECTION,
    TREND_VALUE_TEXT, TREND_POINTS, UPDATED_AT, SOURCE_QUALITY, SOURCE_NOTE
  )
  WITH latest AS (
    SELECT *
    ...
    QUALIFY ROW_NUMBER() OVER (
      PARTITION BY COMPANY, ENVIRONMENT, WINDOW_DAYS
  ...
  SELECT COMPANY, ENVIRONMENT, DATEADD('day', -COALESCE(WINDOW_DAYS, 7), SNAPSHOT_TS), SNAPSHOT_TS, 'All scoped warehouses',
         'summary_status', 'Summary Status', HEADLINE, NULL, 'text',
         CASE WHEN LOWER(STATE) LIKE '%critical%' THEN 'critical'
              WHEN LOWER(STATE) LIKE '%pending%' OR LOWER(STATE) LIKE '%stale%' THEN 'warning'
              WHEN LOWER(STATE) LIKE '%ready%' OR LOWER(STATE) LIKE '%clear%' THEN 'healthy'
              ELSE 'neutral' END,
         STATE, NULL, SOURCE, ARRAY_CONSTRUCT(), CURRENT_TIMESTAMP(), CONFIDENCE, SOURCE_GAP_DETAIL
  FROM latest
  UNION ALL
  SELECT COMPANY, ENVIRONMENT, DATEADD('day', -COALESCE(WINDOW_DAYS, 7), SNAPSHOT_TS), SNAPSHOT_TS, 'All scoped warehouses',
         'source_status', 'Source Status', COALESCE(DATA_AVAILABILITY_STATE, 'Unavailable'), SOURCE_COVERAGE_PCT, 'pct',
         CASE WHEN COALESCE(REQUIRED_MISSING_SOURCE_COUNT, MISSING_SOURCE_COUNT, 0) > 0 THEN 'warning' ELSE 'healthy' END,
         SOURCE_FRESHNESS, NULL, SOURCE_GAP_DETAIL, ARRAY_CONSTRUCT(), CURRENT_TIMESTAMP(), CONFIDENCE, SOURCE_OBJECTS
  FROM latest
  UNION ALL
  SELECT COMPANY, ENVIRONMENT, DATEADD('day', -COALESCE(WINDOW_DAYS, 7), SNAPSHOT_TS), SNAPSHOT_TS, 'All scoped warehouses',
  ...
  UNION ALL
  SELECT COMPANY, ENVIRONMENT, DATEADD('day', -COALESCE(WINDOW_DAYS, 7), SNAPSHOT_TS), SNAPSHOT_TS, 'All scoped warehouses',
         'open_actions', 'Open Actions', TO_VARCHAR(COALESCE(a.ACTION_COUNT, 0)), COALESCE(a.ACTION_COUNT, 0), 'count',
         CASE WHEN COALESCE(a.ACTION_COUNT, 0) > 0 THEN 'warning' ELSE 'healthy' END,
         'Action queue', NULL, NULL, ARRAY_CONSTRUCT(), CURRENT_TIMESTAMP(), CONFIDENCE, SOURCE
  FROM latest l
  LEFT JOIN (
    SELECT BRIEF_ID, COUNT(*) AS ACTION_COUNT
    FROM MART_SECTION_COMMAND_ACTION
    WHERE SECTION_NAME = 'Executive Landing'
    GROUP BY BRIEF_ID
  ) a ON a.BRIEF_ID = l.BRIEF_ID;

  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_WAREHOUSE (
    COMPANY, ENVIRONMENT, WINDOW_START, WINDOW_END, WAREHOUSE_NAME,
    CREDITS_USED, ESTIMATED_COST_USD, PCT_OF_TOTAL, QUERY_COUNT,
    FAILED_QUERY_COUNT, AVG_DURATION_SEC, P95_DURATION_SEC, UPDATED_AT
  )
  WITH wh AS (
    SELECT
      COALESCE(COMPANY, 'ALL') AS COMPANY,
      'ALL' AS ENVIRONMENT,
      MIN(HOUR_START) AS WINDOW_START,
      MAX(HOUR_START) AS WINDOW_END,
      WAREHOUSE_NAME,
      SUM(COALESCE(CREDITS_USED, 0)) AS CREDITS_USED,
      SUM(COALESCE(EST_COST_USD, 0)) AS ESTIMATED_COST_USD
    FROM FACT_WAREHOUSE_HOURLY
    WHERE HOUR_START >= DATEADD('day', -7, CURRENT_TIMESTAMP())
    GROUP BY COALESCE(COMPANY, 'ALL'), WAREHOUSE_NAME
  )
  SELECT
    COMPANY, ENVIRONMENT, WINDOW_START, WINDOW_END, WAREHOUSE_NAME,
    CREDITS_USED, ESTIMATED_COST_USD,
    IFF(SUM(CREDITS_USED) OVER (PARTITION BY COMPANY, ENVIRONMENT) = 0, 0,
        CREDITS_USED / SUM(CREDITS_USED) OVER (PARTITION BY COMPANY, ENVIRONMENT) * 100),
    0, 0, NULL, NULL, CURRENT_TIMESTAMP()
  FROM wh;

  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_ALERTS (
    COMPANY, ENVIRONMENT, SEVERITY, STATUS, SIGNAL, DETAILS, OBJECT_NAME,
    WORKFLOW_ROUTE, SLA, EVENT_TS, ROUTE_KEY, EVIDENCE_ID, UPDATED_AT
  )
  SELECT
    COMPANY, ENVIRONMENT, SEVERITY, 'Open', SIGNAL, DETAIL, ENTITY_NAME,
    COALESCE(ROUTE_NAME, WORKFLOW_ROUTE), SLA_STATE, COALESCE(FIRST_SEEN_TS, SNAPSHOT_TS),
    ROUTE_SECTION, EVIDENCE_ID, CURRENT_TIMESTAMP()
  FROM MART_SECTION_COMMAND_EXCEPTION
  WHERE SECTION_NAME = 'Executive Landing';

  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_CONTEXT (
    COMPANY, ENVIRONMENT, WINDOW_START, WINDOW_END, WAREHOUSES_MONITORED,
    DATA_SOURCE_STATUS, FRESHNESS_STATUS, EVIDENCE_LOAD_STATUS,
    LAST_SUCCESSFUL_SNAPSHOT_AT, SOURCE_QUALITY, UPDATED_AT
  )
  SELECT
  ...
  INSERT INTO MART_EXECUTIVE_COMMAND_CENTER_TIMESERIES (
    COMPANY, ENVIRONMENT, WAREHOUSE_NAME, METRIC_KEY, POINT_TS, POINT_DATE,
    METRIC_VALUE, METRIC_VALUE_TEXT, STATUS_TONE, UPDATED_AT
  )
  SELECT
    COALESCE(COMPANY, 'ALL'), 'ALL', WAREHOUSE_NAME, 'warehouse_credits',
    HOUR_START, TO_DATE(HOUR_START), SUM(COALESCE(CREDITS_USED, 0)),
    TO_VARCHAR(SUM(COALESCE(CREDITS_USED, 0))), 'info', CURRENT_TIMESTAMP()
  FROM FACT_WAREHOUSE_HOURLY
  WHERE HOUR_START >= DATEADD('day', -7, CURRENT_TIMESTAMP())
  GROUP BY COALESCE(COMPANY, 'ALL'), WAREHOUSE_NAME, HOUR_START;

  RETURN 'Executive command-center marts refreshed.';
END;
$$;
...
CREATE OR REPLACE PROCEDURE SP_OVERWATCH_LOAD_SECURITY_CREDENTIAL_EXPIRATIONS()
RETURNS VARCHAR
LANGUAGE SQL
AS
$$
...
BEGIN
  INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, STATUS, MESSAGE)
  VALUES ('SP_OVERWATCH_LOAD_SECURITY_CREDENTIAL_EXPIRATIONS', :started_at, 'RUNNING', 'Started credential-expiration compact refresh.');

  BEGIN
    DELETE FROM MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT;

    INSERT INTO MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT (
      SNAPSHOT_TS, COMPANY, ENVIRONMENT, CREDENTIAL_ID, CREDENTIAL_NAME, USER_NAME,
      USER_DISPLAY_NAME, USER_CHART_LABEL, USER_ADMIN_LABEL, USER_EMAIL, TYPE, DOMAIN, STATUS,
      CREATED_BY, LAST_ALTERED_BY, CREATED_ON, LAST_USED_ON, LAST_ALTERED, EXPIRATION_DATE,
      DAYS_TO_EXPIRATION, EXPIRATION_BUCKET, CREDENTIAL_EXPIRING_30D_FLAG,
      CREDENTIAL_EXPIRING_7D_FLAG, CREDENTIAL_EXPIRED_FLAG, CREDENTIAL_EXPIRATION_SEVERITY,
      RECOMMENDED_ACTION, LOAD_TS
    )
    WITH user_company AS (
      SELECT
        GRANTEE_NAME AS USER_NAME,
    ...
    source_rows AS (
      SELECT
        c.CREDENTIAL_ID::VARCHAR AS CREDENTIAL_ID,
        c.NAME::VARCHAR AS CREDENTIAL_NAME,
        c.USER_NAME::VARCHAR AS USER_NAME,
        c.TYPE::VARCHAR AS TYPE,
        c.DOMAIN::VARCHAR AS DOMAIN,
        c.STATUS::VARCHAR AS STATUS,
        c.CREATED_BY::VARCHAR AS CREATED_BY,
        c.LAST_ALTERED_BY::VARCHAR AS LAST_ALTERED_BY,
        c.CREATED_ON::TIMESTAMP_NTZ AS CREATED_ON,
        c.LAST_USED_ON::TIMESTAMP_NTZ AS LAST_USED_ON,
        c.LAST_ALTERED::TIMESTAMP_NTZ AS LAST_ALTERED,
        c.EXPIRATION_DATE::TIMESTAMP_NTZ AS EXPIRATION_DATE,
        DATEDIFF('DAY', CURRENT_DATE(), c.EXPIRATION_DATE::DATE) AS DAYS_TO_EXPIRATION
      FROM SNOWFLAKE.ACCOUNT_USAGE.CREDENTIALS c
      WHERE c.EXPIRATION_DATE IS NOT NULL
        AND c.EXPIRATION_DATE <= DATEADD('DAY', 30, CURRENT_TIMESTAMP())
    )
    SELECT
      :started_at AS SNAPSHOT_TS,
      CASE
        WHEN COALESCE(u.NAME, sr.USER_NAME) ILIKE 'TRXS_%'
          OR (COALESCE(uc.HAS_TRXS_ROLE, 0) = 1 AND COALESCE(uc.HAS_NON_TRXS_ROLE, 0) = 0)
          THEN 'Trexis'
        ELSE 'ALFA'
      END AS COMPANY,
      'ALL' AS ENVIRONMENT,
      sr.CREDENTIAL_ID,
      sr.CREDENTIAL_NAME,
      COALESCE(u.NAME, sr.USER_NAME) AS USER_NAME,
      COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, NULLIF(u.LOGIN_NAME, ''), 'Unknown user') AS USER_DISPLAY_NAME,
      COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), u.NAME, NULLIF(u.LOGIN_NAME, ''), 'Unknown user') AS USER_CHART_LABEL,
      IFF(
        u.NAME IS NOT NULL
        AND u.NAME <> COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, NULLIF(u.LOGIN_NAME, ''), 'Unknown user'),
        COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, NULLIF(u.LOGIN_NAME, ''), 'Unknown user') || ' (' || u.NAME || ')',
        COALESCE(NULLIF(TRIM(COALESCE(u.FIRST_NAME, '') || ' ' || COALESCE(u.LAST_NAME, '')), ''), NULLIF(u.DISPLAY_NAME, ''), u.NAME, NULLIF(u.LOGIN_NAME, ''), 'Unknown user')
      ) AS USER_ADMIN_LABEL,
      u.EMAIL AS USER_EMAIL,
      sr.TYPE,
      sr.DOMAIN,
      ...
      sr.LAST_ALTERED,
      sr.EXPIRATION_DATE,
      sr.DAYS_TO_EXPIRATION,
      CASE
        WHEN sr.DAYS_TO_EXPIRATION < 0 THEN 'expired'
        WHEN sr.DAYS_TO_EXPIRATION <= 7 THEN 'expires_0_7_days'
        WHEN sr.DAYS_TO_EXPIRATION <= 30 THEN 'expires_8_30_days'
        ELSE 'ok'
      END AS EXPIRATION_BUCKET,
      sr.DAYS_TO_EXPIRATION BETWEEN 0 AND 30 AS CREDENTIAL_EXPIRING_30D_FLAG,
      sr.DAYS_TO_EXPIRATION BETWEEN 0 AND 7 AS CREDENTIAL_EXPIRING_7D_FLAG,
      sr.DAYS_TO_EXPIRATION < 0 AS CREDENTIAL_EXPIRED_FLAG,
      CASE
        WHEN sr.DAYS_TO_EXPIRATION < 0 THEN 'Critical'
        WHEN sr.DAYS_TO_EXPIRATION <= 7 THEN 'High'
        WHEN sr.DAYS_TO_EXPIRATION <= 30 THEN 'Medium'
        ELSE 'Info'
      END AS CREDENTIAL_EXPIRATION_SEVERITY,
      'Rotate or renew credential before expiration.' AS RECOMMENDED_ACTION,
      CURRENT_TIMESTAMP() AS LOAD_TS
    FROM source_rows sr
    LEFT JOIN SNOWFLAKE.ACCOUNT_USAGE.USERS u
      ON UPPER(u.NAME) = UPPER(sr.USER_NAME)
    LEFT JOIN user_company uc
      ON UPPER(uc.USER_NAME) = UPPER(COALESCE(u.NAME, sr.USER_NAME));
    ...
    SET LOAD_FINISHED_AT = CURRENT_TIMESTAMP(),
        STATUS = 'SUCCEEDED',
        ROWS_LOADED = :rows_loaded,
        MESSAGE = 'Credential-expiration compact mart refreshed.'
    WHERE LOAD_NAME = 'SP_OVERWATCH_LOAD_SECURITY_CREDENTIAL_EXPIRATIONS'
      AND LOAD_STARTED_AT = :started_at;
  EXCEPTION
    WHEN OTHER THEN
      UPDATE OVERWATCH_LOAD_AUDIT
  ...
  RETURN 'OVERWATCH credential-expiration compact refresh complete or skipped if unavailable.';
END;
$$;
...
BEGIN
  ...
  SELECT
    ...
    'Open Cost & Contract root cause, assign route, and route action after telemetry review.',
  ...
  INSERT INTO OVERWATCH_ALERTS (
    ...
    ENTITY_NAME, ENTITY, MESSAGE, DETAIL, SUGGESTED_ACTION, PROOF_QUERY, WORKFLOW_ROUTE,
...
BEGIN
  ...
  DELETE FROM MART_OPERATIONAL_ROUTE_COVERAGE
  WHERE SNAPSHOT_TS < DATEADD('DAY', -35, CURRENT_TIMESTAMP());
  ...
  INSERT INTO OVERWATCH_DATA_TRUST_STATUS (
    ...
    STATUS, CONFIDENCE, BUSINESS_IMPACT, WORKFLOW_ROUTE, NEXT_ACTION
  )
  (WITH companies AS (
  ...
  expanded AS (
    ...
    GROUP BY
      ...
      src.BUSINESS_IMPACT, src.WORKFLOW_ROUTE
  )
  SELECT
    :snapshot_ts,
    COMPANY,
  ...
  INSERT INTO MART_DATA_TRUST_SUMMARY (
    ...
    CONFIDENCE, VALUE, VALUE_USD, FRESHNESS_MINUTES, SOURCE_OBJECT, WORKFLOW_ROUTE,
  ...
  INSERT INTO MART_OPERATIONAL_ROUTE_COVERAGE (
    ...
    TOP_GAP_ENTITY, WORKFLOW_ROUTE, NEXT_ACTION
  )
  (WITH raw_items AS (
    SELECT
      COALESCE(COMPANY, 'ALL') AS COMPANY,
  ...
  annotated AS (
    SELECT
      r.*,
      UPPER(TRIM(COALESCE(r.WORKFLOW_ROUTE, ''))) AS ROUTE_KEY,
      COALESCE(NULLIF(TRIM(r.WORKFLOW_ROUTE), ''), MAX(m.WORKFLOW_ROUTE) OVER (PARTITION BY r.SURFACE, r.ENTITY_TYPE), 'DBA Review') AS ROUTE
    FROM raw_items r
    LEFT JOIN OVERWATCH_OPERATIONAL_ROUTE_MAP m
      ON m.ACTIVE
  ...
  routed AS (
    SELECT
      *,
      IFF(ROUTE_KEY NOT IN ('', 'DBA', 'OVERWATCH', 'UNKNOWN', 'N/A', 'NONE'), 1, 0) AS ROUTE_READY
    FROM annotated
  )
  SELECT
    :snapshot_ts,
    ...
    CASE
      WHEN COUNT(*) - SUM(ROUTE_READY) > 0 THEN 'Workflow Gap'
      ELSE 'Route Routed'
    END AS TRUST_LEVEL,
    'estimated' AS CONFIDENCE,
    MIN(IFF(ROUTE_READY = 0, ENTITY_NAME, NULL)) AS TOP_GAP_ENTITY,
    MAX(ROUTE) AS WORKFLOW_ROUTE,
    CASE
      WHEN COUNT(*) - SUM(ROUTE_READY) > 0 THEN 'Assign a named route, review target, or ticket route before closure.'
  ...
  baseline AS (
    SELECT * FROM VALUES
      ('Alert Center', 'ALERT', 'DBA / Alert reviewer'),
      ('Security Monitoring', 'SECURITY', 'Security / DBA'),
      ('Cost & Contract', 'ACTION', 'DBA / Cost attribution'),
      ('DBA Control Room', 'ACTION', 'DBA Review')
    AS t(SURFACE, ENTITY_TYPE, WORKFLOW_ROUTE)
  )
  SELECT
    :snapshot_ts,
    c.COMPANY,
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM MART_OPERATIONAL_ROUTE_COVERAGE existing
  ...
  INSERT INTO MART_EXECUTIVE_VALUE_LEDGER (
    SNAPSHOT_TS, COMPANY, ENVIRONMENT, WINDOW_DAYS, METRIC, STATUS, WORKFLOW_ROUTE,
  ...
  (WITH value_items AS (
    SELECT
      ...
      COALESCE(ENVIRONMENT, 'ALL') AS ENVIRONMENT,
      STATUS,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), 'DBA / Cost attribution') AS WORKFLOW_ROUTE,
    ...
    SELECT
      ...
      COALESCE(ENVIRONMENT, 'ALL') AS ENVIRONMENT,
      STATUS,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEWED_BY, ''), NULLIF(WORKFLOW_ROUTE, ''), 'DBA / Cost attribution') AS WORKFLOW_ROUTE,
  ...
  GROUP BY v.COMPANY, v.ENVIRONMENT, w.WINDOW_DAYS, COALESCE(v.STATUS, 'Proposed'), v.WORKFLOW_ROUTE, v.CONFIDENCE);
  ...
  SELECT
    ...
    'DBA / Cost attribution' AS WORKFLOW_ROUTE,
...
BEGIN
  ...
  INSERT INTO OVERWATCH_PRODUCTION_VALIDATION_STATUS (
    ...
    FRESHNESS_MINUTES, WORKFLOW_ROUTE, RUNBOOK_STEP, CONFIDENCE
  )
...
BEGIN
  ...
  INSERT INTO OVERWATCH_EXECUTIVE_SCORECARD_HISTORY (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, VALUE_AT_RISK_USD, CONFIDENCE, SOURCE_OBJECTS,
    LAST_REFRESHED_TS
  )
  (WITH companies AS (
    SELECT COLUMN1::VARCHAR AS COMPANY
  ...
  trust_agg AS (
    SELECT
      ...
      MAX(WORKFLOW_ROUTE) AS WORKFLOW_ROUTE,
  ...
  latest_value AS (
    ...
    QUALIFY ROW_NUMBER() OVER (
      PARTITION BY COALESCE(COMPANY, 'ALL'), COALESCE(ENVIRONMENT, 'ALL'), STATUS, WORKFLOW_ROUTE, CONFIDENCE
      ORDER BY SNAPSHOT_TS DESC, LOAD_TS DESC
    ) = 1
  ),
  value_agg AS (
  ...
  alert_agg AS (
    SELECT
      ...
      MAX(COALESCE(WORKFLOW_ROUTE, 'DBA / Alert reviewer')) AS WORKFLOW_ROUTE,
  ...
  action_agg AS (
    SELECT
      ...
      MAX(COALESCE(WORKFLOW_ROUTE, REVIEWED_BY, WORKFLOW_ROUTE, 'DBA Review')) AS WORKFLOW_ROUTE,
  ...
  raw_scores AS (
    SELECT
      ...
      'DBA / Platform' AS WORKFLOW_ROUTE,
      FALSE AS WORKFLOW_GAP,
    ...
    SELECT
      ...
      COALESCE(val.WORKFLOW_ROUTE, 'DBA / Cost attribution'),
      COALESCE(val.WORKFLOW_ROUTE, '') = '',
    ...
    SELECT
      ...
      COALESCE(sec_own.WORKFLOW_ROUTE, alerts.WORKFLOW_ROUTE, 'Security / DBA'),
      COALESCE(sec_own.GAP_ITEMS, 0) > 0,
      0::NUMBER(18,2),
      IFF(alerts.COMPANY IS NULL AND sec_own.COMPANY IS NULL, 'fallback', 'estimated'),
      'ALERT_EVENTS; MART_OPERATIONAL_ROUTE_COVERAGE',
    ...
    SELECT
      ...
      'Open Alert Center and DBA Control Room to assign route, SLA, and next action.',
      COALESCE(actions.WORKFLOW_ROUTE, own.WORKFLOW_ROUTE, alerts.WORKFLOW_ROUTE, 'DBA Review'),
      ...
      'ALERT_EVENTS; MART_OPERATIONAL_ROUTE_COVERAGE; OVERWATCH_ACTION_QUEUE',
  ...
  annotated AS (
    SELECT
      ...
      COALESCE(NULLIF(r.WORKFLOW_ROUTE, ''), cfg.WORKFLOW_ROUTE) AS WORKFLOW_ROUTE,
      COALESCE(r.WORKFLOW_GAP, FALSE) AS WORKFLOW_GAP,
...
BEGIN
  ...
  INSERT INTO OVERWATCH_FORECAST_HISTORY (
    ...
    METHODOLOGY, MAIN_DRIVER, RECOMMENDED_ACTION, WORKFLOW_ROUTE, WORKFLOW_GAP,
  ...
  cost_daily_base AS (
    SELECT
      ...
      SUM(COALESCE(CREDITS_BILLED, CREDITS_USED, 0)) AS CREDITS_USED,
      MAX(LOAD_TS) AS LAST_REFRESHED_TS
    FROM FACT_COST_DAILY
  ...
  INSERT INTO MART_EXECUTIVE_FORECAST_SUMMARY (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, VALUE_UNIT, VALUE_AT_RISK_USD, SOURCE_OBJECTS,
    LAST_REFRESHED_TS
  )
  SELECT
    h.SNAPSHOT_TS,
...
BEGIN
  ...
  INSERT INTO OVERWATCH_CHANGE_EVENT (
    ...
    RISK_LEVEL, BUSINESS_IMPACT, WORKFLOW_ROUTE, WORKFLOW_GAP, RELATED_ALERT_COUNT,
  ...
  INSERT INTO OVERWATCH_CHANGE_CORRELATION (
    ...
    WORKFLOW_ROUTE, CONFIDENCE, EVIDENCE, LAST_REFRESHED_TS
  )
  (WITH changes AS (
    SELECT *
  ...
  INSERT INTO MART_CHANGE_INTELLIGENCE_SUMMARY (
    ...
    CHANGE_COUNT, HIGH_RISK_COUNT, WORKFLOW_GAP_COUNT, RELATED_ALERT_COUNT,
    CORRELATION_CANDIDATE_COUNT, LATEST_CHANGE_TS, TOP_OBJECT_NAME, TOP_CHANGED_BY,
    RISK_LEVEL, BUSINESS_IMPACT, WORKFLOW_ROUTE, CONFIDENCE, LAST_REFRESHED_TS
  )
  (WITH companies AS (
    SELECT COLUMN1::VARCHAR AS COMPANY
    FROM VALUES ('ALL'), ('ALFA'), ('Trexis')
  ...
  event_agg AS (
    SELECT
      ...
      COUNT_IF(WORKFLOW_GAP) AS WORKFLOW_GAP_COUNT,
  ...
  SELECT
    ...
    COALESCE(e.WORKFLOW_GAP_COUNT, 0),
...
BEGIN
  ...
  INSERT INTO OVERWATCH_ACTION_WORKFLOW (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, BUSINESS_IMPACT, RISK_LEVEL, RECOMMENDED_ACTION,
  ...
  (WITH action_queue AS (
    SELECT
      ...
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEW_STATUS, ''), NULLIF(WORKFLOW_ROUTE, ''), 'Route gap') AS WORKFLOW_ROUTE,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), NULLIF(REVIEW_STATUS, ''), NULLIF(WORKFLOW_ROUTE, '')) IS NULL AS WORKFLOW_GAP,
      COALESCE(NULLIF(RECOVERY_EVIDENCE, ''), 'Business impact must be confirmed by the route before execution.') AS BUSINESS_IMPACT,
      CASE
      ...
      'Rollback requires the captured before-state, review status, and post-rollback verification. Do not execute rollback SQL from OVERWATCH automatically.' AS ROLLBACK_GUIDANCE,
      ...
      COALESCE(NULLIF(VERIFICATION_RESULT, ''), NULLIF(VERIFICATION_NOTES, ''), NULLIF(ALLOCATION_BASIS, ''), NULLIF(RECOVERY_EVIDENCE, ''), '') AS EVIDENCE,
  ...
  alert_actions AS (
    SELECT
      ...
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), 'Route gap') AS WORKFLOW_ROUTE,
      COALESCE(NULLIF(WORKFLOW_ROUTE, ''), '') = '' AS WORKFLOW_GAP,
      COALESCE(NULLIF(BUSINESS_IMPACT, ''), NULLIF(IMPACT_ESTIMATE, ''), 'Alert requires route review to confirm business impact.') AS BUSINESS_IMPACT,
      CASE
        WHEN UPPER(COALESCE(SEVERITY, '')) IN ('CRITICAL', 'HIGH', 'MEDIUM', 'LOW') THEN INITCAP(LOWER(SEVERITY))
        ELSE 'Medium'
      END AS RISK_LEVEL,
      COALESCE(RECOMMENDED_ACTION, 'Triage alert, assign route, and verify closure evidence.') AS RECOMMENDED_ACTION,
      COALESCE(STATUS, 'New') AS ACTION_STATUS,
  ...
  dry_run_actions AS (
    SELECT
      ...
      COALESCE(NULLIF(p.REQUIRED_REVIEW, ''), 'DBA review') AS WORKFLOW_ROUTE,
      FALSE AS WORKFLOW_GAP,
      COALESCE(d.EXPECTED_EFFECT, 'Dry-run result requires route review before any action.') AS BUSINESS_IMPACT,
      'High' AS RISK_LEVEL,
  ...
  INSERT INTO OVERWATCH_ACTION_APPROVAL (
    ...
    APPROVAL_EVIDENCE, RISK_LEVEL, WORKFLOW_ROUTE, LAST_REFRESHED_TS
  )
  SELECT
    SNAPSHOT_TS,
    WORKFLOW_ID,
    ...
    APPROVED_BY,
    APPROVAL_TS,
    WORKFLOW_ROUTE AS APPROVAL_ROUTE,
  ...
  INSERT INTO MART_CLOSED_LOOP_OPERATIONS_SUMMARY (
    ...
    WORKFLOW_GAP_COUNT, EVIDENCE_COUNT, EXPECTED_SAVINGS_USD,
  ...
  SELECT
    ...
    COALESCE(a.WORKFLOW_GAP_COUNT, 0),
    ...
    CASE
      ...
      WHEN COALESCE(a.OPEN_ACTION_COUNT, 0) > 0 THEN 'Work open actions through route, approval, verification, and measurement.'
...
BEGIN
  ...
  INSERT INTO OVERWATCH_COMMAND_CENTER_FINDING (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, RELATED_CHANGES, RELATED_ALERTS,
  ...
  findings AS (
    SELECT
      ...
      'Cost movement can consume contract capacity and create avoidable spend if no route-backed action is verified.' AS BUSINESS_IMPACT,
      'Cost pressure is tied to compact spend, forecast, value-ledger, and action-lifecycle signals.' AS TECHNICAL_IMPACT,
      COALESCE(cs.WORKFLOW_ROUTE, q.DEFAULT_WORKFLOW_ROUTE) AS WORKFLOW_ROUTE,
      COALESCE(cs.WORKFLOW_GAP, FALSE) AS WORKFLOW_GAP,
    ...
    SELECT
      ...
      COALESCE(wf.WORKFLOW_ROUTE, os.WORKFLOW_ROUTE, q.DEFAULT_WORKFLOW_ROUTE),
      COALESCE(wf.WORKFLOW_GAP, os.WORKFLOW_GAP, FALSE),
    ...
    SELECT
      ...
      COALESCE(ct.BUSINESS_IMPACT, 'Recent changes can explain incident timing and workflow routing but need validation before causality is claimed.'),
      'Change intelligence uses object, alert, cost, security, and workload proximity evidence from OVERWATCH marts.',
      COALESCE(ct.WORKFLOW_ROUTE, q.DEFAULT_WORKFLOW_ROUTE),
      COALESCE(ct.WORKFLOW_GAP_COUNT, 0) > 0,
      ...
      'Validate change timing, route, affected object, and related alert evidence before escalating root-cause status.',
    ...
    SELECT
      ...
      COALESCE(os.WORKFLOW_ROUTE, wf.WORKFLOW_ROUTE, q.DEFAULT_WORKFLOW_ROUTE),
      COALESCE(os.WORKFLOW_GAP, wf.WORKFLOW_GAP, FALSE),
    ...
    SELECT
      ...
      'Security alerts ' || TO_VARCHAR(COALESCE(ar.SECURITY_ALERTS, 0))
        || '; workflow gaps ' || TO_VARCHAR(COALESCE(og.GAP_ITEMS, 0))
      ...
      'Security evidence is tied to scorecard, workflow route coverage, security changes, and alert events.',
      COALESCE(ss.WORKFLOW_ROUTE, cs.WORKFLOW_ROUTE, q.DEFAULT_WORKFLOW_ROUTE),
      COALESCE(ss.WORKFLOW_GAP, FALSE) OR COALESCE(og.GAP_ITEMS, 0) > 0,
      ...
      'Security forecast not required; deterministic score, change, alert, and route evidence used.',
      COALESCE(cl.NEXT_ACTION, q.DEFAULT_ACTION),
    ...
    LEFT JOIN workflow_gap og ON og.COMPANY = c.COMPANY
    ...
    SELECT
      ...
      'Executive risk combines health, readiness, trust, route, value, and action lifecycle exposure for leadership triage.',
      'Executive evidence is a deterministic rollup of scorecard, readiness, trust, closed-loop, and value ledger marts.',
      COALESCE(sl.WORKFLOW_ROUTE, pr.NEXT_ACTION, q.DEFAULT_WORKFLOW_ROUTE),
      COALESCE(sl.WORKFLOW_GAP, FALSE) OR COALESCE(cl.OPEN_ACTION_COUNT, 0) > 0 AND COALESCE(cl.TOP_FINDING, '') ILIKE '%Route gap%',
  ...
  INSERT INTO OVERWATCH_COMMAND_CENTER_RECOMMENDATION (
    ...
    RECOMMENDED_ACTION, RISK_LEVEL, WORKFLOW_ROUTE, EXECUTION_PLAN_REF,
  ...
  INSERT INTO MART_COMMAND_CENTER_SUMMARY (
    ...
    FINDING_COUNT, HIGH_RISK_COUNT, WORKFLOW_GAP_COUNT, RELATED_CHANGE_COUNT,
...
BEGIN
  ...
  BEGIN
    CALL SP_OVERWATCH_LOAD_SECURITY_CREDENTIAL_EXPIRATIONS();
  EXCEPTION
    WHEN OTHER THEN
      INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, LOAD_FINISHED_AT, STATUS, MESSAGE)
      VALUES ('SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS_CREDENTIALS', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), 'SKIPPED', :SQLERRM);
  END;
  ...
  scope_windows AS (
    SELECT DISTINCT COMPANY, WINDOW_DAYS
    FROM scopes
  ),
  cost_rollup AS (
    SELECT
      s.COMPANY,
      s.WINDOW_DAYS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * cfg.CREDIT_PRICE_USD, 0)) AS COST_USD,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -2 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()), COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * cfg.CREDIT_PRICE_USD, 0)) AS PRIOR_COST_USD,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_BILLED, 0), 0)) AS ACCOUNT_BILLED_CREDITS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_USED, c.CREDITS_BILLED, 0), 0)) AS ACCOUNT_USED_CREDITS,
      SUM(IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), COALESCE(c.CREDITS_ADJUSTMENT_CLOUD_SERVICES, 0), 0)) AS CLOUD_SERVICES_ADJUSTMENT,
      DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AS BILLING_WINDOW_START,
      DATEADD('DAY', -1, CURRENT_DATE()) AS BILLING_WINDOW_END,
      COUNT(DISTINCT IFF(c.USAGE_DATE >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND c.USAGE_DATE < CURRENT_DATE(), c.USAGE_DATE, NULL)) AS OBSERVED_BILLING_DAYS,
      MAX(c.USAGE_DATE)::TIMESTAMP_NTZ AS SIGNAL_TS,
      MAX(c.LOAD_TS) AS LOAD_TS
    FROM scope_windows s
    CROSS JOIN settings cfg
    LEFT JOIN FACT_COST_DAILY c
      ON UPPER(COALESCE(c.COMPANY, 'ACCOUNT-WIDE')) IN ('ACCOUNT-WIDE', 'ALL')
  ...
  warehouse_rollup AS (
    SELECT
      s.COMPANY,
      s.WINDOW_DAYS,
      SUM(IFF(wh.HOUR_START >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND wh.HOUR_START < CURRENT_DATE(), COALESCE(wh.CREDITS_USED_COMPUTE, 0), 0)) AS COMPUTE_CREDITS,
      SUM(IFF(wh.HOUR_START >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND wh.HOUR_START < CURRENT_DATE(), COALESCE(wh.CREDITS_USED_CLOUD_SERVICES, 0), 0)) AS CLOUD_SERVICES_CREDITS,
      SUM(IFF(wh.HOUR_START >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND wh.HOUR_START < CURRENT_DATE(), COALESCE(wh.CREDITS_USED_COMPUTE, 0) + COALESCE(wh.CREDITS_USED_CLOUD_SERVICES, 0), 0)) AS WAREHOUSE_CREDITS,
      SUM(IFF(wh.HOUR_START >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE()) AND wh.HOUR_START < CURRENT_DATE(), (COALESCE(wh.CREDITS_USED_COMPUTE, 0) + COALESCE(wh.CREDITS_USED_CLOUD_SERVICES, 0)) * cfg.CREDIT_PRICE_USD, 0)) AS WAREHOUSE_COST_USD,
      MAX(wh.LOAD_TS) AS LOAD_TS
    FROM scope_windows s
    CROSS JOIN settings cfg
    LEFT JOIN FACT_WAREHOUSE_HOURLY wh
      ON (s.COMPANY = 'ALL' OR wh.COMPANY = s.COMPANY)
     AND wh.HOUR_START >= DATEADD('DAY', -1 * s.WINDOW_DAYS, CURRENT_DATE())
     AND wh.HOUR_START < CURRENT_DATE()
    GROUP BY s.COMPANY, s.WINDOW_DAYS
  ),
  top_cost_driver AS (
    ...
    FROM (
      SELECT
        ...
        ROW_NUMBER() OVER (
          ...
          ORDER BY SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * cfg.CREDIT_PRICE_USD) DESC
        ) AS RN
      FROM scope_windows s
      CROSS JOIN settings cfg
  ...
  cortex_rollup AS (
    SELECT
      s.COMPANY,
      s.WINDOW_DAYS,
      SUM(COALESCE(cx.CREDITS_USED, 0)) AS CORTEX_AI_CREDITS,
      SUM(COALESCE(cx.CREDITS_USED, 0) * COALESCE(cfg.AI_CREDIT_PRICE_USD, 2.20)) AS CORTEX_COST_USD,
  ...
  alert_rollup AS (
    SELECT
      ...
      COUNT_IF(NULLIF(TRIM(COALESCE(a.WORKFLOW_ROUTE, '')), '') IS NULL AND UPPER(COALESCE(a.STATUS, 'NEW')) NOT IN ('RESOLVED', 'FIXED', 'IGNORED', 'CLOSED')) AS UNASSIGNED_ALERTS,
      ...
      MAX_BY(a.WORKFLOW_ROUTE, COALESCE(a.LAST_SEEN_AT, a.EVENT_TS, a.DETECTED_AT)) AS TOP_ALERT_ROUTE,
      MIN(IFF(
  ...
  action_rollup AS (
    SELECT
      ...
      MAX_BY(
        COALESCE(NULLIF(q.WORKFLOW_ROUTE, ''), NULLIF(q.REVIEWED_BY, ''), NULLIF(q.WORKFLOW_ROUTE, ''), NULLIF(q.REVIEW_STATUS, '')),
  ...
  credential_rollup AS (
    SELECT
      ...
      'ALL' AS ENVIRONMENT,
      s.WINDOW_DAYS,
      COUNT_IF(c.CREDENTIAL_EXPIRED_FLAG OR c.CREDENTIAL_EXPIRING_30D_FLAG) AS SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT,
      COUNT_IF(c.CREDENTIAL_EXPIRING_30D_FLAG) AS SECURITY_CREDENTIALS_EXPIRING_30D_COUNT,
      COUNT_IF(c.CREDENTIAL_EXPIRING_7D_FLAG) AS SECURITY_CREDENTIALS_EXPIRING_7D_COUNT,
      COUNT_IF(c.CREDENTIAL_EXPIRED_FLAG) AS SECURITY_CREDENTIALS_EXPIRED_COUNT,
      MIN(c.EXPIRATION_DATE) AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS,
      MIN_BY(c.USER_DISPLAY_NAME, c.DAYS_TO_EXPIRATION) AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER,
      MIN_BY(c.TYPE, c.DAYS_TO_EXPIRATION) AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE,
      IFF(COUNT_IF(c.CREDENTIAL_EXPIRING_30D_FLAG OR c.CREDENTIAL_EXPIRED_FLAG) > 0, 'due_or_expired', 'no_credentials_due') AS SECURITY_CREDENTIAL_EXPIRATION_STATUS,
      ARRAY_AGG(
        IFF(
          c.CREDENTIAL_ID IS NULL,
          NULL,
          OBJECT_CONSTRUCT_KEEP_NULL(
            'finding_key', 'CREDENTIAL_EXPIRING::' || COALESCE(c.USER_NAME, 'UNKNOWN_USER') || '::' || COALESCE(c.CREDENTIAL_ID, 'UNKNOWN_CREDENTIAL'),
            'user', c.USER_DISPLAY_NAME,
            'credential', c.CREDENTIAL_NAME,
            'type', c.TYPE,
            'status', c.STATUS,
            'expires', c.EXPIRATION_DATE,
            'days_left', c.DAYS_TO_EXPIRATION,
            'severity', c.CREDENTIAL_EXPIRATION_SEVERITY,
            'recommended_action', c.RECOMMENDED_ACTION
          )
        )
      ) WITHIN GROUP (ORDER BY c.DAYS_TO_EXPIRATION, c.EXPIRATION_DATE) AS SECURITY_CREDENTIAL_EXPIRATION_FINDINGS,
      IFF(COUNT(c.CREDENTIAL_ID) > 0 AND COUNT_IF(c.CREDENTIAL_EXPIRED_FLAG OR c.CREDENTIAL_EXPIRING_30D_FLAG) = 0, TRUE, FALSE) AS SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO,
      IFF(COUNT(c.CREDENTIAL_ID) > 0, 'available', 'pending') AS SECURITY_CREDENTIAL_SOURCE_STATUS,
      MAX(COALESCE(c.LOAD_TS, c.SNAPSHOT_TS)) AS SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS,
      IFF(COUNT(c.CREDENTIAL_ID) > 0, 'Credential expiration source current', 'Credential expiration source pending') AS SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE,
      MAX(c.SNAPSHOT_TS) AS SIGNAL_TS,
      MAX(c.LOAD_TS) AS LOAD_TS
    FROM scopes s
    LEFT JOIN MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT c
      ON (s.COMPANY = 'ALL' OR c.COMPANY = s.COMPANY)
     AND c.SNAPSHOT_TS >= DATEADD('DAY', -1 * GREATEST(s.WINDOW_DAYS, 7), CURRENT_TIMESTAMP())
    WHERE s.ENVIRONMENT = 'ALL'
    GROUP BY s.COMPANY, s.WINDOW_DAYS
  ),
  workflow_route_coverage_rollup AS (
    SELECT
      s.COMPANY,
      s.WINDOW_DAYS,
      ROUND(AVG(COALESCE(o.COVERAGE_PCT, 0)), 2) AS ROUTE_COVERAGE_PCT,
      SUM(COALESCE(o.GAP_ITEMS, 0)) AS WORKFLOW_GAP_ITEMS,
      MAX(COALESCE(o.LOAD_TS, o.SNAPSHOT_TS)) AS LOAD_TS
    FROM scopes s
    LEFT JOIN MART_OPERATIONAL_ROUTE_COVERAGE o
  ...
  facts AS (
    SELECT
      ...
      IFF(c.LOAD_TS IS NULL, NULL, c.ACCOUNT_BILLED_CREDITS) AS ACCOUNT_BILLED_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, c.COST_USD) AS ACCOUNT_BILLED_COST_USD,
      IFF(c.LOAD_TS IS NULL, NULL, c.ACCOUNT_USED_CREDITS) AS ACCOUNT_USED_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(wh.COMPUTE_CREDITS, 0)) AS COMPUTE_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(wh.CLOUD_SERVICES_CREDITS, 0)) AS CLOUD_SERVICES_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, c.CLOUD_SERVICES_ADJUSTMENT) AS CLOUD_SERVICES_ADJUSTMENT,
      IFF(c.LOAD_TS IS NULL, NULL, c.CLOUD_SERVICES_ADJUSTMENT) AS ACCOUNT_CLOUD_SERVICES_ADJUSTMENT,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(wh.WAREHOUSE_CREDITS, 0)) AS WAREHOUSE_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(wh.WAREHOUSE_COST_USD, 0)) AS WAREHOUSE_COST_ESTIMATE_USD,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(wh.WAREHOUSE_COST_USD, 0)) AS WAREHOUSE_COST_USD,
      IFF(c.LOAD_TS IS NULL, NULL, GREATEST(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0), 0)) AS SERVICE_OTHER_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, GREATEST(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0), 0) * COALESCE(cfg.CREDIT_PRICE_USD, 0)) AS SERVICE_OTHER_COST_USD,
      IFF(c.LOAD_TS IS NULL, NULL, COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0)) AS BILLING_BRIDGE_DELTA_CREDITS,
      IFF(c.LOAD_TS IS NULL, NULL, (COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0)) * COALESCE(cfg.CREDIT_PRICE_USD, 0)) AS BILLING_BRIDGE_DELTA_USD,
      CASE
        WHEN c.LOAD_TS IS NULL THEN 'pending'
        WHEN ABS(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0)) < 0.000001 THEN 'matched'
        WHEN COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) > COALESCE(wh.WAREHOUSE_CREDITS, 0) THEN 'warehouse_lower_than_billed'
        ELSE 'warehouse_higher_than_billed'
      END AS BILLING_BRIDGE_STATUS,
      COALESCE(cx.CORTEX_AI_CREDITS, 0) AS CORTEX_AI_CREDITS,
      COALESCE(cx.CORTEX_COST_USD, 0) AS CORTEX_AI_COST_USD,
      IFF(c.LOAD_TS IS NULL, 'pending',
        CASE
          WHEN ABS(COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) - COALESCE(wh.WAREHOUSE_CREDITS, 0)) < 0.000001 THEN 'matched'
          WHEN COALESCE(c.ACCOUNT_BILLED_CREDITS, 0) > COALESCE(wh.WAREHOUSE_CREDITS, 0) THEN 'warehouse_lower_than_billed'
          ELSE 'warehouse_higher_than_billed'
        END
      ) AS BILLING_RECONCILIATION_STATUS,
      c.BILLING_WINDOW_START AS BILLING_WINDOW_START,
      c.BILLING_WINDOW_END AS BILLING_WINDOW_END,
      IFF(c.LOAD_TS IS NULL, FALSE, COALESCE(c.OBSERVED_BILLING_DAYS, 0) >= LEAST(c.WINDOW_DAYS, DATEDIFF('DAY', c.BILLING_WINDOW_START, CURRENT_DATE()))) AS BILLING_WINDOW_COMPLETE,
      c.LOAD_TS AS BILLING_SOURCE_FRESHNESS_TS,
      IFF(c.LOAD_TS IS NULL, 'Billing reconciliation pending', IFF(c.OBSERVED_BILLING_DAYS < s.WINDOW_DAYS, 'Billing window may be partial because account billing lags usage.', 'Completed billing window')) AS BILLING_LATENCY_NOTE,
      c.BILLING_WINDOW_START AS BILLING_RECONCILIATION_WINDOW_START,
      c.BILLING_WINDOW_END AS BILLING_RECONCILIATION_WINDOW_END,
      IFF(c.LOAD_TS IS NULL, 'pending', 'current') AS BILLING_RECONCILIATION_FRESHNESS,
      IFF(c.LOAD_TS IS NULL OR COALESCE(c.PRIOR_COST_USD, 0) = 0, NULL, (COALESCE(c.COST_USD, 0) - COALESCE(c.PRIOR_COST_USD, 0)) / NULLIF(ABS(c.PRIOR_COST_USD), 0) * 100) AS SPEND_MOVEMENT_PCT,
      IFF(c.LOAD_TS IS NULL, NULL, IFF(COALESCE(fc.FORECAST_VALUE_USD, 0) > 0, fc.FORECAST_VALUE_USD, COALESCE(c.COST_USD, 0) / NULLIF(s.WINDOW_DAYS, 0) * 30)) AS FORECAST_RUN_RATE_USD,
      ...
      COALESCE(cred.SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT, 0) AS SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT,
      COALESCE(cred.SECURITY_CREDENTIALS_EXPIRING_30D_COUNT, 0) AS SECURITY_CREDENTIALS_EXPIRING_30D_COUNT,
      COALESCE(cred.SECURITY_CREDENTIALS_EXPIRING_7D_COUNT, 0) AS SECURITY_CREDENTIALS_EXPIRING_7D_COUNT,
      COALESCE(cred.SECURITY_CREDENTIALS_EXPIRED_COUNT, 0) AS SECURITY_CREDENTIALS_EXPIRED_COUNT,
      cred.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS,
      cred.SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER,
      cred.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE AS SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE,
      COALESCE(cred.SECURITY_CREDENTIAL_EXPIRATION_STATUS, 'source_pending') AS SECURITY_CREDENTIAL_EXPIRATION_STATUS,
      cred.SECURITY_CREDENTIAL_EXPIRATION_FINDINGS AS SECURITY_CREDENTIAL_EXPIRATION_FINDINGS,
      COALESCE(cred.SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO, FALSE) AS SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO,
      COALESCE(cred.SECURITY_CREDENTIAL_SOURCE_STATUS, 'pending') AS SECURITY_CREDENTIAL_SOURCE_STATUS,
      cred.SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS AS SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS,
      COALESCE(cred.SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE, 'Credential expiration source pending') AS SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE,
      COALESCE(route.ROUTE_COVERAGE_PCT, 0) AS ROUTE_COVERAGE_PCT,
      COALESCE(route.WORKFLOW_GAP_ITEMS, 0) AS WORKFLOW_GAP_ITEMS,
      ...
      COALESCE(cfg.AI_CREDIT_PRICE_USD, 2.20) AS AI_CREDIT_PRICE_USD,
      ...
      COALESCE(a.TOP_ALERT_EVENT_ID::VARCHAR, a.TOP_ALERT_KEY) AS TOP_ALERT_EVIDENCE_ID,
      a.TOP_ALERT_KEY AS TOP_ALERT_KEY,
      a.TOP_ALERT_ROUTE AS TOP_ALERT_ROUTE,
      ac.TOP_ACTION_ID AS TOP_ACTION_ID,
      ac.TOP_ACTION_ROUTE AS TOP_ACTION_ROUTE,
      ...
      cred.SIGNAL_TS AS CREDENTIAL_SIGNAL_TS,
      ...
      cred.LOAD_TS AS CREDENTIAL_SOURCE_TS,
      route.LOAD_TS AS ROUTE_COVERAGE_SOURCE_TS,
      ...
      GREATEST_IGNORE_NULLS(
        c.LOAD_TS, wh.LOAD_TS, cx.LOAD_TS, q.LOAD_TS, t.LOAD_TS, p.LOAD_TS, cp.LOAD_TS,
        a.LOAD_TS, alert_delivery.LOAD_TS, ack.LOAD_TS, ac.LOAD_TS, sec.LOAD_TS,
        login.LOAD_TS, grants.LOAD_TS, cred.LOAD_TS, route.LOAD_TS, val.LOAD_TS, prod.LOAD_TS, app.LOAD_TS, fc.LOAD_TS,
    ...
    LEFT JOIN warehouse_rollup wh ON wh.COMPANY = s.COMPANY AND wh.WINDOW_DAYS = s.WINDOW_DAYS
    ...
    LEFT JOIN credential_rollup cred ON cred.COMPANY = s.COMPANY AND cred.ENVIRONMENT = s.ENVIRONMENT AND cred.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN workflow_route_coverage_rollup route ON route.COMPANY = s.COMPANY AND route.WINDOW_DAYS = s.WINDOW_DAYS
  ...
  UNION ALL
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
         cfg.REQUIRED, f.ROUTE_COVERAGE_SOURCE_TS, cfg.TARGET_FRESHNESS_MINUTES, cfg.DEFAULT_CONFIDENCE, COALESCE(cfg.SUPPORTS_ENVIRONMENT, FALSE), COALESCE(cfg.ENVIRONMENT_MODE, 'not_applicable'), :snapshot_ts
  FROM TMP_SECTION_COMMAND_FACTS f
  JOIN OVERWATCH_SECTION_COMMAND_SOURCE_CONFIG cfg ON cfg.SECTION_NAME = f.SECTION_NAME AND cfg.SOURCE_KEY = 'workflow_route_coverage' AND COALESCE(cfg.ENABLED, TRUE)
  UNION ALL
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
  ...
  trend_aggregates AS (
    SELECT
      ...
      SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * s.CREDIT_PRICE_USD) AS TREND_VALUE
    FROM trend_scopes s
    JOIN FACT_COST_DAILY c
      ON (s.COMPANY = 'ALL' OR c.COMPANY = s.COMPANY)
    ...
    SELECT
      ...
      'cortex_spend',
      cx.USAGE_DATE,
      SUM(COALESCE(cx.CREDITS_USED, 0) * s.AI_CREDIT_PRICE_USD)
    FROM trend_scopes s
  ...
  GROUP BY BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, METRIC_KEY;
  ...
  WITH executive_decision AS (
    SELECT f.*,
      ...
      IFF(CORTEX_ALERTS > 0 OR COST_DELTA_USD > 0, 'DBA / Cost attribution', IFF(RISKY_GRANTS > 0, 'IAM / Security route', 'DBA Review')) AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
      DATEDIFF('MINUTE',
        CASE
  ...
  dba_decision AS (
    SELECT f.*,
      ...
      'DBA operating view of failures, queue pressure, cost, Cortex cost, security warnings, changes, and workflow action state.' AS DECISION_SUMMARY,
  ...
  alert_decision AS (
    SELECT f.*,
      ...
      'Alert family view of active, critical/high, Cortex predictive, cost, reliability, security, delivery, and workflow routing state.' AS DECISION_SUMMARY,
      ...
      IFF(ACTIVE_ALERTS > 0, 'Alert Route Route', 'DBA Review') AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
  ...
  cost_decision AS (
    SELECT f.*,
      ...
      'DBA / Cost attribution' AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
      DATEDIFF('MINUTE',
        CASE
  ...
  workload_decision AS (
    SELECT f.*,
      ...
      'DBA / Workload reviewer' AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
      DATEDIFF('MINUTE',
        CASE
  ...
  security_decision AS (
    SELECT f.*,
      CASE WHEN SOURCE_SNAPSHOT_TS IS NULL THEN 'Data Gap' WHEN FRESHNESS_MINUTES > TARGET_FRESHNESS_MINUTES THEN 'Stale' WHEN RISKY_GRANTS > 0 OR SECURITY_ALERTS > 0 OR SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN 'At Risk' WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 OR SECURITY_EVENTS > 0 OR SECURITY_OPEN_ACTIONS > 0 THEN 'Watch' ELSE 'Healthy' END AS DECISION_STATE,
      CASE WHEN SOURCE_SNAPSHOT_TS IS NULL THEN 'Security summaries have not reached the decision mart.' WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRED_COUNT) || ' credentials are expired.' WHEN SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRING_7D_COUNT) || ' credentials expire within 7 days.' WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRING_30D_COUNT) || ' credentials expire within 30 days.' WHEN RISKY_GRANTS > 0 THEN TO_VARCHAR(RISKY_GRANTS) || ' risky grants need review.' WHEN SECURITY_ALERTS > 0 THEN TO_VARCHAR(SECURITY_ALERTS) || ' security alerts are open.' WHEN SECURITY_EVENTS > 0 THEN 'Security activity needs review.' ELSE 'Security posture is inside the current action threshold.' END AS DECISION_HEADLINE,
      'Security view of failed logins, credential expirations, risky grants, privilege changes, sharing exposure, security alerts, and workflow coverage.' AS DECISION_SUMMARY,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN 'Credential expirations' WHEN RISKY_GRANTS > 0 THEN 'Risky grants' WHEN SECURITY_ALERTS > 0 THEN 'Security alerts' WHEN SECURITY_EVENTS > 0 THEN 'Security activity' ELSE 'Security baseline clear' END AS DECISION_TOP_SIGNAL,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN COALESCE(SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER, 'Credential owners') WHEN RISKY_GRANTS > 0 THEN 'Risky Grants' WHEN SECURITY_ALERTS > 0 THEN 'Security Alerts' ELSE 'Security Monitoring' END AS DECISION_TOP_ENTITY,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN 'Rotate or renew credential before expiration.' WHEN RISKY_GRANTS > 0 THEN 'Open risky grants and load proof only when needed.' WHEN SECURITY_ALERTS > 0 THEN 'Open security alerts.' ELSE 'Refresh summary only when current proof is required.' END AS DECISION_TOP_ACTION,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN 'security_credential_expirations' WHEN RISKY_GRANTS > 0 THEN 'security_risky_grants' WHEN SECURITY_ALERTS > 0 THEN 'security_alerts' ELSE 'security_overview' END AS DECISION_ROUTE_KEY,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN 'Review Credential Expirations' WHEN RISKY_GRANTS > 0 THEN 'Review Risky Grants' WHEN SECURITY_ALERTS > 0 THEN 'Open Security Alerts' ELSE 'Open Overview' END AS DECISION_ACTION_LABEL,
      'Route to the security workflow that owns the top exposure.' AS DECISION_ACTION_DETAIL,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR RISKY_GRANTS > 0 OR SECURITY_ALERTS > 0 THEN 'High' WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 OR SECURITY_EVENTS > 0 THEN 'Watch' ELSE 'Clear' END AS DECISION_SEVERITY,
      CASE WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN 96 WHEN RISKY_GRANTS > 0 THEN 92 WHEN SECURITY_ALERTS > 0 THEN 88 WHEN SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0 THEN 82 WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN 74 WHEN SECURITY_EVENTS > 0 THEN 65 ELSE 0 END AS DECISION_PRIORITY_SCORE,
      CASE
        WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN SECURITY_CREDENTIALS_EXPIRED_COUNT
        WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT
        WHEN RISKY_GRANTS > 0 THEN RISKY_GRANTS
      ...
      'IAM / Security route' AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
      DATEDIFF('MINUTE',
        CASE
          WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN CREDENTIAL_SIGNAL_TS
          WHEN RISKY_GRANTS > 0 THEN SECURITY_SIGNAL_TS
      ...
      CASE
        ...
        ELSE 'Due ' || TO_VARCHAR(ACTION_DUE_DATE)
      END AS DECISION_SLA_STATE,
      'FACT_SECURITY_OPERABILITY_DAILY; MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT; ALERT_EVENTS; MART_OPERATIONAL_ROUTE_COVERAGE' AS DECISION_EVIDENCE_SOURCE
  ...
  WITH candidates AS (
    SELECT
      ...
      'alerts' AS DECISION_IMPACT_UNIT,
      'DBA / Alert reviewer' AS DECISION_WORKFLOW_ROUTE,
      FALSE AS DECISION_WORKFLOW_GAP,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      IFF(COST_DELTA_USD > 100, 85, 70), COST_DELTA_USD, 'USD', 'DBA / Cost attribution', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      90, DBA_FAILED_QUERIES + DBA_FAILED_TASKS, 'events', 'DBA Review', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      88, FAILED_QUERIES, 'queries', 'DBA / Workload reviewer', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      86, FAILED_TASKS + FAILED_PROCEDURES + COPY_ERRORS, 'failures', 'DBA / Pipeline route', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      75, QUEUED_MS / 1000, 'seconds queued', 'DBA / Workload reviewer', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      98, CRITICAL_HIGH_ALERTS, 'alerts', 'Alert Route Route', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      72, NOTIFICATION_FAILURES, 'notifications', 'DBA / Alert reviewer', FALSE,
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      65, UNVERIFIED_SAVINGS_USD, 'USD', 'DBA / Cost attribution', FALSE,
    ...
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      'security_credential_expiration',
      'CREDENTIAL_EXPIRING::' || COMPANY || ':' || WINDOW_DAYS,
      CASE
        WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN 'Critical'
        WHEN SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0 THEN 'High'
        ELSE 'Medium'
      END,
      'Credential expirations',
      COALESCE(SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER, 'Credential owners'),
      'Rotate or renew credential before expiration.',
      'security_credential_expirations',
      IFF(SECURITY_CREDENTIALS_EXPIRED_COUNT > 0, 96, IFF(SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0, 82, 74)),
      SECURITY_CREDENTIALS_EXPIRED_COUNT + SECURITY_CREDENTIALS_EXPIRING_30D_COUNT,
      'credentials',
      'IAM / Security route',
      WORKFLOW_GAP_ITEMS > 0,
      CREDENTIAL_SIGNAL_TS,
    ...
    WHERE SECTION_NAME = 'Security Monitoring'
      AND SECURITY_CREDENTIALS_EXPIRED_COUNT + SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS,
      ...
      92, RISKY_GRANTS + ACTIVE_GRANT_ROWS, 'grants', 'IAM / Security route', WORKFLOW_GAP_ITEMS > 0,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN c.FINDING_KEY LIKE '%credential%' THEN 'user_credential'
    ...
    CASE
      ...
      WHEN c.FINDING_KEY LIKE '%credential%'
        THEN COALESCE(NULLIF(f.SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER, ''), NULLIF(c.DECISION_TOP_ENTITY, ''))
    ...
    IFF(
      c.DECISION_WORKFLOW_GAP
      OR COALESCE(NULLIF(f.TOP_ALERT_ROUTE, ''), NULLIF(f.TOP_ACTION_ROUTE, '')) IS NULL
      OR REGEXP_LIKE(UPPER(COALESCE(c.DECISION_WORKFLOW_ROUTE, '')), 'WORKFLOW_ROUTE|ROUTE|REVIEW|DBA /|IAM /|COST ATTRIBUTION|ALERT WORKFLOW_ROUTE|WORKLOAD WORKFLOW_ROUTE|PIPELINE WORKFLOW_ROUTE'),
      TRUE,
      FALSE
    ) AS DECISION_WORKFLOW_GAP_RESOLVED,
    COALESCE(NULLIF(f.TOP_ALERT_ROUTE, ''), NULLIF(f.TOP_ACTION_ROUTE, '')) AS DECISION_WORKFLOW_ID,
    COALESCE(NULLIF(f.TOP_ALERT_ROUTE, ''), NULLIF(f.TOP_ACTION_ROUTE, '')) AS DECISION_WORKFLOW_NAME,
    ...
    c.DECISION_WORKFLOW_ROUTE AS WORKFLOW_ROUTE,
    IFF(
      c.DECISION_WORKFLOW_GAP
      OR COALESCE(NULLIF(f.TOP_ALERT_ROUTE, ''), NULLIF(f.TOP_ACTION_ROUTE, '')) IS NULL
      OR REGEXP_LIKE(UPPER(COALESCE(c.DECISION_WORKFLOW_ROUTE, '')), 'WORKFLOW_ROUTE|ROUTE|REVIEW|DBA /|IAM /|COST ATTRIBUTION|ALERT WORKFLOW_ROUTE|WORKLOAD WORKFLOW_ROUTE|PIPELINE WORKFLOW_ROUTE'),
  ...
  SELECT metric_candidates.BRIEF_ID, metric_candidates.SECTION_NAME, metric_candidates.COMPANY,
         metric_candidates.ENVIRONMENT, metric_candidates.WINDOW_DAYS, metric_candidates.METRIC_KEY,
         metric_candidates.METRIC_LABEL, metric_candidates.METRIC_VALUE, metric_candidates.METRIC_NUMERIC_VALUE,
         metric_candidates.METRIC_TEXT_VALUE, metric_candidates.METRIC_FORMAT, metric_candidates.METRIC_UNIT,
         metric_candidates.METRIC_DETAIL, metric_candidates.METRIC_TONE, metric_candidates.TREND_NUMERIC_VALUE,
         metric_candidates.TREND_LABEL,
         tr.TREND_POINTS,
         tr.TREND_PERIOD,
         tr.TREND_POINT_COUNT,
         tr.TREND_QUALITY,
         tr.ZERO_FILL_POLICY,
         ...
         CASE
           WHEN metric_candidates.METRIC_KEY IN ('platform_health','production_readiness','data_trust','verified_value','workflow_route_coverage') THEN 'lower_is_worse'
           WHEN metric_candidates.METRIC_KEY IN ('top_cost_driver','hottest_warehouse','top_dba_risk') THEN 'neutral'
         ...
         CASE
           WHEN metric_candidates.METRIC_KEY IN ('cortex_spend','cortex_predictive_alerts','cortex_spend_share','cortex_risk') THEN 'cortex_daily'
           WHEN metric_candidates.METRIC_KEY IN ('cortex_cost','cost_24h') THEN 'dba_control_room'
           WHEN metric_candidates.METRIC_KEY IN ('total_spend','spend_movement_pct','top_cost_driver') THEN 'cost_daily'
           WHEN metric_candidates.METRIC_KEY IN ('budget_contract_risk') THEN 'forecast'
           WHEN metric_candidates.METRIC_KEY IN ('forecast_run_rate') THEN 'forecast'
           WHEN metric_candidates.METRIC_KEY IN ('verified_savings','unverified_savings') THEN 'value_ledger'
           WHEN metric_candidates.METRIC_KEY IN ('critical_high','critical_high_issues','active_alerts','overdue_alerts','cost_alerts','reliability_alerts','cortex_predictive','unassigned_alerts') THEN 'alert_events'
           WHEN metric_candidates.METRIC_KEY = 'security_alerts' AND metric_candidates.SECTION_NAME = 'Security Monitoring' THEN 'security_alerts'
           WHEN metric_candidates.METRIC_KEY = 'security_alerts' THEN 'alert_events'
           ...
           WHEN metric_candidates.METRIC_KEY IN ('credential_expirations') THEN 'credential_expiration'
           WHEN metric_candidates.METRIC_KEY IN ('risky_grants','privilege_changes','access_changes') THEN 'grant_daily'
           WHEN metric_candidates.METRIC_KEY IN ('mfa_gaps','sharing_exposure','security_warnings','security_risk') THEN 'security_operability'
           WHEN metric_candidates.METRIC_KEY IN ('workflow_route_coverage','unassigned_findings') THEN 'workflow_route_coverage'
           WHEN metric_candidates.METRIC_KEY IN ('recent_changes','recent_workload_changes') THEN 'change_summary'
           WHEN metric_candidates.METRIC_KEY IN ('platform_health') THEN 'executive_observability'
           WHEN metric_candidates.METRIC_KEY IN ('production_readiness') THEN 'production_readiness'
           WHEN metric_candidates.METRIC_KEY IN ('data_trust') THEN 'data_trust'
           WHEN metric_candidates.METRIC_KEY IN ('verified_value') THEN 'value_ledger'
           WHEN metric_candidates.METRIC_KEY IN ('top_dba_risk') THEN 'dba_control_room'
           WHEN metric_candidates.METRIC_KEY = 'hottest_warehouse' AND metric_candidates.SECTION_NAME = 'DBA Control Room' THEN 'dba_control_room'
           WHEN metric_candidates.METRIC_KEY = 'hottest_warehouse' THEN 'query_hourly'
           WHEN metric_candidates.METRIC_KEY IN ('monitoring_overhead') THEN 'app_observability'
           ELSE NULL
         END AS SOURCE_KEY,
         metric_candidates.CONFIDENCE,
         metric_candidates.SORT_ORDER, :snapshot_ts, CURRENT_TIMESTAMP()
  FROM (
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'total_spend' AS METRIC_KEY, 'Total Spend' AS METRIC_LABEL,
           IFF(ACCOUNT_BILLED_COST_USD IS NULL, 'Billing reconciliation pending', '$' || TO_VARCHAR(ROUND(ACCOUNT_BILLED_COST_USD, 2))) AS METRIC_VALUE,
           ACCOUNT_BILLED_COST_USD AS METRIC_NUMERIC_VALUE,
           IFF(ACCOUNT_BILLED_COST_USD IS NULL, 'Billing reconciliation pending', NULL) AS METRIC_TEXT_VALUE,
           'currency' AS METRIC_FORMAT, 'USD' AS METRIC_UNIT, 'Account billed spend for the selected completed window' AS METRIC_DETAIL, IFF(COST_DELTA_USD > 0, 'warning', 'neutral') AS METRIC_TONE,
           COST_DELTA_USD AS TREND_NUMERIC_VALUE, IFF(COST_DELTA_USD >= 0, '+$', '-$') || TO_VARCHAR(ABS(ROUND(COST_DELTA_USD, 2))) AS TREND_LABEL, CONFIDENCE, 10 AS SORT_ORDER
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_spend', 'Cortex AI Spend',
           '$' || TO_VARCHAR(ROUND(CORTEX_AI_COST_USD, 2)), CORTEX_AI_COST_USD, NULL, 'currency', 'USD',
           'Cortex AI selected-window spend', IFF(CORTEX_AI_COST_USD > 0, 'info', 'neutral'), NULL, TO_VARCHAR(ROUND(CORTEX_REQUESTS, 0)) || ' requests', CONFIDENCE, 20
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'critical_high', 'Critical / High',
           ...
           'Open critical and high alert events', IFF(CRITICAL_HIGH_ALERTS > 0, 'critical', 'neutral'), NULL, TO_VARCHAR(ACTIVE_ALERTS) || ' active', CONFIDENCE, 30
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_predictive', 'Cortex Predictive Alerts',
           ...
           'Cortex forecast, anomaly, or run-rate alerts', IFF(CORTEX_ALERTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 40
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'failed_queries', 'Failed SQL',
           ...
           'Failed query count in selected window', IFF(FAILED_QUERIES > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 50
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'risky_grants', 'Risky Grants',
           ...
           'Grant/access review blockers', IFF(RISKY_GRANTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 80
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'credential_expirations', 'Credential expirations',
           CASE
             WHEN NOT SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO
                  AND COALESCE(SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT, 0) = 0
                  AND COALESCE(SECURITY_CREDENTIAL_SOURCE_STATUS, 'pending') <> 'available'
               THEN 'Credential expiration source pending'
             WHEN SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRED_COUNT) || ' expired · ' || TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRING_30D_COUNT) || ' due within 30d'
             WHEN SECURITY_CREDENTIALS_EXPIRING_30D_COUNT > 0 THEN TO_VARCHAR(SECURITY_CREDENTIALS_EXPIRING_30D_COUNT) || ' due within 30d'
             ELSE 'No credentials due within 30d'
           END,
           IFF(
             NOT SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO
             AND COALESCE(SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT, 0) = 0
             AND COALESCE(SECURITY_CREDENTIAL_SOURCE_STATUS, 'pending') <> 'available',
             NULL,
             SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT
           ...
           CASE
             WHEN NOT SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO
                  AND COALESCE(SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT, 0) = 0
                  AND COALESCE(SECURITY_CREDENTIAL_SOURCE_STATUS, 'pending') <> 'available'
               THEN 'Security source pending'
             WHEN SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER IS NULL THEN 'No credential expiration exposure in the selected window'
             ELSE 'Next: ' || SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER || ' · ' || COALESCE(SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE, 'credential') || IFF(SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS IS NULL, '', ' · ' || TO_VARCHAR(DATEDIFF('DAY', CURRENT_DATE(), SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS::DATE)) || 'd')
           END,
           IFF(SECURITY_CREDENTIALS_EXPIRED_COUNT > 0 OR SECURITY_CREDENTIALS_EXPIRING_7D_COUNT > 0, 'warning', 'neutral'),
           NULL,
           IFF(SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS IS NULL, '', 'Next ' || TO_VARCHAR(SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS)),
           CONFIDENCE,
           82
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'top_cost_driver', 'Top Cost Driver',
           ...
           'Largest known cost/service driver', 'neutral', NULL, '', CONFIDENCE, 110
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'platform_health', 'Platform Health',
           ...
           IFF(CRITICAL_HIGH_ALERTS > 0 OR RISKY_GRANTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 1
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'spend_movement_pct', 'Spend Movement',
           IFF(SPEND_MOVEMENT_PCT IS NULL, 'Billing reconciliation pending', TO_VARCHAR(ROUND(SPEND_MOVEMENT_PCT, 1)) || '%'),
           SPEND_MOVEMENT_PCT,
           ...
           IFF(COST_DELTA_USD >= 0, '+$', '-$') || TO_VARCHAR(ABS(ROUND(COST_DELTA_USD, 2))), CONFIDENCE, 11
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME IN ('Executive Landing', 'Cost & Contract')
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'critical_high_issues', 'Critical / High Issues',
           ...
           IFF(CRITICAL_HIGH_ALERTS + RISKY_GRANTS > 0, 'critical', 'neutral'), NULL, '', CONFIDENCE, 12
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_risk', 'Cortex Risk',
           ...
           'Cortex predictive, anomaly, or cost-risk signals', IFF(CORTEX_ALERTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 21
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'operational_risk', 'Operational Risk',
           ...
           'Workload failure and pipeline risk signals', IFF(FAILED_QUERIES + FAILED_TASKS + FAILED_PROCEDURES + COPY_ERRORS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 22
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'security_risk', 'Security Risk',
           ...
           'Security alerts, risky grants, and security operability signals', IFF(RISKY_GRANTS + SECURITY_ALERTS + SECURITY_EVENTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 23
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'verified_value', 'Verified Value',
           ...
           IFF(COALESCE(VERIFIED_SAVINGS_USD, 0) > 0, 'positive', 'neutral'), NULL, '', CONFIDENCE, 26
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'monitoring_overhead', 'Monitoring Overhead',
           ...
           IFF(COALESCE(MONITORING_OVERHEAD_USD, 0) > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 27
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Executive Landing'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cost_24h', 'Cost 24h',
           ...
           'DBA control-room 24-hour credit cost estimate', IFF(DBA_CREDITS > 0, 'watch', 'neutral'), NULL, TO_VARCHAR(ROUND(DBA_CREDITS, 2)) || ' credits', CONFIDENCE, 71
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_cost', 'Cortex Cost',
           ...
           'DBA control-room Cortex cost watch', IFF(DBA_CORTEX_COST > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 72
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'DBA Control Room'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'security_warnings', 'Security Warnings',
           ...
           'DBA-coordinated security warning count', IFF(DBA_SECURITY_EVENTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 73
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'DBA Control Room'
    UNION ALL
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'active_alerts', 'Active Alerts',
           ...
           'Active alert events in selected window', IFF(ACTIVE_ALERTS > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 31
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'overdue_alerts', 'Overdue Alerts',
           ...
           'Alerts/actions past due date', IFF(OVERDUE_ACTIONS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 32
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cost_alerts', 'Cost Alerts',
           ...
           'Cost alert family count', IFF(COST_ALERTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 33
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'reliability_alerts', 'Reliability Alerts',
           ...
           'Reliability and operations alert family count', IFF(GREATEST(ACTIVE_ALERTS - COST_ALERTS - SECURITY_ALERTS - CORTEX_ALERTS, 0) > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 34
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'security_alerts', 'Security Alerts',
           TO_VARCHAR(SECURITY_ALERTS), SECURITY_ALERTS, NULL, 'integer', 'alerts',
           'Security alert family count', IFF(SECURITY_ALERTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 35
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Alert Center'
    UNION ALL
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'forecast_run_rate', 'Forecast / Run-rate',
           IFF(FORECAST_RUN_RATE_USD IS NULL, 'Billing reconciliation pending', '$' || TO_VARCHAR(ROUND(FORECAST_RUN_RATE_USD, 2))),
           FORECAST_RUN_RATE_USD, IFF(FORECAST_RUN_RATE_USD IS NULL, 'Billing reconciliation pending', NULL), 'currency', 'USD',
           IFF(FORECAST_VALUE_USD > 0, 'Forecast mart value from forecast summary', 'Run-rate fallback from account billed spend'), IFF(COST_DELTA_USD > 0, 'warning', 'neutral'), COST_DELTA_USD, IFF(FORECAST_VALUE_USD > 0, 'forecast', 'run-rate fallback'), CONFIDENCE, 12
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Cost & Contract'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_spend_share', 'Cortex Spend Share',
           IFF(ACCOUNT_BILLED_COST_USD IS NULL OR ACCOUNT_BILLED_COST_USD = 0, 'Billing reconciliation pending', TO_VARCHAR(ROUND(CORTEX_AI_COST_USD / NULLIF(ACCOUNT_BILLED_COST_USD, 0) * 100, 1)) || '%'),
           IFF(ACCOUNT_BILLED_COST_USD IS NULL OR ACCOUNT_BILLED_COST_USD = 0, NULL, CORTEX_AI_COST_USD / NULLIF(ACCOUNT_BILLED_COST_USD, 0) * 100),
           IFF(ACCOUNT_BILLED_COST_USD IS NULL OR ACCOUNT_BILLED_COST_USD = 0, 'Billing reconciliation pending', NULL), 'percentage', 'percent',
           'Cortex AI share of account billed spend', IFF(CORTEX_AI_COST_USD > 0, 'info', 'neutral'), NULL, '', CONFIDENCE, 13
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Cost & Contract'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'cortex_predictive_alerts', 'Cortex Predictive Alerts',
           TO_VARCHAR(CORTEX_ALERTS), CORTEX_ALERTS, NULL, 'integer', 'alerts',
           'Cortex predictive cost alert count', IFF(CORTEX_ALERTS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 14
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Cost & Contract'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'budget_contract_risk', 'Budget / Contract Risk',
           ...
           'Forecast or contract threshold signals from the forecast mart and settings', IFF(BUDGET_CONTRACT_RISK > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 15
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Cost & Contract'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'verified_savings', 'Verified Savings',
           ...
           IFF(VALUE_LEDGER_SOURCE_TS IS NULL, 'Value ledger mart has no current summary row', 'Verified savings from the executive value ledger'),
           IFF(COALESCE(VERIFIED_SAVINGS_USD, 0) > 0, 'positive', 'neutral'), NULL, '', CONFIDENCE, 16
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Cost & Contract'
    UNION ALL
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'spill_bytes', 'Spill Bytes',
           ...
           'Bytes spilled by workload queries', IFF(SPILL_BYTES > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 73
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Workload Operations'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'long_running_queries', 'Long Running Queries',
           ...
           'Recent query detail rows still running beyond the long-running threshold', IFF(LONG_RUNNING_QUERIES > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 74
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Workload Operations'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'recent_workload_changes', 'Recent Workload Changes',
           TO_VARCHAR(RECENT_WORKLOAD_CHANGES), RECENT_WORKLOAD_CHANGES, NULL, 'integer', 'changes',
           'Recent workload-related change intelligence rows', IFF(RECENT_WORKLOAD_CHANGES > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 75
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Workload Operations'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'suspended_tasks', 'Suspended Tasks',
           ...
           'Suspended tasks observed in the task snapshot', IFF(SUSPENDED_TASKS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 76
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Workload Operations'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'copy_load_failures', 'Copy / Load Failures',
           ...
           'Copy/load error count', IFF(COPY_ERRORS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 77
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Workload Operations'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'failed_logins', 'Failed Logins',
           ...
           'Failed login count from login daily facts', IFF(FAILED_LOGINS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 81
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'mfa_gaps', 'MFA Gaps',
           ...
           'Security controls requiring MFA or identity review', IFF(MFA_GAPS > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 82
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'sharing_exposure', 'Sharing Exposure',
           ...
           'Sharing or no-database-context exposure rows from security operability facts', IFF(SHARING_EXPOSURE > 0, 'warning', 'neutral'), NULL, '', CONFIDENCE, 83
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'privilege_changes', 'Privilege Changes',
           ...
           'Active grant rows from grant daily facts', IFF(ACTIVE_GRANT_ROWS > 0, 'watch', 'neutral'), NULL, '', CONFIDENCE, 84
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
  ...
  LEFT JOIN TMP_SECTION_METRIC_TRENDS tr
    ON tr.BRIEF_ID = metric_candidates.BRIEF_ID
   AND tr.SECTION_NAME = metric_candidates.SECTION_NAME
   AND tr.COMPANY = metric_candidates.COMPANY
   AND COALESCE(tr.ENVIRONMENT, 'ALL') = COALESCE(metric_candidates.ENVIRONMENT, 'ALL')
   AND tr.WINDOW_DAYS = metric_candidates.WINDOW_DAYS
   AND tr.METRIC_KEY = metric_candidates.METRIC_KEY;
  ...
  INSERT INTO MART_SECTION_COMMAND_EXCEPTION (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, FIRST_SEEN_TS, DUE_TS, ROUTE_ID, ROUTE_NAME,
  ...
  SELECT
    ...
    DECISION_WORKFLOW_ROUTE,
    DECISION_WORKFLOW_GAP_RESOLVED,
    FIRST_SEEN_TS,
    DUE_TS,
    DECISION_WORKFLOW_ID,
    DECISION_WORKFLOW_NAME,
  ...
  JOIN (
    SELECT * FROM VALUES
      ...
      ('Security Monitoring', 'credential_expiration_review', 'security_credential_expirations', 'Review Credential Expirations', 'Open expiring credentials from compact security evidence.', 'Review Credentials', 'Security Monitoring', 'Credential Expirations', 5),
  ...
  SELECT
    ...
    l.ACCOUNT_BILLED_CREDITS,
    l.ACCOUNT_BILLED_COST_USD,
    l.ACCOUNT_USED_CREDITS,
    l.COMPUTE_CREDITS,
    l.CLOUD_SERVICES_CREDITS,
    l.CLOUD_SERVICES_ADJUSTMENT,
    l.ACCOUNT_CLOUD_SERVICES_ADJUSTMENT,
    l.WAREHOUSE_CREDITS,
    l.WAREHOUSE_COST_ESTIMATE_USD,
    l.WAREHOUSE_COST_USD,
    l.SERVICE_OTHER_CREDITS,
    l.SERVICE_OTHER_COST_USD,
    l.BILLING_BRIDGE_DELTA_CREDITS,
    l.BILLING_BRIDGE_DELTA_USD,
    l.BILLING_BRIDGE_STATUS,
    l.CORTEX_AI_CREDITS,
    l.CORTEX_AI_COST_USD,
    l.BILLING_RECONCILIATION_STATUS,
    l.BILLING_WINDOW_START,
    l.BILLING_WINDOW_END,
    l.BILLING_WINDOW_COMPLETE,
    l.BILLING_SOURCE_FRESHNESS_TS,
    l.BILLING_LATENCY_NOTE,
    l.BILLING_RECONCILIATION_WINDOW_START,
    l.BILLING_RECONCILIATION_WINDOW_END,
    l.BILLING_RECONCILIATION_FRESHNESS,
    l.SPEND_MOVEMENT_PCT,
    l.FORECAST_RUN_RATE_USD,
    l.SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT,
    l.SECURITY_CREDENTIALS_EXPIRING_30D_COUNT,
    l.SECURITY_CREDENTIALS_EXPIRING_7D_COUNT,
    l.SECURITY_CREDENTIALS_EXPIRED_COUNT,
    l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS,
    l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER,
    l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE,
    l.SECURITY_CREDENTIAL_EXPIRATION_STATUS,
    l.SECURITY_CREDENTIAL_EXPIRATION_FINDINGS,
    l.SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO,
    l.SECURITY_CREDENTIAL_SOURCE_STATUS,
    l.SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS,
    l.SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE,
    l.LOAD_TS,
    ...
    OBJECT_CONSTRUCT_KEEP_NULL(
      ...
      'ACCOUNT_BILLED_CREDITS', l.ACCOUNT_BILLED_CREDITS,
      'ACCOUNT_BILLED_COST_USD', l.ACCOUNT_BILLED_COST_USD,
      'ACCOUNT_USED_CREDITS', l.ACCOUNT_USED_CREDITS,
      'COMPUTE_CREDITS', l.COMPUTE_CREDITS,
      'CLOUD_SERVICES_CREDITS', l.CLOUD_SERVICES_CREDITS,
      'CLOUD_SERVICES_ADJUSTMENT', l.CLOUD_SERVICES_ADJUSTMENT,
      'ACCOUNT_CLOUD_SERVICES_ADJUSTMENT', l.ACCOUNT_CLOUD_SERVICES_ADJUSTMENT,
      'WAREHOUSE_CREDITS', l.WAREHOUSE_CREDITS,
      'WAREHOUSE_COST_ESTIMATE_USD', l.WAREHOUSE_COST_ESTIMATE_USD,
      'WAREHOUSE_COST_USD', l.WAREHOUSE_COST_USD,
      'SERVICE_OTHER_CREDITS', l.SERVICE_OTHER_CREDITS,
      'SERVICE_OTHER_COST_USD', l.SERVICE_OTHER_COST_USD,
      'BILLING_BRIDGE_DELTA_CREDITS', l.BILLING_BRIDGE_DELTA_CREDITS,
      'BILLING_BRIDGE_DELTA_USD', l.BILLING_BRIDGE_DELTA_USD,
      'BILLING_BRIDGE_STATUS', l.BILLING_BRIDGE_STATUS,
      'CORTEX_AI_CREDITS', l.CORTEX_AI_CREDITS,
      'CORTEX_AI_COST_USD', l.CORTEX_AI_COST_USD,
      'BILLING_RECONCILIATION_STATUS', l.BILLING_RECONCILIATION_STATUS,
      'BILLING_WINDOW_START', l.BILLING_WINDOW_START,
      'BILLING_WINDOW_END', l.BILLING_WINDOW_END,
      'BILLING_WINDOW_COMPLETE', l.BILLING_WINDOW_COMPLETE,
      'BILLING_SOURCE_FRESHNESS_TS', l.BILLING_SOURCE_FRESHNESS_TS,
      'BILLING_LATENCY_NOTE', l.BILLING_LATENCY_NOTE,
      'BILLING_RECONCILIATION_WINDOW_START', l.BILLING_RECONCILIATION_WINDOW_START,
      'BILLING_RECONCILIATION_WINDOW_END', l.BILLING_RECONCILIATION_WINDOW_END,
      'BILLING_RECONCILIATION_FRESHNESS', l.BILLING_RECONCILIATION_FRESHNESS,
      'SPEND_MOVEMENT_PCT', l.SPEND_MOVEMENT_PCT,
      'FORECAST_RUN_RATE_USD', l.FORECAST_RUN_RATE_USD,
      'SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT', l.SECURITY_CREDENTIAL_EXPIRATION_RISK_COUNT,
      'SECURITY_CREDENTIALS_EXPIRING_30D_COUNT', l.SECURITY_CREDENTIALS_EXPIRING_30D_COUNT,
      'SECURITY_CREDENTIALS_EXPIRING_7D_COUNT', l.SECURITY_CREDENTIALS_EXPIRING_7D_COUNT,
      'SECURITY_CREDENTIALS_EXPIRED_COUNT', l.SECURITY_CREDENTIALS_EXPIRED_COUNT,
      'SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS', l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TS,
      'SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER', l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_USER,
      'SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE', l.SECURITY_CREDENTIAL_NEXT_EXPIRATION_TYPE,
      'SECURITY_CREDENTIAL_EXPIRATION_STATUS', l.SECURITY_CREDENTIAL_EXPIRATION_STATUS,
      'SECURITY_CREDENTIAL_EXPIRATION_FINDINGS', l.SECURITY_CREDENTIAL_EXPIRATION_FINDINGS,
      'SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO', l.SECURITY_CREDENTIAL_SOURCE_CONFIRMED_ZERO,
      'SECURITY_CREDENTIAL_SOURCE_STATUS', l.SECURITY_CREDENTIAL_SOURCE_STATUS,
      'SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS', l.SECURITY_CREDENTIAL_SOURCE_FRESHNESS_TS,
      'SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE', l.SECURITY_CREDENTIAL_SOURCE_LATENCY_NOTE,
      'LOAD_TS', l.LOAD_TS,
  ...
  SELECT
    ...
    PRIMARY_ACTION_LABEL, PRIMARY_ACTION_DETAIL,
    ACCOUNT_BILLED_CREDITS, ACCOUNT_BILLED_COST_USD, ACCOUNT_USED_CREDITS, COMPUTE_CREDITS,
    CLOUD_SERVICES_CREDITS, CLOUD_SERVICES_ADJUSTMENT, ACCOUNT_CLOUD_SERVICES_ADJUSTMENT,
    WAREHOUSE_CREDITS, WAREHOUSE_COST_ESTIMATE_USD, WAREHOUSE_COST_USD,
    SERVICE_OTHER_CREDITS, SERVICE_OTHER_COST_USD, BILLING_BRIDGE_DELTA_CREDITS,
 ...
 AND cur.WINDOW_DAYS = next.WINDOW_DAYS
  ...
  WHERE COALESCE(
    ...
    AND COALESCE(
          TRY_TO_NUMBER(DECISION_PACKET:"REQUIRED_STALE_SOURCE_COUNT"::VARCHAR),
          TRY_TO_NUMBER(DECISION_PACKET:"STALE_SOURCE_COUNT"::VARCHAR),
          0
        ) = 0
  ...
  INSERT INTO MART_EXECUTIVE_DECISION_INBOX (
    ...
    WORKFLOW_ROUTE, WORKFLOW_GAP, AGE_MINUTES, SLA_STATE, ROUTE_KEY, SNAPSHOT_TS, LOAD_TS
  )
  ...
  SELECT COALESCE(SUM(ROW_COUNT), 0)
    INTO :child_rows
    FROM (
      SELECT COUNT(*) AS ROW_COUNT FROM MART_SECTION_COMMAND_METRIC WHERE SNAPSHOT_TS = :snapshot_ts
      UNION ALL
      SELECT COUNT(*) AS ROW_COUNT FROM MART_SECTION_COMMAND_EXCEPTION WHERE SNAPSHOT_TS = :snapshot_ts
      UNION ALL
      SELECT COUNT(*) AS ROW_COUNT FROM MART_SECTION_COMMAND_ACTION WHERE SNAPSHOT_TS = :snapshot_ts
      UNION ALL
      SELECT COUNT(*) AS ROW_COUNT FROM MART_SECTION_COMMAND_SOURCE WHERE SNAPSHOT_TS = :snapshot_ts
    );
...
BEGIN
  ...
  EXCEPTION
    WHEN OTHER THEN
      INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, LOAD_FINISHED_AT, STATUS, MESSAGE)
      VALUES ('SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS_FAST_CREDENTIALS', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), 'SKIPPED', :SQLERRM);
  END;
  ...
  SELECT
    ...
    'WAREHOUSE', c.WAREHOUSE_SIZE, c.COST_ATTRIBUTION, NULL,
    COALESCE(c.WAREHOUSE_NAME, c.USER_NAME, c.ROLE_NAME, c.DATABASE_NAME, c.COST_ATTRIBUTION, 'cost evidence'),
    COALESCE(c.WAREHOUSE_NAME, c.USER_NAME, c.ROLE_NAME, c.DATABASE_NAME, c.COST_ATTRIBUTION, TO_VARCHAR(c.USAGE_DATE)),
  ...
  QUALIFY ROW_NUMBER() OVER (ORDER BY COALESCE(g.CREATED_ON, TO_TIMESTAMP_NTZ(g.SNAPSHOT_DATE)) DESC, IFF(UPPER(COALESCE(g.ROLE_NAME, '')) IN ('ACCOUNTADMIN', 'SECURITYADMIN', 'SYSADMIN', 'ORGADMIN'), 'Critical', 'Medium') DESC) <= 5000;

  INSERT INTO MART_SECURITY_EVIDENCE_RECENT (
    SNAPSHOT_TS, EVENT_TS, COMPANY, ENVIRONMENT, USER_NAME, LOGIN_NAME, ROLE_NAME, GRANTEE_NAME,
    GRANT_ID, SHARE_NAME, DATABASE_NAME, OBJECT_NAME, EVIDENCE_KIND, SEVERITY, SUMMARY, DETAILS,
    USER_DISPLAY_NAME, USER_CHART_LABEL, CREDENTIAL_ID, CREDENTIAL_NAME, CREDENTIAL_TYPE,
    CREDENTIAL_STATUS, EXPIRATION_DATE, DAYS_TO_EXPIRATION, RECOMMENDED_ACTION, LOAD_TS
  )
  SELECT
    COALESCE(c.SNAPSHOT_TS, c.LOAD_TS), c.EXPIRATION_DATE, COALESCE(c.COMPANY, 'Shared/Unclassified'), COALESCE(c.ENVIRONMENT, 'ALL'),
    c.USER_NAME, c.USER_NAME, NULL, NULL,
    c.CREDENTIAL_ID, NULL, NULL, c.CREDENTIAL_NAME, 'credential_expiration',
    c.CREDENTIAL_EXPIRATION_SEVERITY,
    LEFT('Credential expiration for ' || COALESCE(c.USER_DISPLAY_NAME, c.USER_NAME, 'credential route'), 2000),
    OBJECT_CONSTRUCT_KEEP_NULL(
      'credential', c.CREDENTIAL_NAME,
      'type', c.TYPE,
      'domain', c.DOMAIN,
    ...
    CURRENT_TIMESTAMP()
  FROM MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT c
  WHERE COALESCE(c.CREDENTIAL_EXPIRING_30D_FLAG, FALSE)
     OR COALESCE(c.CREDENTIAL_EXPIRED_FLAG, FALSE)
  QUALIFY ROW_NUMBER() OVER (ORDER BY c.DAYS_TO_EXPIRATION, c.EXPIRATION_DATE, c.CREDENTIAL_ID) <= 5000;
  ...
  QUALIFY ROW_NUMBER() OVER (ORDER BY p.START_TIME DESC, IFF(UPPER(COALESCE(p.STATUS, '')) IN ('FAILED', 'FAILED_WITH_ERROR', 'ERROR'), 'High', 'Medium') DESC) <= 5000;
  ...
  UNION ALL
  SELECT 'credential_expiration', MAX(COALESCE(EXPIRATION_DATE, LOAD_TS)), COUNT(*)
    FROM MART_SECURITY_CREDENTIAL_EXPIRATIONS_CURRENT
   WHERE COALESCE(SNAPSHOT_TS, LOAD_TS) >= DATEADD('day', -7, CURRENT_TIMESTAMP())
  UNION ALL
  ...
  LEFT JOIN TMP_FAST_SOURCE_SNAPSHOT s
    ...
    OR (UPPER(b.SECTION_NAME) LIKE '%SECURITY%' AND s.SOURCE_KEY IN ('login_daily', 'grant_daily', 'credential_expiration', 'security_evidence_recent'))
    OR (UPPER(b.SECTION_NAME) LIKE '%EXECUTIVE%' AND s.SOURCE_KEY IN ('query_recent', 'alert_events', 'cost_daily', 'login_daily', 'grant_daily', 'credential_expiration'))
  ...
  SELECT
    ...
    l.PRIMARY_ACTION_LABEL, l.PRIMARY_ACTION_DETAIL,
    l.ACCOUNT_BILLED_CREDITS,
    l.ACCOUNT_BILLED_COST_USD,
    l.ACCOUNT_USED_CREDITS,
    l.COMPUTE_CREDITS,
  ...
  SELECT UUID_STRING(), CURRENT_TIMESTAMP(), 'FAST', :started_at, CURRENT_TIMESTAMP(),
         ...
         OBJECT_CONSTRUCT('brief_rows', :parent_rows, 'metric_rows', :metric_rows, 'exception_rows', :exception_rows,
                          ...
                          'command_freshness_modes',
                            (SELECT OBJECT_AGG(FRESHNESS_MODE, TO_VARIANT(MODE_COUNT))
                               FROM (
                          ...
                          'source_fact_max_ts_by_source', (SELECT OBJECT_AGG(SOURCE_KEY, TO_VARIANT(SOURCE_FACT_MAX_TS)) FROM TMP_FAST_SOURCE_SNAPSHOT),
                          'command_source_snapshot_ts_by_section',
                            (SELECT OBJECT_AGG(SECTION_NAME, TO_VARIANT(MAX_SOURCE_SNAPSHOT_TS))
                               FROM (
...
BEGIN
  ...
  cost_daily AS (
    SELECT
      ...
      SUM(COALESCE(EST_COST_USD, COALESCE(CREDITS_BILLED, CREDITS_USED, 0) * :credit_price)) AS COST_USD,
      MAX(LOAD_TS) AS LOAD_TS
    FROM FACT_COST_DAILY
  ...
  cost_driver_ranked AS (
    SELECT
      ...
      s.WINDOW_DAYS,
      COALESCE(c.SERVICE_CATEGORY, 'Unknown') || ' / ' || COALESCE(c.SERVICE_TYPE, 'Unknown') AS DRIVER,
      SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0)) AS CREDITS,
      SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * :credit_price) AS COST_USD,
      ROW_NUMBER() OVER (
        PARTITION BY s.COMPANY, s.WINDOW_DAYS
        ORDER BY
          SUM(COALESCE(c.CREDITS_BILLED, c.CREDITS_USED, 0) * :credit_price) DESC,
  ...
  cortex_rollup AS (
    SELECT
      ...
      SUM(COALESCE(cx.CREDITS_USED, 0) * :ai_credit_price) AS COST_USD,
      MAX(cx.LOAD_TS) AS LOAD_TS
    FROM scopes s
    LEFT JOIN FACT_CORTEX_DAILY cx
      ON (s.COMPANY = 'ALL' OR cx.COMPANY = s.COMPANY)
...
CREATE OR REPLACE TASK OVERWATCH_V2_FIRST_PAINT_REFRESH
  WAREHOUSE = WH_ALFA_OVERWATCH
  AFTER OVERWATCH_DECISION_BRIEF_FULL_REFRESH
AS
  CALL SP_OVERWATCH_REFRESH_V2_FIRST_PAINT_MARTS();
...
ALTER TASK OVERWATCH_V2_FIRST_PAINT_REFRESH RESUME;
