...
BEGIN
  ...
  environments AS (
    ...
    UNION
    SELECT DISTINCT COALESCE(ENVIRONMENT, 'ALL') FROM FACT_LOGIN_DAILY WHERE COALESCE(ENVIRONMENT, '') <> ''
    UNION
    SELECT DISTINCT COALESCE(ENVIRONMENT, 'ALL') FROM FACT_GRANT_DAILY WHERE COALESCE(ENVIRONMENT, '') <> ''
  ),
  settings AS (
    SELECT
  ...
  facts AS (
    ...
    LEFT JOIN query_rollup q ON q.COMPANY = s.COMPANY AND q.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN task_rollup t ON t.COMPANY = s.COMPANY AND t.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN procedure_rollup p ON p.COMPANY = s.COMPANY AND p.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN copy_rollup cp ON cp.COMPANY = s.COMPANY AND cp.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN alert_rollup a ON a.COMPANY = s.COMPANY AND a.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN action_rollup ac ON ac.COMPANY = s.COMPANY AND ac.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN security_rollup sec ON sec.COMPANY = s.COMPANY AND sec.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN login_rollup login ON login.COMPANY = s.COMPANY AND login.WINDOW_DAYS = s.WINDOW_DAYS
    LEFT JOIN grant_rollup grants ON grants.COMPANY = s.COMPANY AND grants.WINDOW_DAYS = s.WINDOW_DAYS
  ...
  SELECT
    *,
    CASE
      WHEN FINDING_KEY LIKE 'alert_%' OR FINDING_KEY LIKE '%critical_high%' THEN 'alert_family'
      WHEN FINDING_KEY LIKE '%cortex%' THEN 'cortex_service'
      WHEN FINDING_KEY LIKE '%spend%' OR FINDING_KEY LIKE '%savings%' THEN 'cost_driver'
      WHEN FINDING_KEY LIKE '%dba%' THEN 'dba_signal'
      WHEN FINDING_KEY LIKE '%query%' THEN 'query_signature'
      WHEN FINDING_KEY LIKE '%pipeline%' THEN 'pipeline'
      WHEN FINDING_KEY LIKE '%queue%' THEN 'warehouse_pressure'
      WHEN FINDING_KEY LIKE '%login%' THEN 'user_login'
      WHEN FINDING_KEY LIKE '%grant%' THEN 'grant_exposure'
      ELSE 'operational_entity'
    END AS DECISION_ENTITY_TYPE,
    DEDUPE_KEY AS DECISION_ENTITY_ID,
    DECISION_TOP_ENTITY AS DECISION_ENTITY_NAME,
    DEDUPE_KEY AS DECISION_EVIDENCE_ID,
    'Evidence source: ' || DECISION_EVIDENCE_SOURCE || '; finding: ' || FINDING_KEY AS DECISION_EVIDENCE_QUERY,
    IFF(
      DECISION_OWNER_GAP
      OR REGEXP_LIKE(UPPER(COALESCE(DECISION_OWNER_ROUTE, '')), 'OWNER|ROUTE|ON-CALL|DBA /|IAM /|COST OWNER|ALERT OWNER|WORKLOAD OWNER|PIPELINE OWNER'),
      TRUE,
      FALSE
    ) AS DECISION_OWNER_GAP_RESOLVED,
    IFF(
      REGEXP_LIKE(UPPER(COALESCE(DECISION_OWNER_ROUTE, '')), 'OWNER|ROUTE|ON-CALL|DBA /|IAM /|COST OWNER|ALERT OWNER|WORKLOAD OWNER|PIPELINE OWNER'),
      NULL,
      DECISION_OWNER_ROUTE
    ) AS DECISION_OWNER_ID,
    IFF(
      REGEXP_LIKE(UPPER(COALESCE(DECISION_OWNER_ROUTE, '')), 'OWNER|ROUTE|ON-CALL|DBA /|IAM /|COST OWNER|ALERT OWNER|WORKLOAD OWNER|PIPELINE OWNER'),
      NULL,
      DECISION_OWNER_ROUTE
    ) AS DECISION_OWNER_NAME,
    ...
    DATEDIFF('MINUTE', FIRST_SEEN_TS, :snapshot_ts) AS DECISION_AGE_MINUTES,
    CASE
      WHEN DUE_TS IS NULL THEN IFF(FIRST_SEEN_TS IS NULL, 'SLA unavailable', 'No SLA')
      WHEN DUE_TS < CURRENT_TIMESTAMP() THEN 'Overdue'
      WHEN TO_DATE(DUE_TS) = CURRENT_DATE() THEN 'Due today'
      ELSE 'Due ' || TO_VARCHAR(TO_DATE(DUE_TS))
    END AS DECISION_SLA_STATE,
    FIRST_SEEN_TS,
    DUE_TS,
    CASE
      ...
      ELSE 'Due ' || TO_VARCHAR(TO_DATE(DUE_TS))
    END AS SLA_STATE,
    DATEDIFF('MINUTE', FIRST_SEEN_TS, :snapshot_ts) AS AGE_MINUTES,
    DECISION_OWNER_ROUTE AS OWNER_ROUTE,
    IFF(
      DECISION_OWNER_GAP
      OR REGEXP_LIKE(UPPER(COALESCE(DECISION_OWNER_ROUTE, '')), 'OWNER|ROUTE|ON-CALL|DBA /|IAM /|COST OWNER|ALERT OWNER|WORKLOAD OWNER|PIPELINE OWNER'),
      TRUE,
    ...
    DECISION_EVIDENCE_SOURCE AS EVIDENCE_SOURCE
  FROM candidates
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY BRIEF_ID
    ORDER BY
      CASE DECISION_SEVERITY WHEN 'Critical' THEN 0 WHEN 'High' THEN 1 WHEN 'Medium' THEN 2 WHEN 'Watch' THEN 3 ELSE 4 END,
      DECISION_PRIORITY_SCORE DESC,
      DECISION_IMPACT_VALUE DESC
  ) <= 3;
 ...
 AND cur.WINDOW_DAYS = next.WINDOW_DAYS
  ...
  WHEN NOT MATCHED THEN INSERT (
    ...
    next.SNAPSHOT_TS, next.SOURCE_SNAPSHOT_TS, next.FRESHNESS_MINUTES, next.PACKET_BYTES, next.LOAD_TS
  );
