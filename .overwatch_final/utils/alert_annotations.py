...
WITH base AS (
    SELECT
        ...
        NULL AS ROUTE_EMAIL,
        COALESCE(r.OWNER, a.OWNER, 'DBA Review') AS REVIEW_PRIMARY,
        NULL AS REVIEW_SECONDARY,
        NULL AS REVIEW_GROUP,
        COALESCE(r.OWNER, a.ESCALATED_TO, a.OWNER, 'DBA Lead') AS REVIEW_TARGET,
        CASE WHEN r.OWNER IS NOT NULL THEN 'ALERT_RULE' ELSE 'ALERT_ROW' END AS ROUTE_SOURCE,
        CASE WHEN r.OWNER IS NOT NULL THEN 'Matched alert rule owner' ELSE 'Using alert row owner or DBA fallback' END AS ROUTE_EVIDENCE,
        COALESCE(
            r.SLA_HOURS,
...
SELECT
    sla.*,
    CASE
        WHEN SLA_STATE = 'Overdue' AND UPPER(COALESCE(SEVERITY, 'Medium')) IN ('CRITICAL', 'HIGH') THEN COALESCE(REVIEW_TARGET, ESCALATED_TO, 'DBA Lead')
        ELSE COALESCE(REVIEW_TARGET, ROUTED_OWNER, OWNER, 'DBA')
