...
USING (
  SELECT * FROM VALUES
    ...
    ('NATIVE_COST_WAREHOUSE_CREDIT_SPIKE', 'COST_WAREHOUSE_CREDIT_SPIKE', 'Cost', 'OVERWATCH_ALERT_WAREHOUSE_CREDIT_SPIKE', 'Cost & Contract', 'OVERWATCH_WH', '60 MINUTE', 'CANDIDATE', 'FACT_WAREHOUSE_HOURLY current-day credits vs 30-day baseline', 'Insert recommend-only cost movement event into ALERT_EVENTS', 'CREATE OR REPLACE ALERT OVERWATCH_ALERT_WAREHOUSE_CREDIT_SPIKE WAREHOUSE = OVERWATCH_WH SCHEDULE = ''60 MINUTE'' IF (EXISTS (SELECT 1 FROM FACT_WAREHOUSE_HOURLY WHERE HOUR_START >= DATEADD(''hour'', -24, CURRENT_TIMESTAMP()) GROUP BY WAREHOUSE_NAME HAVING SUM(METERED_CREDITS) > 10)) THEN INSERT INTO ALERT_EVENTS (ALERT_KEY, CATEGORY, SEVERITY, STATUS, OWNER, RECOMMENDED_ACTION, ENTITY_TYPE, REMEDIATION_MODE) SELECT ''COST_WAREHOUSE_CREDIT_SPIKE'', ''Cost'', ''High'', ''New'', ''DBA / Cost owner'', ''Explain warehouse credit spike before changing capacity.'', ''WAREHOUSE'', ''RECOMMEND'';', 'DROP ALERT IF EXISTS OVERWATCH_ALERT_WAREHOUSE_CREDIT_SPIKE;', FALSE, 'Recommend-only. Do not resize, suspend, or alter warehouses automatically.'),
    ...
    ('NATIVE_BEHAVIOR_USER_QUERY_ANOMALY', 'BEHAVIOR_USER_QUERY_ANOMALY', 'Behavior', 'OVERWATCH_ALERT_USER_QUERY_BEHAVIOR', 'Workload Operations', 'OVERWATCH_WH', '60 MINUTE', 'CANDIDATE', 'SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY user failure, runtime, spill, and warehouse pressure patterns', 'Insert status-review behavior event into ALERT_EVENTS', 'CREATE OR REPLACE ALERT OVERWATCH_ALERT_USER_QUERY_BEHAVIOR WAREHOUSE = OVERWATCH_WH SCHEDULE = ''60 MINUTE'' IF (EXISTS (SELECT 1 FROM SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY WHERE START_TIME >= DATEADD(''hour'', -2, CURRENT_TIMESTAMP()) GROUP BY USER_NAME, ROLE_NAME, WAREHOUSE_NAME HAVING COUNT(*) >= 10)) THEN INSERT INTO ALERT_EVENTS (ALERT_KEY, CATEGORY, SEVERITY, STATUS, OWNER, RECOMMENDED_ACTION, ENTITY_TYPE, REMEDIATION_MODE) SELECT ''BEHAVIOR_USER_QUERY_ANOMALY'', ''Behavior'', ''High'', ''New'', ''DBA / Workload reviewer'', ''Review repeated user query behavior before controls.'', ''USER'', ''STATUS_REVIEW'';', 'DROP ALERT IF EXISTS OVERWATCH_ALERT_USER_QUERY_BEHAVIOR;', FALSE, 'Status-review only. Do not cancel queries, disable users, or change grants automatically.')
