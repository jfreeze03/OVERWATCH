...
def build_streamlit_manifest_contract(root: str | Path | None = None) -> pd.DataFrame:
    ...
    rows = [
        ...
        _contract_row(
            ...
            "owner-default Streamlit deployment with no unsupported execute_as override",
            "unsupported execute_as present" if "execute_as:" in manifest else "owner default documented",
            "execute_as:" not in manifest and "owner rights by default" in manifest,
            "Do not add unsupported execute_as fields; keep owner-rights safety in app gates.",
        ),
        _contract_row(
    ...
    rows = [
        {
            ...
            "WHY_IT_MATTERS": "Keeps app execution on the approved current warehouse with Snowflake owner-default behavior.",
        },
        {
