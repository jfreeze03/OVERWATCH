...
SELECT
    ...
    'CREATE OR REPLACE TASK "' || DATABASE_NAME || '"."' || SCHEMA_NAME || '"."OVERWATCH_REFRESH_' || OBJECT_NAME || '" WAREHOUSE = WH_ALFA_OVERWATCH SCHEDULE = ''60 MINUTE'' AS CALL "' || DATABASE_NAME || '"."' || SCHEMA_NAME || '"."SP_OVERWATCH_REFRESH_' || OBJECT_NAME || '"();' AS GENERATED_TASK_STUB_SQL
FROM overwatch_dynamic_collisions
...
WITH secure_views AS (
    ...
    WHERE DELETED IS NULL
      AND UPPER(COALESCE(IS_SECURE, 'NO')) = 'YES'
      AND LAST_ALTERED >= DATEADD(day, -30, CURRENT_TIMESTAMP())
    ORDER BY TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME
    LIMIT 1000
),
dynamic_dependencies AS (
    ...
    -- OVERWATCH_ADMIN_BOUNDED_ACCOUNT_USAGE_SCAN: OBJECT_DEPENDENCIES has no event timestamp; this explicit admin audit is bounded by deterministic ordering and LIMIT.
...
SELECT
    ...
    'CREATE OR REPLACE TASK "' || d.REFERENCING_DATABASE || '"."' || d.REFERENCING_SCHEMA || '"."OVERWATCH_REFRESH_' || d.REFERENCING_OBJECT_NAME || '" WAREHOUSE = WH_ALFA_OVERWATCH SCHEDULE = ''60 MINUTE'' AS CALL "' || d.REFERENCING_DATABASE || '"."' || d.REFERENCING_SCHEMA || '"."SP_OVERWATCH_REFRESH_' || d.REFERENCING_OBJECT_NAME || '"();' AS GENERATED_TASK_STUB_SQL
