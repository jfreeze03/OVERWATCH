...
def _alert_recovery_sql_guidance(row: pd.Series | dict, reliability_kind: str) -> str:
    ...
    owner = _row_value(row, "OWNER", "WORKFLOW_ROUTE", default="DBA / owner")
...
def alert_history_to_actions(df_alerts: pd.DataFrame, company: str = "ALFA") -> list[dict]:
    ...
    for _, row in alerts.head(500).iterrows():
        ...
        review_target = _row_value(row, "WORKFLOW_ROUTE", default="") or owner_context.get("WORKFLOW_ROUTE", "")
        review_group = owner_context.get("REVIEW_STATUS") or review_target or owner
        ...
        action = {
            ...
            "Reviewed By": owner_context.get("REVIEWED_BY", ""),
            "Review Status": review_group,
            "Workflow Route": review_target,
