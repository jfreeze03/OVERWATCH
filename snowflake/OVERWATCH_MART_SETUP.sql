...
BEGIN
  INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, STATUS, MESSAGE)
  VALUES ('SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS_FAST_IMPL', :started_at, 'RUNNING', 'Started compact FAST packet publish.');
  ...
  SELECT
    ...
    OPTIONAL_STALE_SOURCE_COUNT, SOURCE_COVERAGE_PCT, DATA_AVAILABILITY_STATE, STALE_SOURCE_COUNT,
    SOURCE_GAP_DETAIL, PRIMARY_ACTION_KEY, PRIMARY_ROUTE_KEY, PRIMARY_ACTION_LABEL, PRIMARY_ACTION_DETAIL,
    LOAD_TS, METRICS, EXCEPTIONS, ACTIONS, SOURCES, PACKET_BYTES, TRUE AS IS_ACTIVE
  FROM MART_SECTION_DECISION_CURRENT_FLAT
  WHERE COALESCE(IS_ACTIVE, TRUE)
    AND WINDOW_DAYS_NORM IN (1, 7)
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY SECTION_NAME_NORM, COMPANY_NORM, ENVIRONMENT_NORM, WINDOW_DAYS_NORM
    ORDER BY LOAD_TS DESC
  ) = 1;

  SELECT COUNT(*), COALESCE(MAX(PACKET_BYTES), 0)
    INTO :packet_rows, :max_packet_bytes
  ...
  SELECT
    ...
    OPTIONAL_STALE_SOURCE_COUNT, SOURCE_COVERAGE_PCT, DATA_AVAILABILITY_STATE, STALE_SOURCE_COUNT,
    SOURCE_GAP_DETAIL, PRIMARY_ACTION_KEY, PRIMARY_ROUTE_KEY, PRIMARY_ACTION_LABEL, PRIMARY_ACTION_DETAIL,
    CURRENT_TIMESTAMP(), METRICS, EXCEPTIONS, ACTIONS, SOURCES, PACKET_BYTES, TRUE
  FROM TMP_FAST_SECTION_DECISION_PACKET_FLAT;
  ...
  UPDATE MART_SECTION_DECISION_CURRENT_FLAT flat
   ...
   WHERE COALESCE(IS_ACTIVE, TRUE)
     ...
     AND NOT EXISTS (
       SELECT 1
       FROM TMP_FAST_SECTION_DECISION_PACKET_FLAT next
       WHERE next.SECTION_NAME = flat.SECTION_NAME
         AND next.COMPANY = flat.COMPANY
         AND next.ENVIRONMENT = flat.ENVIRONMENT
         AND next.WINDOW_DAYS = flat.WINDOW_DAYS
  ...
  SELECT
    UUID_STRING(), CURRENT_TIMESTAMP(), 'FAST', :started_at, CURRENT_TIMESTAMP(),
    DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
    :packet_rows, 0, 0, 0, 0,
    :packet_rows, 0, :max_packet_bytes, COALESCE(AVG(PACKET_BYTES), 0),
    0, 0, 0, 0,
    0, TRUE,
    COUNT(DISTINCT WINDOW_DAYS_NORM), :packet_rows,
    NULL,
    OBJECT_CONSTRUCT('fast_flat_rows', :packet_rows),
    OBJECT_CONSTRUCT('max_packet_bytes', :max_packet_bytes),
    CURRENT_TIMESTAMP()
  FROM TMP_FAST_SECTION_DECISION_PACKET_FLAT;
  ...
  UPDATE OVERWATCH_LOAD_AUDIT
     SET LOAD_FINISHED_AT = CURRENT_TIMESTAMP(),
         STATUS = 'SUCCESS',
         ROWS_LOADED = :packet_rows,
         MESSAGE = 'Compact FAST packet publish completed.'
   WHERE LOAD_NAME = 'SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS_FAST_IMPL'
     AND LOAD_STARTED_AT = :started_at;

  RETURN 'OVERWATCH FAST_IMPL generated only first-viewport Decision Brief packet scopes.';
EXCEPTION
  WHEN OTHER THEN
    UPDATE OVERWATCH_LOAD_AUDIT
       SET LOAD_FINISHED_AT = CURRENT_TIMESTAMP(),
...
BEGIN
  ...
  IF (enable_cluster) THEN
    ALTER TABLE IF EXISTS MART_SECTION_DECISION_CURRENT
      CLUSTER BY (IS_ACTIVE, SECTION_NAME_NORM, COMPANY_NORM, ENVIRONMENT_NORM, WINDOW_DAYS_NORM);
    ALTER TABLE IF EXISTS MART_SECTION_DECISION_CURRENT_FLAT
      CLUSTER BY (IS_ACTIVE, SECTION_NAME_NORM, COMPANY_NORM, ENVIRONMENT_NORM, WINDOW_DAYS_NORM);
    ALTER TABLE IF EXISTS MART_SECTION_DECISION_LAST_GOOD
      CLUSTER BY (SECTION_NAME_NORM, COMPANY_NORM, ENVIRONMENT_NORM, WINDOW_DAYS_NORM);
    ALTER TABLE IF EXISTS FACT_QUERY_DETAIL_RECENT
      CLUSTER BY (QUERY_ID, QUERY_HASH, WAREHOUSE_NAME, START_TIME);
    ALTER TABLE IF EXISTS ALERT_EVENTS
      CLUSTER BY (EVENT_ID, ALERT_ID, ALERT_KEY, EVENT_TS);
    ALTER TABLE IF EXISTS FACT_GRANT_DAILY
      CLUSTER BY (ROLE_NAME, GRANTEE_NAME, GRANTED_TO, SNAPSHOT_DATE);
    ALTER TABLE IF EXISTS FACT_LOGIN_DAILY
      CLUSTER BY (USER_NAME, LOGIN_DATE);
    ALTER TABLE IF EXISTS FACT_TASK_RUN
      CLUSTER BY (TASK_NAME, ROOT_TASK_NAME, WAREHOUSE_NAME, SCHEDULED_TIME);
    ALTER TABLE IF EXISTS FACT_PROCEDURE_RUN
      CLUSTER BY (PROCEDURE_NAME, START_TIME);
    INSERT INTO OVERWATCH_PERFORMANCE_OPTIMIZATION_AUDIT (
      SETTING_VALUES, OBJECT_NAME, OPERATION, STATUS, ELAPSED_SECONDS, SKIPPED_REASON
    )
    VALUES (
      OBJECT_CONSTRUCT('OVERWATCH_ENABLE_CLUSTER_KEYS', :enable_cluster),
      'APP_OWNED_LOOKUP_MARTS',
      'CLUSTER_KEYS',
      'SUCCESS',
      DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
      NULL
  ...
  ELSE
    ...
    VALUES (
      ...
      'INFO',
      DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
      'Cluster-key optimization disabled by OVERWATCH settings.'
    );
  END IF;

  IF (enable_search) THEN
    ALTER TABLE IF EXISTS MART_SECTION_DECISION_CURRENT_FLAT
      ADD SEARCH OPTIMIZATION ON EQUALITY(SECTION_NAME_NORM, COMPANY_NORM, ENVIRONMENT_NORM, WINDOW_DAYS_NORM);
    ALTER TABLE IF EXISTS FACT_QUERY_DETAIL_RECENT
      ADD SEARCH OPTIMIZATION ON EQUALITY(QUERY_ID, QUERY_HASH, WAREHOUSE_NAME);
    INSERT INTO OVERWATCH_PERFORMANCE_OPTIMIZATION_AUDIT (
      SETTING_VALUES, OBJECT_NAME, OPERATION, STATUS, ELAPSED_SECONDS, SKIPPED_REASON
    )
    VALUES (
      OBJECT_CONSTRUCT('OVERWATCH_ENABLE_SEARCH_OPTIMIZATION', :enable_search),
      'APP_OWNED_LOOKUP_MARTS',
      'SEARCH_OPTIMIZATION',
      'SUCCESS',
      DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
  ...
  ELSE
    ...
    VALUES (
      ...
      'INFO',
      DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
      'Search Optimization is disabled by default and requires explicit admin opt-in.'
    );
  END IF;

  RETURN 'Optional OVERWATCH performance optimization DDL applied when enabled.';
EXCEPTION
  WHEN OTHER THEN
    INSERT INTO OVERWATCH_PERFORMANCE_OPTIMIZATION_AUDIT (
    ...
    VALUES (
      OBJECT_CONSTRUCT('OVERWATCH_ENABLE_CLUSTER_KEYS', :enable_cluster, 'OVERWATCH_ENABLE_SEARCH_OPTIMIZATION', :enable_search),
      'APP_OWNED_LOOKUP_MARTS',
      'OPTIONAL_PERFORMANCE_DDL',
      'WARN',
      DATEDIFF('millisecond', :started_at, CURRENT_TIMESTAMP()) / 1000,
      'Optional optimization DDL was skipped; see admin Snowflake query history for details.',
      'Optional feature unsupported or disabled in this account.'
    );
    RETURN 'Optional OVERWATCH performance optimization DDL skipped with WARN audit row.';
END;
$$;
