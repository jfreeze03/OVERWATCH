...
BEGIN
  ...
  INSERT INTO FACT_QUERY_HOURLY (
    HOUR_START, COMPANY, WAREHOUSE_NAME, USER_NAME, ROLE_NAME, DATABASE_NAME,
  ...
  SELECT
    ...
    PREDECESSORS,
    DEFINITION,
    REGEXP_SUBSTR(DEFINITION, '(?i)CALL[[:space:]]+([A-Za-z0-9_.$"]+)', 1, 1, 'e', 1) AS PROCEDURE_NAME
  ...
  INSERT INTO FACT_QUERY_DETAIL_RECENT (
    ...
    ERROR_MESSAGE, TOTAL_ELAPSED_TIME, QUEUED_OVERLOAD_TIME,
    QUEUED_PROVISIONING_TIME, QUEUED_REPAIR_TIME, BYTES_SCANNED,
    BYTES_SPILLED_TO_REMOTE_STORAGE, QUERY_TEXT, QUERY_TAG
  )
  SELECT
    QUERY_ID,
    START_TIME,
  ...
  INSERT INTO FACT_OBJECT_CHANGE (
    ...
    SCHEMA_NAME, CHANGE_CATEGORY, QUERY_TYPE, QUERY_TEXT
  )
  SELECT
    START_TIME,
    CASE
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'ALFA'
    END AS COMPANY,
    REGEXP_SUBSTR(q.QUERY_TEXT, '(?i)CALL[[:space:]]+([A-Za-z0-9_.$"]+)', 1, 1, 'e', 1) AS PROCEDURE_NAME,
...
BEGIN
  ...
  INSERT INTO FACT_GRANT_DAILY (
    SNAPSHOT_DATE, COMPANY, ROLE_NAME, GRANTEE_NAME, GRANTED_TO, DELETED_ON, GRANT_COUNT
  )
  SELECT
...
BEGIN
  ...
  BEGIN
    ...
    SELECT
      USAGE_DATE,
      CASE WHEN USER_ID ILIKE 'TRXS_%' THEN 'Trexis' ELSE 'ALFA' END AS COMPANY,
      USER_ID,
      SOURCE,
      SUM(CREDITS_USED) AS CREDITS_USED,
      ROUND(SUM(CREDITS_USED) * :credit_price, 2) AS EST_COST_USD,
    ...
    FROM (
      SELECT TO_DATE(START_TIME) AS USAGE_DATE, USER_ID, 'SNOWSIGHT' AS SOURCE, CREDITS_USED
      FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY
      WHERE START_TIME >= DATEADD('DAY', -35, CURRENT_TIMESTAMP())
      UNION ALL
      SELECT TO_DATE(START_TIME) AS USAGE_DATE, USER_ID, 'CLI' AS SOURCE, CREDITS_USED
