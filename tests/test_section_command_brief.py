...
class SectionCommandBriefTests(unittest.TestCase):
    ...
    def test_command_brief_sql_pipeline_is_wired(self):
        ...
        full_block = procs.split("CREATE OR REPLACE PROCEDURE SP_OVERWATCH_REFRESH_DECISION_BRIEFS_FULL()", 1)[1].split("CREATE OR REPLACE PROCEDURE SP_OVERWATCH_APPLY_OPTIONAL_PERFORMANCE_OPTIMIZATION()", 1)[0]
        self.assertIn("CALL SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS('FAST')", fast_impl_block)
