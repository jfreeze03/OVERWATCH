...
class FullAppRuntimeValidationTests(unittest.TestCase):
    ...
    def test_full_app_validation_artifacts_cover_current_surface_from_runtime_clicks(self):
        ...
        self.assertTrue(summary["cost_overview_no_autoload_passed"])
        ...
        for row in fallback_render["rows"]:
            ...
            self.assertEqual(row["unavailable_wall_count"], 0, row)
            self.assertTrue(row["snapshot_path"], row)
            self.assertTrue((ROOT / row["snapshot_path"]).exists(), row)
