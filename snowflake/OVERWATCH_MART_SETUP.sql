...
CREATE OR REPLACE PROCEDURE SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS()
RETURNS VARCHAR
LANGUAGE SQL
AS
$$
...
BEGIN
  INSERT INTO OVERWATCH_LOAD_AUDIT (LOAD_NAME, LOAD_STARTED_AT, STATUS, MESSAGE)
  VALUES ('SP_OVERWATCH_REFRESH_SECTION_COMMAND_BRIEFS', :started_at, 'RUNNING', 'Started primary-section command brief refresh.');
  ...
  SELECT
    MD5(SECTION_NAME || '|' || COMPANY || '|ALL|' || WINDOW_DAYS || '|' || TO_VARCHAR(:snapshot_ts, 'YYYYMMDDHH24MISSFF3')) AS BRIEF_ID,
    *,
...
BEGIN
  ...
  SELECT
    ...
    NULL,
    NULL,
    COUNT_IF(DECISION_PACKET:"DATA_AVAILABILITY_STATE"::VARCHAR = 'Data Gap'),
    COUNT_IF(
      COALESCE(DECISION_PACKET:"OPTIONAL_MISSING_SOURCE_COUNT"::NUMBER, 0) > 0
      OR COALESCE(DECISION_PACKET:"OPTIONAL_STALE_SOURCE_COUNT"::NUMBER, 0) > 0
      OR DECISION_PACKET:"DATA_AVAILABILITY_STATE"::VARCHAR = 'Stale'
    ),
    COUNT_IF(
      COALESCE(DECISION_PACKET:"REQUIRED_MISSING_SOURCE_COUNT"::NUMBER, 0) > 0
      OR COALESCE(DECISION_PACKET:"REQUIRED_STALE_SOURCE_COUNT"::NUMBER, 0) > 0
    ),
    NULL,
    CURRENT_TIMESTAMP()
  FROM MART_SECTION_DECISION_CURRENT
