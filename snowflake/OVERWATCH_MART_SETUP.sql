...
BEGIN
  ...
  DELETE FROM MART_SECTION_COMMAND_ACTION WHERE SNAPSHOT_TS >= DATEADD('HOUR', -2, :snapshot_ts);
  DELETE FROM MART_SECTION_COMMAND_EXCEPTION WHERE SNAPSHOT_TS >= DATEADD('HOUR', -2, :snapshot_ts);
  DELETE FROM MART_SECTION_COMMAND_METRIC WHERE SNAPSHOT_TS >= DATEADD('HOUR', -2, :snapshot_ts);
  DELETE FROM MART_SECTION_COMMAND_BRIEF WHERE SNAPSHOT_TS >= DATEADD('HOUR', -2, :snapshot_ts);
  ...
  alert_rollup AS (
    SELECT
      ...
      COUNT_IF(UPPER(COALESCE(a.CATEGORY, '') || ' ' || COALESCE(a.ALERT_KEY, '') || ' ' || COALESCE(a.BUSINESS_IMPACT, '')) LIKE '%CORTEX%' OR UPPER(COALESCE(a.CATEGORY, '') || ' ' || COALESCE(a.ALERT_KEY, '') || ' ' || COALESCE(a.BUSINESS_IMPACT, '')) LIKE '%PREDICT%') AS CORTEX_ALERTS,
  ...
  INSERT INTO MART_SECTION_COMMAND_BRIEF (
    ...
    RESOLVED_COMPANY, RESOLVED_ENVIRONMENT, RESOLVED_WINDOW_DAYS, CONFIDENCE, LOAD_TS
  )
  SELECT
    BRIEF_ID,
    SECTION_NAME,
    ...
    FRESHNESS_MINUTES > TARGET_FRESHNESS_MINUTES,
    COMPANY,
    ENVIRONMENT,
    WINDOW_DAYS,
    CONFIDENCE,
  ...
  INSERT INTO MART_SECTION_COMMAND_METRIC (
    ...
    TREND_LABEL, SORT_ORDER, SNAPSHOT_TS, LOAD_TS
  )
  SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, METRIC_KEY, METRIC_LABEL,
         ...
         METRIC_DETAIL, METRIC_TONE, TREND_NUMERIC_VALUE, TREND_LABEL, SORT_ORDER, :snapshot_ts, CURRENT_TIMESTAMP()
  FROM (
  ...
  INSERT INTO MART_SECTION_COMMAND_EXCEPTION (
    ...
    ROUTE_SECTION, ROUTE_WORKFLOW, SORT_ORDER, SNAPSHOT_TS, LOAD_TS
  )
  SELECT
    BRIEF_ID,
    SECTION_NAME,
