    ...
    if isinstance(errors, (list, tuple)):
        for err in [str(item).strip() for item in errors if str(item).strip()][:3]:
            _append_card(cards, {
                ...
                "do_not": "Do not use limited executive telemetry for business decisions without a data-health note.",
    ...
    if _is_df(migration):
        ...
        for _, row in blockers.head(4).iterrows():
            _append_card(cards, {
                ...
                "next_action": "Resolve data-health status before executive sign-off.",
