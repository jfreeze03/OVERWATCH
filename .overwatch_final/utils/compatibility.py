...
def build_cost_formula_audit() -> pd.DataFrame:
    ...
    rows = [
        ...
        (
            ...
            "Apply ALFA/Trexis warehouse, database, user, and environment boundaries before allocating warehouse credits.",
            "Exact boundary when naming patterns are current",
            "OVERWATCH extension",
            "Prevents scoped views from inheriting spend from the other company.",
            "Run the company-scope audit after warehouse or database naming changes.",
        ),
        (
            "Forecast run rate",
            "Year-to-date METERING_HISTORY daily credits; recent observed-day average * days remaining in year.",
            "Cost Forecast uses WAREHOUSE_METERING_HISTORY daily credits, fills missing calendar days with zero, then projects a 30-day warehouse run rate.",
            "Estimated forecast",
            "Intentional metric change",
            "OVERWATCH shows an operational 30-day warehouse forecast, not the old annual all-service projection.",
            "Add a separate annual service projection if leadership wants the original dashboard view back.",
        ),
        (
            "Storage footprint",
            "DATABASE_STORAGE_USAGE_HISTORY + STAGE_STORAGE_USAGE_HISTORY + STORAGE_USAGE hybrid/archive bytes.",
            "Storage Monitor uses fast mart rows or live DATABASE_STORAGE_USAGE_HISTORY plus STAGE_STORAGE_USAGE_HISTORY for ALL scope; company scope is database/failsafe only.",
            "Estimated dollar conversion",
            "Coverage gap to evaluate",
            "Hybrid table storage and archive cool/cold storage are account-level in the source dashboard but are not fully surfaced in OVERWATCH storage views yet.",
            "Add account-wide hybrid/archive storage rows with clear non-company-scoped labeling.",
        ),
        (
            "Storage dollars",
            ...
            "TOTAL_STORAGE_TB * configured storage $/TB/month.",
            "Estimated dollar conversion",
            "Needs formula expansion",
            "OVERWATCH is directionally correct for standard storage but can understate accounts using hybrid or archive storage.",
            "Split standard, hybrid, archive cool, and archive cold storage cost formulas before using storage dollars for contract reconciliation.",
        ),
        (
        ...
        (
            ...
            "Cortex Code Snowsight/CLI plus optional Cortex AI Functions; service lens covers broad AI/Cortex billing categories.",
            "Mixed",
            "Coverage gap to evaluate",
            "OVERWATCH has stronger spend controls for Cortex Code, but does not yet expose every source-dashboard Cortex sub-service detail.",
            "Prioritize Cortex Search, Analyst, Intelligence, and REST/API token views if those features are enabled in the account.",
        ),
        (
            "SPCS credits",
