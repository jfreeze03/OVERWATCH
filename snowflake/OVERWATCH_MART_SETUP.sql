...
BEGIN
  ...
  sections AS (
    SELECT * FROM VALUES
      ('Executive Landing', 60, 'MART_EXECUTIVE_OBSERVABILITY; MART_EXECUTIVE_SCORECARD_SUMMARY; MART_EXECUTIVE_FORECAST_SUMMARY; MART_CLOSED_LOOP_OPERATIONS_SUMMARY; MART_PRODUCTION_READINESS_SUMMARY', 'allocated'),
      ('DBA Control Room', 30, 'MART_DBA_CONTROL_ROOM; OVERWATCH_ACTION_QUEUE; MART_CHANGE_INTELLIGENCE_SUMMARY; ALERT_EVENTS', 'allocated'),
      ('Alert Center', 15, 'ALERT_EVENTS; OVERWATCH_ACTION_QUEUE; ALERT_NOTIFICATION_LOG', 'exact'),
      ('Cost & Contract', 60, 'FACT_COST_DAILY; FACT_CORTEX_DAILY; FACT_COST_MONITORING_SIGNAL; MART_EXECUTIVE_FORECAST_SUMMARY; MART_EXECUTIVE_VALUE_LEDGER', 'allocated'),
      ('Workload Operations', 30, 'FACT_QUERY_HOURLY; FACT_TASK_RUN; FACT_PROCEDURE_RUN; FACT_COPY_LOAD_DAILY; MART_CHANGE_INTELLIGENCE_SUMMARY', 'allocated'),
      ('Security Monitoring', 60, 'FACT_SECURITY_OPERABILITY_DAILY; ALERT_EVENTS; MART_OPERATIONAL_OWNER_COVERAGE; MART_CHANGE_INTELLIGENCE_SUMMARY', 'allocated')
    AS T(SECTION_NAME, TARGET_FRESHNESS_MINUTES, SOURCE_OBJECTS, CONFIDENCE)
  ),
  cost_rollup AS (
    SELECT
      s.COMPANY,
  ...
  facts AS (
    SELECT
      ...
      GREATEST_IGNORE_NULLS(
        ...
        ch.LOAD_TS, qd.LOAD_TS, ts.LOAD_TS, ctrl.LOAD_TS, ex.LOAD_TS, cfg.SETTINGS_SOURCE_TS
  ...
  SELECT
    ...
    *,
    ...
    ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';')) AS REQUIRED_SOURCE_COUNT,
    IFF(SOURCE_SNAPSHOT_TS IS NULL, 0, ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';'))) AS AVAILABLE_SOURCE_COUNT,
    ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';')) - IFF(SOURCE_SNAPSHOT_TS IS NULL, 0, ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';'))) AS MISSING_SOURCE_COUNT,
    IFF(SOURCE_SNAPSHOT_TS IS NULL, 0, ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';'))) AS AVAILABLE_REQUIRED_SOURCE_COUNT,
    ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';')) - IFF(SOURCE_SNAPSHOT_TS IS NULL, 0, ARRAY_SIZE(SPLIT(SOURCE_OBJECTS, ';'))) AS REQUIRED_MISSING_SOURCE_COUNT,
    IFF(SOURCE_SNAPSHOT_TS IS NULL OR DATEDIFF('MINUTE', SOURCE_SNAPSHOT_TS, :snapshot_ts) <= TARGET_FRESHNESS_MINUTES, 0, 1) AS REQUIRED_STALE_SOURCE_COUNT,
    ...
    IFF(SOURCE_SNAPSHOT_TS IS NULL, 0, 100) AS SOURCE_COVERAGE_PCT,
    ...
    IFF(SOURCE_SNAPSHOT_TS IS NULL OR DATEDIFF('MINUTE', SOURCE_SNAPSHOT_TS, :snapshot_ts) <= TARGET_FRESHNESS_MINUTES, 0, 1) AS STALE_SOURCE_COUNT,
  ...
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
         cfg.REQUIRED,
         (SELECT MAX(COALESCE(s.LOAD_TS, s.SNAPSHOT_TS)) FROM MART_EXECUTIVE_SCORECARD_SUMMARY s WHERE f.COMPANY = 'ALL' OR s.COMPANY = f.COMPANY),
         cfg.TARGET_FRESHNESS_MINUTES, cfg.DEFAULT_CONFIDENCE, :snapshot_ts
  FROM TMP_SECTION_COMMAND_FACTS f
  ...
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
         cfg.REQUIRED,
         (SELECT MAX(COALESCE(c.LOAD_TS, c.SNAPSHOT_TS)) FROM MART_CLOSED_LOOP_OPERATIONS_SUMMARY c WHERE f.COMPANY = 'ALL' OR c.COMPANY = f.COMPANY),
         cfg.TARGET_FRESHNESS_MINUTES, cfg.DEFAULT_CONFIDENCE, :snapshot_ts
  FROM TMP_SECTION_COMMAND_FACTS f
  ...
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
         cfg.REQUIRED,
         (SELECT MAX(COALESCE(d.LOAD_TS, d.SNAPSHOT_TS)) FROM MART_DATA_TRUST_SUMMARY d WHERE f.COMPANY = 'ALL' OR d.COMPANY = f.COMPANY),
         cfg.TARGET_FRESHNESS_MINUTES, cfg.DEFAULT_CONFIDENCE, :snapshot_ts
  FROM TMP_SECTION_COMMAND_FACTS f
  ...
  SELECT f.BRIEF_ID, f.SECTION_NAME, f.COMPANY, f.ENVIRONMENT, f.WINDOW_DAYS, cfg.SOURCE_KEY, cfg.SOURCE_OBJECT,
         cfg.REQUIRED,
         (SELECT MAX(s.SNAPSHOT_TS) FROM FACT_COST_MONITORING_SIGNAL s WHERE f.COMPANY = 'ALL' OR s.COMPANY = f.COMPANY),
         cfg.TARGET_FRESHNESS_MINUTES, cfg.DEFAULT_CONFIDENCE, :snapshot_ts
  FROM TMP_SECTION_COMMAND_FACTS f
  ...
  SELECT
    ...
    LISTAGG(IFF(REQUIRED AND SOURCE_SNAPSHOT_TS IS NULL, SOURCE_OBJECT, NULL), '; ') WITHIN GROUP (ORDER BY SOURCE_KEY) AS SOURCE_GAP_DETAIL
  ...
  SELECT
    *,
    ...
    DECISION_OWNER_GAP AS OWNER_GAP,
  ...
  SELECT
    ...
    w.CONFIDENCE,
    CASE
      WHEN w.SOURCE_SNAPSHOT_TS IS NULL THEN w.SOURCE_OBJECT || ' has no current summary row'
      WHEN DATEDIFF('MINUTE', w.SOURCE_SNAPSHOT_TS, :snapshot_ts) > w.TARGET_FRESHNESS_MINUTES THEN w.SOURCE_OBJECT || ' is stale'
      ELSE ''
    END AS GAP_REASON,
    :snapshot_ts,
    CURRENT_TIMESTAMP()
  ...
  FROM (
    ...
    SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, 'access_changes', 'Access Changes',
           TO_VARCHAR(RECENT_WORKLOAD_CHANGES), RECENT_WORKLOAD_CHANGES, NULL, 'integer', 'changes',
           'Recent access and security-sensitive change intelligence rows', IFF(RECENT_WORKLOAD_CHANGES > 0, 'watch', 'neutral'), NULL, '', 85
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME = 'Security Monitoring'
    UNION ALL
