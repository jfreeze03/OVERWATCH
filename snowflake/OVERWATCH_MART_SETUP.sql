...
BEGIN
  ...
  facts AS (
    SELECT
      ...
      COALESCE(a.NOTIFICATION_FAILURES, 0) AS NOTIFICATION_FAILURES,
      ...
      GREATEST_IGNORE_NULLS(
        ...
        a.LOAD_TS, ac.LOAD_TS, sec.LOAD_TS, ctrl.LOAD_TS, ex.LOAD_TS
  ...
  SELECT
    ...
    cfg.REQUIRED,
    CASE
      WHEN cfg.SOURCE_KEY IN ('cost_daily', 'cost_signals', 'settings', 'value_ledger') THEN f.COST_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('cortex_daily') THEN f.CORTEX_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('query_summary', 'query_hourly', 'query_recent') THEN f.QUERY_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('task_runs') THEN f.TASK_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('procedure_runs') THEN f.PROCEDURE_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('copy_load') THEN f.COPY_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('alert_events', 'security_alerts', 'acknowledgements', 'notification_log') THEN f.ALERT_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('action_queue') THEN f.ACTION_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('security_operability', 'login_daily', 'grant_daily', 'owner_coverage') THEN f.SECURITY_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('dba_control_room') THEN f.CONTROL_ROOM_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('executive_observability', 'executive_scorecard', 'executive_forecast', 'closed_loop', 'production_readiness', 'data_trust', 'app_observability', 'forecast') THEN f.EXECUTIVE_SOURCE_TS
      WHEN cfg.SOURCE_KEY IN ('change_summary') THEN GREATEST_IGNORE_NULLS(f.ACTION_SOURCE_TS, f.ALERT_SOURCE_TS, f.EXECUTIVE_SOURCE_TS)
      ELSE f.SOURCE_SNAPSHOT_TS
    END AS SOURCE_SNAPSHOT_TS,
    cfg.TARGET_FRESHNESS_MINUTES,
    cfg.DEFAULT_CONFIDENCE AS CONFIDENCE,
    :snapshot_ts AS SNAPSHOT_TS
  ...
  trend_points AS (
    SELECT
      ...
      SUM(COALESCE(c.EST_COST_USD, COALESCE(c.CREDITS_BILLED, c.CREDITS_USED_COMPUTE, 0) * COALESCE(c.RATE_USD, 3.68))) AS TREND_VALUE
    FROM trend_scopes s
    JOIN FACT_COST_DAILY c
      ON (s.COMPANY = 'ALL' OR c.COMPANY = s.COMPANY)
    ...
    SELECT
      ...
      'cortex_spend',
      cx.USAGE_DATE,
      SUM(COALESCE(cx.EST_COST_USD, COALESCE(cx.CREDITS_USED, 0) * 2.20))
    FROM trend_scopes s
    ...
    SELECT
      ...
      'risky_grants',
      sec.SNAPSHOT_DATE,
      SUM(COALESCE(sec.REVIEW_BLOCKER_ROWS, 0))
    FROM trend_scopes s
    JOIN FACT_SECURITY_OPERABILITY_DAILY sec
      ON (s.COMPANY = 'ALL' OR sec.COMPANY = s.COMPANY)
     AND sec.SNAPSHOT_DATE >= DATEADD('DAY', -1 * s.TREND_DAYS + 1, CURRENT_DATE())
    WHERE s.SECTION_NAME = 'Security Monitoring'
    GROUP BY s.BRIEF_ID, sec.SNAPSHOT_DATE
    UNION ALL
    SELECT
      s.BRIEF_ID,
      'open_actions',
  ...
  SELECT
    *,
    IFF(DECISION_AGE_MINUTES IS NULL, NULL, DATEADD('MINUTE', -1 * DECISION_AGE_MINUTES, :snapshot_ts)) AS FIRST_SEEN_TS,
    ACTION_DUE_DATE AS DUE_TS,
    DECISION_SLA_STATE AS SLA_STATE,
    DECISION_AGE_MINUTES AS AGE_MINUTES,
  ...
  SELECT BRIEF_ID, SECTION_NAME, COMPANY, ENVIRONMENT, WINDOW_DAYS, METRIC_KEY, METRIC_LABEL,
         ...
         CASE
           WHEN METRIC_KEY IN ('cortex_spend','cortex_cost','cortex_predictive','cortex_predictive_alerts','cortex_spend_share') THEN 'cortex_daily'
           WHEN METRIC_KEY IN ('total_spend','spend_movement_pct','top_cost_driver','budget_contract_risk','open_cost_actions','cost_24h','hottest_warehouse') THEN 'cost_daily'
           ...
           WHEN METRIC_KEY IN ('critical_high','active_alerts','overdue_alerts','cost_alerts','reliability_alerts','security_alerts','notification_failures','unassigned_alerts') THEN 'alert_events'
           WHEN METRIC_KEY IN ('open_actions','open_action_queue','overdue_actions','overdue_security_actions') THEN 'action_queue'
           WHEN METRIC_KEY IN ('failed_queries','queue_pressure','queue_blocked_pressure','spill_bytes','long_running_queries') THEN 'query_hourly'
           ...
           WHEN METRIC_KEY IN ('mfa_gaps','sharing_exposure','security_warnings') THEN 'security_operability'
           ...
           WHEN METRIC_KEY IN ('platform_health') THEN 'executive_scorecard'
  ...
  FROM (
    ...
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME IN ('Executive Landing', 'Alert Center', 'Security Monitoring')
    UNION ALL
    ...
    FROM TMP_SECTION_COMMAND_FACTS_TRUSTED WHERE SECTION_NAME IN ('Executive Landing', 'Alert Center', 'Cost & Contract')
    UNION ALL
