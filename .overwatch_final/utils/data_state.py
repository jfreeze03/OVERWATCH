...
DATA_STATE_SPECS: dict[DataState, DataStateSpec] = {
    ...
    DataState.NO_ROWS_FOR_SCOPE: DataStateSpec(
        ...
        user_message="No mart rows match the selected company, environment, window, or warehouse.",
        tone="neutral",
        recommended_action="Broaden the scope or verify the selected filters.",
        show_metric_cards=True,
        show_empty_table=True,
        allow_details_action=True,
    ),
    DataState.REFRESH_REQUIRED: DataStateSpec(
        short_label="Loading current summary",
        user_message="Loading the current packet for this view.",
        tone="warning",
        recommended_action="Refresh current packet.",
        show_metric_cards=True,
        show_empty_table=True,
        allow_details_action=True,
    ),
    DataState.SETUP_REQUIRED: DataStateSpec(
        short_label="Loading source setup",
        user_message="Loading source setup status.",
        tone="critical",
        recommended_action="Open Setup Health and run setup validation.",
        show_metric_cards=False,
        show_empty_table=True,
        allow_details_action=False,
    ),
    DataState.CONNECTION_UNAVAILABLE: DataStateSpec(
        short_label="Loading current source",
        user_message="Loading current source.",
        tone="critical",
        recommended_action="Reconnect or open Setup Health.",
        show_metric_cards=False,
        show_empty_table=True,
        allow_details_action=False,
    ),
    DataState.QUERY_FAILED: DataStateSpec(
        short_label="Loading current summary",
        user_message="Safe detail is available in Setup Health.",
        tone="critical",
