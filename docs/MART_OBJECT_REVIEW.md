...
| `OVERWATCH_ACTION_QUEUE`, `OVERWATCH_ALERTS`, `OVERWATCH_ADMIN_ACTION_AUDIT`, alert rule/log tables | Alert Center, recommendations, action queue, admin audit, and delivery/remediation history. Includes `ALERT_NATIVE_OBJECT_REGISTRY`, `ALERT_REMEDIATION_POLICY`, and `ALERT_REMEDIATION_DRY_RUN` for reviewed native alert candidates and future dry-run remediation. |
